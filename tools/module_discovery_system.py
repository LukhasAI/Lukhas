...
class ModuleRegistry:
    ...
    def discover_modules(self, filters: Optional[dict] = None) -> list[ModuleInfo]:
...
class ModuleDiscoveryAPI:
    ...
    def search_modules(self, query: str, filters: Optional[dict] = None) -> list[dict]:
        ...
        if query:
            query = query.lower()
            modules = [m for m in modules if query in m.name.lower() or query in m.owner.lower()]
...
def main():
    ...
    parser.add_argument(
        "--command", choices=["list", "search", "deps", "health", "map"], default="list", help="Command to execute"
    )
