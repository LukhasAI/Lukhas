...
def generate_audit_report(audit_data: Dict[str, Any], output_path: pathlib.Path):
    ...
    for module in modules:
        ...
        if issues:
            problematic_modules.append({"module": module["module"], "issues": issues, "severity": len(issues)})
...
def generate_location_reports(audit_data: Dict[str, Any], md_path: pathlib.Path, csv_path: pathlib.Path):
    ...
    for module in modules:
        ...
        location_data.append({"module": module_name, "contracts": contracts_str, "paths": paths_str})
    ...
    md_lines.extend(
        ["", f"**Total modules**: {len(location_data)}", "", f"*Generated: {audit_data.get('timestamp', 'N/A')}*"]
    )
