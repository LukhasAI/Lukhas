...
def migrate_test_file(filepath: Path) -> tuple[bool, list[str]]:
    ...
    auth_fixture_pattern = (
        r'@pytest\.fixture\s+def auth_headers\(\):[^}]+return \{"Authorization": "Bearer sk-lukhas-[^"]+"\}'
    )
        ...
        if "golden_auth_headers" not in content:
            if "from tests.smoke.fixtures import" in content:
                content = content.replace(
                    "from tests.smoke.fixtures import", "from tests.smoke.fixtures import golden_auth_headers,"
                )
            else:
                import_lines = [
                    i
...
def main():
    ...
    if migrated:
        print("\n✅ Migration complete!")
        print("\nNext steps:")
        print("1. Run: pytest tests/smoke/ -v")
        print("2. Verify all tests still pass")
        print("3. Commit: git add tests/smoke/test_*.py && git commit")
