...
async def get_stale(days: int = 30, keyinfo: dict = Depends(require_api_key)):
    ...
    conn = get_conn()
    rows = conn.execute(
        "SELECT * FROM intents WHERE status IN ('reserved','planned','committed') AND modified_at < ? ORDER BY modified_at ASC",
        (cutoff,),
    ).fetchall()
...
async def intents_by_owner(owner: str, keyinfo: dict = Depends(require_api_key)):
    conn = get_conn()
    rows = conn.execute(
        "SELECT * FROM intents WHERE owner=? ORDER BY modified_at DESC", (owner,)
    ).fetchall()
    conn.close()
...
async def metrics_summary(keyinfo: dict = Depends(require_api_key)):
    ...
    by_code_rows = conn.execute(
        "SELECT code, COUNT(*) as cnt FROM intents GROUP BY code ORDER BY cnt DESC LIMIT 20"
    ).fetchall()
    ...
    avg_resolved = conn.execute(
        "SELECT AVG(julianday(resolved_at)-julianday(created_at)) FROM intents WHERE resolved_at IS NOT NULL"
    ).fetchone()[0]
    conn.close()
    return {
        "total": total,
...
def admin_list_keys(admin_token: Optional[str] = None):
    ...
    conn = get_conn()
    rows = conn.execute(
        "SELECT key,agent_id,owner,scopes,created_at,expires_at,revoked,daily_limit,daily_used FROM api_keys"
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
