...
def record_usage(api_key, agent_id, model, prompt_tokens, completion_tokens, est_cost):
    c = conn()
    c.execute("INSERT INTO llm_usage (timestamp,api_key,agent_id,model,prompt_tokens,completion_tokens,est_cost) VALUES (?,?,?,?,?,?,?)",
              (datetime.utcnow().isoformat(), api_key, agent_id, model, prompt_tokens, completion_tokens, est_cost))
    c.execute("UPDATE api_keys SET daily_used = COALESCE(daily_used,0) + ? WHERE key=?", (est_cost, api_key))
    c.commit(); c.close()

def call_openai_chat(prompt: str, model: str, api_key_env: str="OPENAI_API_KEY", max_completion_tokens: int=1024, agent_api_key=None, agent_id=None):
    ...
    if agent_api_key:
        c = conn()
        row = c.execute("SELECT daily_limit, daily_used FROM api_keys WHERE key=?", (agent_api_key,)).fetchone()
        if row:
    ...
    resp = requests.post("https://api.openai.com/v1/chat/completions",
                         ...
                         json={"model":model, "messages":[{"role":"user","content":prompt}], "max_tokens": max_completion_tokens, "temperature":0.0}, timeout=60)
    ...
    return {"text": text, "prompt_tokens": p_actual, "completion_tokens": c_actual, "cost": actual_cost}
