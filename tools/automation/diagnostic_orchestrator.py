...
try:
    from enhanced_auto_fix_safe import apply_enhanced_fixes, apply_config_fixes
except ImportError:
...
class DiagnosticOrchestrator:
                ...
                if line.startswith('### ') and 'Error' in line:
                    ...
                    if len(parts) >= 2 and parts[0] and not parts[0].startswith('-'):
                        ...
                        errors[current_category].append({
                            'file': parts[0],
                            'details': parts[1] if len(parts) > 1 else 'Unknown'
                        })
    ...
    def apply_config_fixes(self, error_list: List[Dict]) -> Dict:
        ...
        try:
            ...
            return {
                "status": "success",
                "pytest_markers_added": results.get("pytest_markers_added", 0)
            }
    ...
    def run_orchestration(self, diagnostic_report_path: Optional[Path] = None) -> Dict:
        ...
        successful_fixes = sum(
            1 for r in self.results["fixes_applied"].values() 
