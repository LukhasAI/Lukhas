...
@click.option('--type', '-t', type=click.Choice(['decision', 'insight', 'pattern', 'question', 'learning']), default='insight')
...
@click.option('--template', type=click.Choice(['architecture', 'refactoring', 'feature', 'bugfix']), default='architecture')
@click.pass_context
...
def export(ctx, output, days):
    ...
    # Get entries
    start_date = datetime.now() - timedelta(days=days)
    entries = journal.search(date_range=(start_date, datetime.now()))
    
    # Set output path
    if not output:
        output = f"journal_export_{datetime.now().strftime('%Y%m%d')}.md"
...
@click.option('--type', '-t', type=click.Choice(['decision', 'insight', 'pattern', 'question', 'learning', 'auto']), default='auto')
@click.option('--parse-dates', '-d', is_flag=True, help='Try to parse dates from content')
@click.option('--split-by', '-s', type=click.Choice(['heading', 'paragraph', 'date', 'none']), default='heading', help='How to split content into entries')
@click.pass_context
        ...
        for entry_data in entries_data:
            try:
                ...
                if 'timestamp' in entry_data:
                    ...
                    timestamp = datetime.fromisoformat(entry_data['date'])
        ...
        for entry_data in entries:
            try:
                ...
                entry = journal.add_entry(
                    ...
                    tags=tags,
                    metadata={'imported_from': path.name, 'original_title': entry_data.get('title')}
                )
                imported_count += 1
...
def _extract_tags_from_content(content: str) -> list:
    ...
    tech_terms = ['api', 'database', 'ui', 'performance', 'security', 'testing', 
                  ...
                  'lukhas', 'memory_fold', 'quantum', 'consciousness']
...
def weekly(ctx):
    ...
    print(f"  Entries this week: {len(ctx.obj['journal'].search(date_range=(datetime.now() - timedelta(days=7), datetime.now())))}")
    print(f"  Current streak: {stats['streak']} days")
    ...
    if any(p.impact == "negative" for p in patterns):
        print(f"  1. Address negative patterns identified")
    print(f"  2. Continue momentum on current features")
    print(f"  3. Schedule code review sessions")
    print(f"  4. Document recent decisions")
...
def vision(ctx):
    ...
    print(f"  • Continue building on recent wins")
    print(f"  • Document architectural decisions")
    print(f"  • Keep the revolutionary spirit alive!")
...
@cli.command()
@click.option('--type', '-t', type=click.Choice(['code_review', 'bug_analysis', 'feature_design', 'refactoring']), required=True)
@click.pass_context
def prompt(ctx, type):
