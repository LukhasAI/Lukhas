...
class LearningAssistant:
        ...
        for concept, instances in self.knowledge_base.get("concepts", {}).items():
            if any(term in concept.lower() for term in key_terms):
                results["concepts"].append({
                    "concept": concept,
                    "instances": instances[:3]  # Top 3 instances
                })
        
        # Search solutions
        for sol_id, solution in self.knowledge_base.get("solutions", {}).items():
            if any(term in solution["solution"].lower() for term in key_terms):
                results["solutions"].append(solution)
        
        # Search learnings
        for learn_id, learning in self.knowledge_base.get("learnings", {}).items():
    ...
    def _generate_exploratory_answer(self, question: str) -> str:
        ...
        answer = f"I don't have specific information about this in your journal yet, but let's explore it together.\n\n"
    ...
    def _find_related_concepts(self, question: str) -> List[str]:
        ...
        for concept, definition in self.knowledge_base.get("lukhas_concepts", {}).items():
    ...
    def _generate_daily_tasks(self, goals: List[str], current_skills: Dict[str, int], day: int) -> List[str]:
        ...
        if skill_level < 3:
            ...
            tasks.extend([
                ...
                f"Document solution in journal"
            ])
...
Remember: Being stuck is temporary. You've solved {len(self.knowledge_base.get('solutions', {}))} problems before!"""
    ...
    def _mentor_for_failures(self, situation: str) -> str:
        ...
        return f"""Failures are powerful teachers. Let's extract the learning here.
...
Remember: Every expert has a collection of instructive failures. You're building yours!"""
    ...
    def _mentor_for_decisions(self, situation: str) -> str:
        ...
        response = f"""Let's approach this decision systematically.
...
Keep that curiosity alive - it's your superpower!"""
    ...
    def _mentor_general(self, situation: str) -> str:
        """General mentorship"""
        return f"""I'm here to support your journey. Let's explore this together.
