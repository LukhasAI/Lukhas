...
class CodePatternAnalyzer:
    ...
    def extract_fstring_patterns(self, content: str) -> list[dict]:
        ...
        for match in matches:
            ...
            pattern = {
                "has_format_spec": ":" in fstring_content
                and any(c in fstring_content.split(":")[-1] for c in "fFeEgGdiouxXbcrsa"),
    ...
    def get_recent_commits(self, days: int = 30) -> list[dict]:
        ...
        try:
            ...
            result = subprocess.run(
                ["git", "log", "--since", since_date, "--pretty=format:%H|%s|%an|%ad|%d", "--date=iso", "--name-only"],
                capture_output=True,
                text=True,
                cwd=ROOT,
            )
        ...
        for file_path in files_to_scan:
            ...
            if risk_score > 0.7:
                ...
                high_risk_files.append(
                    {"file": str(file_path), "risk_score": risk_score, "predictions": risk_analysis["predictions"]}
                )
            elif risk_score > 0.4:
                risk_level = "medium"
            else:
