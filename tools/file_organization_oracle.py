...
class LUKHASFileOrganizationOracle:
    ...
    ORGANIZATION_PATTERNS = {
        'docs/': {
            ...
            'exclude_patterns': ['README.md', 'MODULE_INDEX.md'],  # Keep main README and index in root
            'subdirs': {
        ...
        'tools/scripts/': {
            ...
            'exclude_patterns': ['setup.sh', 'install.sh']  # Keep setup scripts in root
        },
        'config/': {
            'patterns': ['*.yaml', '*.yml', '*.json', '*.toml', '*.ini', '*.cfg'],
            'exclude_patterns': ['package.json', 'pyproject.toml', 'setup.cfg', '*config*.yaml']
        },
        'data/reports/': {
    ...
    def _divine_file_destination(self, file_path: Path) -> Tuple[Optional[str], str]:
        ...
        for destination, config in self.ORGANIZATION_PATTERNS.items():
            ...
            if any(self._matches_pattern(filename, pattern.upper()) 
                ...
                if 'subdirs' in config:
                    for subdir, subpatterns in config['subdirs'].items():
                        if any(pattern in filename for pattern in subpatterns):
                            return destination + subdir, f"Content analysis: {subpatterns}"
    ...
    def _is_documentation_file(self, file_path: Path) -> bool:
        ...
        doc_indicators = [
            ...
            'GUIDE', 'TUTORIAL', 'DOCS', 'MANUAL', 'HOWTO'
        ]
    ...
    def _is_configuration_file(self, file_path: Path) -> bool:
        ...
        return (any(indicator in file_path.name.upper() for indicator in config_indicators) or
                file_path.suffix.lower() in ['.yaml', '.yml', '.json', '.toml', '.ini', '.cfg'])
