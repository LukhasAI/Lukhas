"""
LUKHAS PWM Protected API Template
=================================
...
async def quantum_endpoint(
    request: BaseRequest,
    user: AuthContext = Depends(require_t4_or_above)
) -> BaseResponse:
    """
...
async def admin_endpoint(
    request: BaseRequest,
    user: AuthContext = Depends(require_t5)
) -> BaseResponse:
    """
...
async def custom_tier_endpoint(user: AuthContext = Depends(get_current_user)):
    ...
    return {
        "message": "Custom tier protection applied",
        "user_tier": user.tier
    }
