...
class ModuleUsageAnalyzer:
    ...
    def find_all_python_files(self):
        ...
        exclude_dirs = {
            ...
            'egg-info', '.eggs', 'node_modules', '.mypy_cache',
            '.backups', '.venv_test', 'site-packages', '.github',
            'reports', 'data', 'logs', 'tmp', 'temp', 'archive',
            'deprecated', 'old', 'backup', 'test_results'
        }
    ...
    def resolve_import_to_file(self, import_name: str) -> List[str]:
        ...
        for prefix in ['lukhas_pwm', 'lukhas', 'core', 'bridge', 'governance']:
    ...
    def find_orphaned_files(self):
        ...
        for py_file in self.all_python_files:
            ...
            if (py_file.startswith('tests/') or 
                py_file.startswith('tools/') or
                py_file.startswith('scripts/') or
                py_file.endswith('setup.py') or
                py_file.endswith('conftest.py') or
                '__pycache__' in py_file):
                continue
        ...
        self.orphaned_files = {
            f for f in self.orphaned_files 
            if not any(fp in f for fp in false_positives)
        }
    ...
    def analyze_module_directories(self) -> Dict[str, Dict]:
        ...
        modules = [
            'core', 'bridge', 'governance', 'consciousness', 'memory',
            'orchestration', 'identity', 'emotion', 'quantum', 'bio',
            'symbolic', 'universal_language', 'vivox', 'qim', 'lukhas',
            'api', 'serve', 'agents', 'adapters', 'architectures'
        ]
        ...
        for module in modules:
            ...
            module_files = [
                ...
                if f.startswith(f"{module}/")
            ]
            
            imported_files = [
                f for f in module_files 
                if f in self.imported_modules
            ]
            
            orphaned = [
                f for f in module_files 
                if f in self.orphaned_files
            ]
...
def main():
    ...
    for module, stats in sorted(report['module_statistics'].items()):
        if stats['total_files'] > 0:
            print(f"{module:<20} {stats['total_files']:<8} {stats['imported_files']:<8} "
