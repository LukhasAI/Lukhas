...
class ActiveImportAnalyzer:
    ...
    ACTIVE_MODULES = {
        'api', 'architectures', 'bio', 'bridge', 'consciousness', 'core',
        'creativity', 'deployments', 'docs', 'emotion', 'ethics', 'examples',
        'governance', 'identity', 'infra', 'integration', 'memory', 'meta',
        'orchestration', 'quantum', 'reasoning', 'recovery', 'security',
        'tests', 'tools', 'trace', 'unified', 'vivox'
    }
    ...
    def _is_stdlib_module(self, module_name: str) -> bool:
        ...
        stdlib_modules = {
            ...
            'pathlib', 'asyncio', 'logging', 'unittest', 'abc', 're',
            'hashlib', 'pickle', 'tempfile', 'shutil', 'subprocess',
            'io', 'threading', 'queue', 'copy', 'warnings', 'inspect',
            'contextlib', 'dataclasses', 'secrets', 'uuid', 'base64',
            'urllib', 'http', 'email', 'mimetypes', 'socketserver'
        }
    ...
    def _is_installed_package(self, module_name: str) -> bool:
        ...
        known_packages = {
            ...
            'pytest', 'black', 'flake8', 'mypy', 'aiofiles', 'aiohttp',
            'requests', 'matplotlib', 'seaborn', 'plotly', 'streamlit',
            'openai', 'transformers', 'langchain', 'pinecone', 'redis'
        }
...
def generate_fixes(report: Dict) -> List[str]:
    ...
    if report['common_patterns']['external_deps']:
        fixes.append("\n# Install missing external dependencies:")
        unique_deps = set(m.split('.')[0] for m in report['common_patterns']['external_deps'])
...
def main():
    ...
    import json
    report_path = PROJECT_ROOT / 'docs' / 'reports' / 'analysis' / 'PWM_ACTIVE_IMPORT_ANALYSIS_REPORT.json'
