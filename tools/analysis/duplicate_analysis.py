...
class DuplicateAnalyzer:
    ...
    def _check_common_patterns(self, node: ast.FunctionDef, file_path: str) -> None:
        ...
        for n in ast.walk(node):
            if isinstance(n, ast.Assign):
                if any(isinstance(target, ast.Name) and target.id == 'logger' for target in n.targets):
    ...
    def _identify_consolidation_opportunities(self) -> List[Dict]:
        ...
        if len(self.common_patterns['logger_init']) > 5:
            opportunities.append({
                ...
                'affected_files': len(set(f.split('::')[0] for f in self.common_patterns['logger_init'])),
                'priority': 'high'
            })
        ...
        if len(self.common_patterns['config_loading']) > 3:
            opportunities.append({
                ...
                'affected_files': len(set(f.split('::')[0] for f in self.common_patterns['config_loading'])),
                'priority': 'high'
            })
            
        # 3. Guardian client initialization
        guardian_imports = [f for f in self.duplicate_imports.get('governance', [])]
    ...
    def analyze_directory(self, directory: Path) -> None:
        ...
        exclude_dirs = {'.git', '__pycache__', '.pytest_cache', 'venv', '.venv', 'archive', '.pwm_cleanup_archive'}
...
def main():
    ...
    modules = ['api', 'consciousness', 'core', 'governance', 'memory', 
               'orchestration', 'reasoning', 'identity', 'quantum', 'bio',
               'creativity', 'emotion', 'bridge', 'security', 'vivox']
    ...
    if duplicates['duplicate_functions']:
        ...
        for dup in duplicates['duplicate_functions'][:5]:
            ...
            for file, name in dup['locations'][:3]:
                print(f"      - {file}")
    ...
    report_path = PROJECT_ROOT / 'docs' / 'reports' / 'analysis' / 'PWM_DUPLICATE_ANALYSIS_REPORT.json'
