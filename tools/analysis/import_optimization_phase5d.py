    ...
    for i in range(0, len(file_list), batch_size):
        ...
        for file_path in batch_files:
            ...
            if fixed:
                ...
                print(f"      ⚪ No safe moves identified")
        
        total_fixes += batch_fixes
        
        if batch_fixes > 0:
            print(f"   🔍 Running world tests for batch validation...")
            if run_world_tests():
                print(f"   ✅ World tests PASSED - batch validated")
            else:
                print(f"   ❌ World tests FAILED - stopping optimization")
                break
        
        print()
    
    print(f"🎯 Phase 5D Conservative Results:")
    print(f"   📁 Files processed: {files_processed}")
    print(f"   🔧 Safe import moves: {total_fixes}")
    print(f"   🧠 Module loading efficiency: IMPROVED")
