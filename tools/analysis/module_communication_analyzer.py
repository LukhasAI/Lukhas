...
class ModuleCommunicationAnalyzer:
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.ImportFrom):
                        if node.module:
                            ...
                            if imported_module in ['core', 'consciousness', 'memory', 'orchestration', 'governance']:
                                self.direct_imports[module_name].add(imported_module)
                                self.communication_graph.add_edge(module_name, imported_module, 
                                                                 method='import', weight=1)
    ...
    def _analyze_communication_bottlenecks(self):
        ...
        if self.communication_graph.nodes():
            try:
                ...
                bottlenecks = [node for node, degree in self.in_degree.items() 
    ...
    def _get_module_name(self, file_path: Path) -> str:
        ...
        if parts[0] in ['core', 'consciousness', 'memory', 'orchestration', 'governance', 'api', 'vivox']:
            return parts[0]
        return 'other'
    ...
    def _summarize_glyphs(self) -> Dict[str, Any]:
        ...
        total_glyph_usage = sum(
            sum(g['count'] for g in glyphs) 
            for glyphs in self.glyph_usage.values()
        )
    ...
    def _summarize_events(self) -> Dict[str, Any]:
        ...
        total_event_usage = sum(
            sum(e['count'] for e in events) 
            for events in self.event_patterns.values()
        )
    ...
    def _identify_inefficiencies(self) -> List[Dict[str, str]]:
        ...
        for module in set(list(self.direct_imports.keys()) + 
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        return {
            'total_modules_analyzed': len(set(list(self.direct_imports.keys()) + 
                                           list(self.glyph_usage.keys()) + 
                                           list(self.event_patterns.keys()))),
    ...
    def _create_communication_graph(self):
        ...
        try:
            ...
            nx.draw_networkx_nodes(self.communication_graph, pos, node_size=node_sizes, 
                                 node_color='lightblue', alpha=0.7)
            
            # Draw edges
            nx.draw_networkx_edges(self.communication_graph, pos, alpha=0.5, 
