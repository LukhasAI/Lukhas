...
class T4MasterFixer:
    ...
    def load_policy(self) -> dict:
        ...
        try:
            ...
            return {
                'scope': {
                    'allow_patterns': ['**/*.py'],
                    'deny_patterns': ['tests/**', 'docs/**', '**/__pycache__/**']
                },
                'rules': {'auto_fix': []},
    ...
    def create_backup(self):
        ...
        try:
            subprocess.run(['git', 'checkout', '-b', self.backup_branch], 
    ...
    def count_errors(self) -> Dict[str, int]:
        ...
        try:
            ...
            result = subprocess.run(
                f'.venv/bin/ruff check branding/ candidate/ tools/ products/ matriz/ next_gen/ lukhas/ --select=E999 --statistics'.split(),
            ...
            counts['unterminated_strings'] = result.stderr.count('unterminated string') + \
                                            result.stderr.count('missing closing quote')
            
            # Count undefined names
            result = subprocess.run(
                f'.venv/bin/ruff check branding/ candidate/ tools/ products/ matriz/ next_gen/ lukhas/ --select=F821 --statistics'.split(),
                capture_output=True, text=True
            )
    ...
    def fix_missing_imports(self):
        ...
        result = subprocess.run(
            ['.venv/bin/ruff', 'check', '.', '--select=F821', '--output-format=json'],
            capture_output=True, text=True
        )
    ...
    def fix_unterminated_strings(self):
        ...
        result = subprocess.run(
            ['.venv/bin/ruff', 'check', '.', '--select=E999', '--output-format=json'],
            capture_output=True, text=True
        )
        ...
        if result.stdout:
            ...
            try:
                ...
                for issue in issues:
                    if 'unterminated string' in issue.get('message', '') or \
                       'missing closing quote' in issue.get('message', ''):
    ...
    def rollback(self):
        ...
        try:
            subprocess.run(['git', 'checkout', self.backup_branch.replace('-backup-', '-main-')],
                         check=True, capture_output=True)
    ...
    def generate_report(self):
        ...
        # Success evaluation
        syntax_reduction = (self.stats['errors_before'] - final_counts['syntax_errors']) / max(self.stats['errors_before'], 1) * 100
        undefined_reduction = (self.stats.get('undefined_before', 0) - final_counts['undefined_names']) / max(self.stats.get('undefined_before', 1), 1) * 100
        ...
        print("\n🛡️ T4 Compliance:")
        print(f"  ✅ Policy loaded from .t4autofix.toml")
        print(f"  ✅ Protected interfaces preserved")
        print(f"  ✅ Consciousness symbols maintained")
...
def main():
    ...
    parser.add_argument(
        '--production',
        action='store_true',
        help='Run in production mode (applies fixes). Default is dry-run.'
    )
    parser.add_argument(
        '--verbose',
        action='store_true',
