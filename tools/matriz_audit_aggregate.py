...
def merge_module_data(
    inventory: Dict[str, Any], validation: Dict[str, Any], imports: Dict[str, Any], lanes: Dict[str, Any]
) -> List[Dict[str, Any]]:
    ...
    for module_data in inventory_modules:
        ...
        merged_module = {
            ...
            # Import issues
            "import_issues": {"count": len(module_import_issues), "issues": module_import_issues},
            # Lane assessment
            "lane": {
                ...
                if i == 0:  # First path becomes the target
                    if path != target_path:
                        suggestion["operations"].append(
                            {"command": f"git mv {path} {target_path}", "type": "primary_move"}
                        )
                else:  # Subsequent paths get merged
                    suggestion["operations"].append(
                        {"command": f"git mv {path}/* {target_path}/", "type": "merge_move"}
                    )
...
def compute_summary_stats(
    modules: List[Dict[str, Any]], validation: Dict[str, Any], imports: Dict[str, Any], lanes: Dict[str, Any]
) -> Dict[str, Any]:
...
def main():
    ...
    if args.verbose:
        ...
        print(
            f"  With contracts: {summary_stats['contracts']['with_contracts']} ({summary_stats['contracts']['percentage_with_contracts']}%)"
        )
