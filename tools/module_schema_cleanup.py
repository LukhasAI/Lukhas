...
class ModuleSchemaCleanup:
    ...
    def cleanup_all_schemas(self) -> None:
        ...
        print(f"🧹 Cleaning up module schemas...")
        print(f"Found {len(self.all_modules)} valid modules")
        
        schema_files = [f for f in self.modules_dir.glob("*.yaml") 
                       if f.name != "module_schema_template.yaml"]
    ...
    def _is_valid_external_import(self, import_name: str) -> bool:
        ...
        valid_externals = [
            'asyncio', 'dataclasses', 'typing', 'pathlib', 'json', 'yaml',
            'requests', 'aiohttp', 'fastapi', 'pydantic', 'sqlalchemy',
            'redis', 'lmdb', 'numpy', 'pandas', 'torch', 'transformers'
        ]
