...
class ComprehensiveOrphanAnalyzer:
    def __init__(self, root_path: str):
        ...
        self.excluded_dirs = {
            ...
            '.github', '.vscode', '.idea', '.DS_Store',
            'coverage', '.coverage', 'wheels', '.wheelhouse',
            'pip-wheel-metadata', '.pip', 'conda-meta',
            'pkgs', 'envs', 'Library', 'Scripts', 'DLLs',
            'migrations', '__pycache__', 'staticfiles',
            '.ipynb_checkpoints', '.pytest_cache', '.tox',
            '.nox', '.hypothesis', '.benchmarks',
            'test_results', 'reports', 'docs/_build',
            'target', 'out', 'output', 'logs', 'tmp', 'temp',
            'cache', '.cache', 'data', 'backups', '.backups'
        }
    ...
    def is_library_import(self, import_name: str) -> bool:
        ...
        stdlib = {
            ...
            'urllib', 'http', 'email', 'csv', 'sqlite3', 'hashlib',
            'logging', 'argparse', 'subprocess', 'threading', 'asyncio',
            'unittest', 'doctest', 'pdb', 'pickle', 'copy', 'io',
            'time', 'calendar', 'decimal', 'fractions', 'statistics',
            'abc', 'dataclasses', 'enum', 'types', 'warnings', 'weakref',
            'contextlib', 'inspect', 'traceback', 'gc', 'atexit',
        ...
        third_party = {
            ...
            'tensorflow', 'torch', 'keras', 'django', 'flask',
            'fastapi', 'requests', 'urllib3', 'beautifulsoup4',
            'selenium', 'pytest', 'setuptools', 'pip', 'wheel',
            'sqlalchemy', 'pymongo', 'redis', 'celery', 'pydantic',
            'uvicorn', 'gunicorn', 'aiohttp', 'httpx', 'openai',
            'anthropic', 'langchain', 'transformers', 'pillow',
            'opencv', 'nltk', 'spacy', 'streamlit', 'gradio'
        }
    ...
    def resolve_import_to_file(self, import_name: str) -> List[str]:
        ...
        for prefix in ['lukhas_pwm', 'lukhas']:
    ...
    def generate_report(self) -> Dict:
        ...
        most_imported = sorted(
            [(f, c) for f, c in import_counts.items()],
            key=lambda x: x[1],
            reverse=True
        )[:20]
...
def print_report(report: Dict):
    ...
    for directory, stats in sorted_dirs[:20]:
        if stats['total'] > 0:
            print(f"{directory:<30} {stats['total']:<10} {stats['used']:<10} "
