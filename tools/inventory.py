...
def accepted_illegal_imports(files):
    ...
    for f in files:
        ...
        for i, line in enumerate(lines, 1):
            if pat.search(line): bad.append({"file": str(f.relative_to(ROOT)), "line": i, "text": line.strip()})
    return bad
report = {}
for lane, path in LANES.items():
    files = list_py(path)
    report[lane] = {"exists": path.exists(), "is_package": has_init(path) if path.exists() else False,
                    "files": len(files), "loc": count_lines(files),
...
out = {"root": str(ROOT), "lanes": report, "accepted_illegal_imports": illegal,
