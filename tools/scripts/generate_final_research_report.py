...
def generate_comprehensive_report():
    ...
    if 'system_status' in component_reports:
        ...
        comprehensive_report['executive_summary']['overall_system_health'] = f"{exec_summary.get('overall_health_score', 0):.1f}/100"
        comprehensive_report['executive_summary']['core_modules_functional'] = f"{len([m for m in system_data.get('core_modules', {}).values() if m.get('status') == 'working'])}/7"
        comprehensive_report['executive_summary']['api_systems_online'] = exec_summary.get('api_status', 'unknown')
        comprehensive_report['executive_summary']['test_success_rate'] = f"{exec_summary.get('test_success_rate', 0):.1f}%"
        comprehensive_report['executive_summary']['vivox_components_operational'] = f"{exec_summary.get('vivox_components_working', 0)}/5"
        ...
        comprehensive_report['executive_summary']['key_findings'].extend([
            ...
            f"VIVOX consciousness system: All 5 components working",
        ...
        comprehensive_report['executive_summary']['critical_issues'].extend(system_data.get('issues_detected', []))
    ...
    if 'drift_test' in component_reports:
        ...
        comprehensive_report['executive_summary']['multi_model_comparison'] = f"{len(drift_data.get('summary', {}))} providers tested"
            ...
            for provider, stats in drift_data['summary'].items():
                if stats.get('status') != 'all_failed':
                    comprehensive_report['executive_summary']['key_findings'].append(
                        f"{provider}: Avg drift {stats['average_drift']:.3f}, Trinity coherence {stats['average_trinity']:.3f}"
                    )
