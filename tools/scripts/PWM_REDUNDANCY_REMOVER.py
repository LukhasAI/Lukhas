...
class RedundancyRemover:
    ...
    def __init__(self, dry_run: bool = True):
        ...
        self.backup_dir = self.root_path / ".redundancy_backup" / datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    def remove_duplicate_logger_setup(self):
        ...
        for py_file in files_to_update[:50]:  # Limit for safety
            try:
                ...
                if needs_import and 'from lukhas.common.utils import get_logger' not in content:
                    ...
                    for i, line in enumerate(lines):
                        if (line.startswith('import ') or line.startswith('from ')) and not import_added:
                            ...
                            j = i
                            while j < len(lines) and (lines[j].startswith(('import ', 'from ')) or lines[j].strip() == ''):
                                j += 1
                            lines.insert(j, 'from lukhas.common.utils import get_logger')
                            import_added = True
                            break
                    
                    if not import_added and len(lines) > 3:
                        # Add after docstring
                        lines.insert(3, 'from lukhas.common.utils import get_logger\n')
                ...
                if content != original_content:
                    ...
                    modified += 1
                    logger.info(f"   {'Would modify' if self.dry_run else 'Modified'}: {py_file.relative_to(self.root_path)}")
    ...
    def run(self):
        ...
        logger.info(f"Report saved: docs/reports/redundancy_removal_report.json")
