...
def complete_cleanup():
    ...
    for py_file in workspace.rglob("PWM_*.py"):
        ...
        if not new_path.exists():
            ...
            changes.append((str(py_file.relative_to(workspace)), str(new_path.relative_to(workspace))))
    ...
    for py_file in workspace.rglob("lukhas_*.py"):
        new_name = py_file.name.replace("lukhas_", "").replace("LUKHAS_", "").replace("Lukhas_", "")
        new_path = py_file.parent / new_name
    ...
    lukhas_dirs = [d for d in workspace.iterdir() if d.is_dir() and d.name.startswith("lukhas")]
    ...
    for old_dir in lukhas_dirs:
        ...
        if not new_path.exists():
            ...
            changes.append((str(old_dir.relative_to(workspace)), str(new_path.relative_to(workspace))))
    ...
    for py_file in workspace.rglob("*.py"):
        ...
        try:
            ...
            content = re.sub(r'from tools\.analysis\.PWM_(\w+)', r'from tools.analysis.\1', content)
            content = re.sub(r'import tools\.analysis\.PWM_(\w+)', r'import tools.analysis.\1', content)
