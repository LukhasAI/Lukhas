...
class UserIDInjector:
    ...
    def inject_user_tracking(self, target_modules: List[str] = None, dry_run: bool = False):
        ...
        print("🔗 LUKHAS PWM User ID Injector")
        print("=" * 40)
        ...
        target_modules = target_modules or ["consciousness", "quantum", "dream", "emotion", "memory"]
    ...
    def _inject_module_user_tracking(self, module_name: str, dry_run: bool):
        ...
        priority_files = [f for f in py_files if any(key in f.name for key in 
                         ["core", "main", "service", "manager", "processor", "engine"])]
        ...
        for line in lines:
            ...
            for pattern in self.storage_patterns:
                if re.search(pattern, line) and 'user_id' not in line:
                    ...
                    if '.save(' in line:
                        modified_line = line.replace('.save()', '.save(user_id=getattr(user, "user_id", "system"))')
    ...
    def _generate_report(self):
        ...
        print(f"\n🔗 Injections Applied:")
        print(f"  • Data dictionary user_id fields")
        print(f"  • Log message user context")
        print(f"  • Storage operation user tracking") 
        print(f"  • Function parameter user context")
        
        if self.backup_dir.exists():
            print(f"\n💾 Backups: {self.backup_dir}")
            
        print(f"\n📋 Manual Steps Required:")
        print(f"  1. Import AuthContext in modified files")
        print(f"  2. Pass user context through function calls")
        print(f"  3. Update API endpoints to provide user context")
        print(f"  4. Test user tracking functionality")
