{
    ...
    "type": "object",
    "additionalProperties": false,  # noqa: F821  # TODO: false
    ...
    "$defs": {
        ...
        "uri": {"type": "string", "format": "uri", "description": "Absolute URI"},
        "relpath": {
            "type": "string",
            "pattern": "^(?!/)(?![A-Za-z]:\\\\).+",
        ...
        "lane": {"type": "string", "enum": ["L0", "L1", "L2", "L3", "L4", "L5"]},
        "otelSemconv": {"type": "string", "pattern": "^[0-9]+\\.[0-9]+\\.[0-9]+$"},
    ...
    "properties": {
        ...
        "identity": {
            ...
            "additionalProperties": false,  # noqa: F821  # TODO: false
            "properties": {
                "requires_auth": {"type": "boolean", "default": false},  # noqa: F821  # TODO: false
        ...
        "tokenization": {
            ...
            "additionalProperties": false,  # noqa: F821  # TODO: false
            "properties": {
                "enabled": {"type": "boolean", "default": false},  # noqa: F821  # TODO: false
    ...
    "allOf": [
        {
            "if": {
                "properties": {"runtime": {"properties": {"language": {"const": "python"}}, "required": ["language"]}}
            },
            "then": {
                "properties": {"runtime": {"required": ["entrypoints"], "properties": {"entrypoints": {"minItems": 1}}}}
            },
        },
        {
            "if": {
                "properties": {
                    "identity": {"properties": {"requires_auth": {"const": true}}, "required": ["requires_auth"]}
                }
            },  # noqa: F821  # TODO: true
        ...
        {
            "if": {
                "properties": {"tokenization": {"properties": {"enabled": {"const": true}}, "required": ["enabled"]}}
            },  # noqa: F821  # TODO: true
            "then": {
                "properties": {
                    "tokenization": {
                        "required": ["chain", "asset_id", "proof_uri"],
                        "properties": {"chain": {"enum": ["solana", "evm"]}},
                    }
                }
            },
        },
        {
            "if": {
                "properties": {
                    "performance": {"properties": {"sla": {"required": ["latency_p95_ms", "latency_p99_ms"]}}}
                }
            },
            "then": {
                "properties": {
                        ...
                        "properties": {
                            "sla": {
                                "allOf": [
                                    ...
                                    {
                                        "properties": {
                                            "latency_p99_ms": {"minimum": {"$data": "1/performance/sla/latency_p95_ms"}}
                                        }
                                    },
                                ]
                            }
