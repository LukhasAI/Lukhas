...
def check_constraints_enforcement() -> Dict[str, Any]:
    ...
    results = {"constraints_exists": constraints_file.exists(), "ci_enforces": False, "constraints_count": 0}
...
def check_dependency_vulnerabilities() -> Dict[str, Any]:
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-m", "safety", "check", "--json"], capture_output=True, text=True, timeout=30
        )
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            sbom = {
                ...
                "specVersion": "1.4",
                "version": 1,
                "metadata": {"timestamp": "2025-09-22T01:35:00Z", "tools": ["pip"]},
                "components": [],
            }
        ...
        return {"status": "failed", "error": str(e), "compliant": False}
