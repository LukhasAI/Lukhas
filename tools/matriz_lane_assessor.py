...
class LaneAssessor:
    ...
    def _assess_test_coverage(self, paths: List[str]) -> Dict[str, Any]:
        ...
        return {"has_tests": has_tests, "test_file_count": test_file_count}
    ...
    def compute_provisional_lane(self, characteristics: Dict[str, Any]) -> str:
        ...
        # Archive lane (L0)
        if name_indicators["has_deprecated"] or path_indicators["in_archive"] or characteristics["file_count"] == 0:
            return "L0"

        # Core lane (L5)
        if (
            path_indicators["in_core"]
            and name_indicators["has_core"]
            and contract_indicators["maturity_score"] >= 5
            and characteristics["dependency_score"] >= 3
        ):
            return "L5"

        # Accepted lane (L4)
        if path_indicators["in_accepted"] or (
            contract_indicators["maturity_score"] >= 4
            and characteristics["test_coverage"]["has_tests"]
            and characteristics["has_contract"]
        ):
            return "L4"

        # Candidate lane (L3)
        if path_indicators["in_candidate"] or (
            contract_indicators["maturity_score"] >= 2
            and characteristics["has_contract"]
            and characteristics["file_count"] >= 5
        ):
            return "L3"

        # Experimental lane (L1)
        if (
            path_indicators["in_experimental"]
            or name_indicators["has_experimental"]
            or not characteristics["has_contract"]
        ):
            return "L1"
