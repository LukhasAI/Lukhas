...
- **No network**: mock LLM/vector store/storage calls via `pytest` fixtures.
...
## Special tasks (Claude)

- **Fix collection errors**: replace modern typing syntax for Python 3.9 compatibility where safe (str | None → Optional[str]) but prefer adding `python_requires` or upgrade plan.
- **Add conftest.py hooks**: ensure tests use drop-in `tests/conftest.py` that writes events.ndjson.
- **Test-gap PRs**: when a failing signature points to uncovered code, open an "add tests" PR instead of a code fix.
- **Spec drift detector tests**: create tests that pin OpenAPI shapes by sampling endpoints and comparing to saved `openapi.json`.
