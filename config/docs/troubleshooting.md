...
2. Validate configuration: `python -c "import config; print('OK')"`
3. Review logs in `logs/config.log`
...
```bash
# Check module health
python -m config --health-check

# Validate configuration
python -m config --validate-config

# Test connections
python -m config --test-connections

# Generate diagnostic report
python -m config --diagnostic-report
```
