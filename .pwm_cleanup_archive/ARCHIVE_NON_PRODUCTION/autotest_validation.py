...
)  # Assuming this script is in core/
...
try:
    from core.automatic_testing_system import (  # Assuming it's in core.automatic_testing_system
        ...
        capture,
        report,
        run,  # These are the one-line API functions
        watch,
    )
    ...
    logger.critical(
        f"ΛTRACE: Import failed for automatic_testing_system components: {e}",
        exc_info=True,
    )
    logger.error(
        f"❌ Import failed: {e}. Ensure 'automatic_testing_system.py' is in the 'core' directory or PYTHONPATH is set correctly."
    )
    sys.exit(1)
except Exception as e_gen:  # Catch any other exception during import phase
    logger.critical(
        f"ΛTRACE: General exception during import phase: {e_gen}", exc_info=True
    )
...
async def test_basic_functionality() -> bool:
    ...
    try:
        ...
        logger.info(
            f"ΛTRACE: AutomaticTestingSystem initialized successfully. Workspace: {workspace_for_test}"
        )
        logger.info("   ✅ System initialized successfully.")
    except Exception as e:
        logger.error(
            f"ΛTRACE: Initialization of AutomaticTestingSystem failed: {e}",
            exc_info=True,
        )
    ...
    try:
        ...
        logger.info(
            f"ΛTRACE: Terminal operation capture completed. Status: {captured_op_for_ai.status}, Duration: {captured_op_for_ai.duration_ms:.2f}ms."
        )
        ...
        if captured_op_for_ai.duration_ms < 100:  # Performance check
            ...
            logger.warning(
                f"ΛTRACE: Performance target for echo test missed ({captured_op_for_ai.duration_ms:.2f}ms)."
            )
            logger.warning(
                f"   ⚠️  Performance target missed ({captured_op_for_ai.duration_ms:.2f}ms)."
            )
    ...
    try:
        if autotest_instance and autotest_instance.performance_monitor:
            ...
            logger.info(
                f"ΛTRACE: Performance metrics captured: CPU System {metrics.get('cpu_percent_system', 'N/A'):.1f}%, Memory System {metrics.get('memory_percent_system', 'N/A'):.1f}%"
            )
            logger.info(
                f"   ✅ Metrics captured: CPU System {metrics.get('cpu_percent_system', 'N/A'):.1f}%"
            )
            logger.info(
                f"   💾 Memory System: {metrics.get('memory_percent_system', 'N/A'):.1f}%"
            )
        else:
            logger.warning(
                "ΛTRACE: Performance monitoring disabled or autotest_instance not available."
            )
            logger.warning(
                "   ⚠️  Performance monitoring disabled or system not initialized."
            )
    except Exception as e:
    ...
    try:
        if autotest_instance and autotest_instance.ai_analyzer and captured_op_for_ai:
            analysis_results = autotest_instance.ai_analyzer.analyze_operation(
                captured_op_for_ai
            )
            logger.info(
                f"ΛTRACE: AI analysis completed. Category: {analysis_results.get('performance_category', 'unknown')}, Success Prob: {analysis_results.get('predicted_success_probability', 0):.2f}"
            )
            logger.info("   ✅ AI analysis completed.")
            logger.info(
                f"   🎯 Performance category: {analysis_results.get('performance_category', 'unknown')}"
            )
            logger.info(
                f"   📊 Predicted success probability: {analysis_results.get('predicted_success_probability', 0):.2f}"
            )
        elif not captured_op_for_ai:
            logger.warning(
                "ΛTRACE: AI analysis skipped as previous capture operation failed."
            )
            logger.warning("   ⚠️  AI analysis skipped due to previous capture failure.")
        else:
            logger.warning(
                "ΛTRACE: AI analysis disabled or autotest_instance not available."
            )
    ...
    logger.info(
        f"ΛTRACE: Test suite '{test_name}' finished. Overall result: {'PASSED' if all_steps_passed else 'FAILED'}"
    )
...
async def test_one_line_api() -> bool:
    ...
    try:
        ...
        logger.info(
            f"ΛTRACE: autotest.run('basic') completed. Status: {run_results.get('status')}, Session ID: {run_results.get('session_id')}"
        )
        logger.info(f"   ✅ Test run completed: {run_results.get('status')}")
        logger.info(f"   📋 Session ID: {run_results.get('session_id')}")
        if "summary_results" in run_results and isinstance(
            run_results["summary_results"], dict
        ):  # Check type
            logger.info(
                f"   📊 Test type from summary: {run_results['summary_results'].get('test_suite_type', 'unknown')}"
            )
    except Exception as e:
    ...
    try:
        captured_op = await capture(
            command_to_run="python --version"
        )  # Using renamed arg
        logger.info(
            f"ΛTRACE: autotest.capture() completed. Status: {captured_op.status}, Duration: {captured_op.duration_ms:.2f}ms."
        )
        ...
        if "Python" in captured_op.output:  # Basic check for output content
            ...
            logger.info("   🐍 Python version detected in output.")
        else:
            logger.warning("ΛTRACE: Python version string not found in capture output.")
            logger.warning(
                f"   ⚠️ Python version string not found in output: {captured_op.output[:100]}"
            )
    except Exception as e:
    ...
    try:
        ...
        session_id_for_report = (
            run_results.get("session_id")
            if run_results and run_results.get("status") == "completed"
            else None
        )
        logger.debug(
            f"ΛTRACE: Generating report for session ID: {session_id_for_report if session_id_for_report else 'most recent'}"
        )

        report_output = await report(
            session_id_str=session_id_for_report
        )  # Using renamed arg
        logger.info(
            f"ΛTRACE: autotest.report() completed. Status: {report_output.get('status')}"
        )
        logger.info(f"   ✅ Report generated: {report_output.get('status')}")

        if report_output.get("status") == "report_generated_successfully":
            logger.info(
                f"ΛTRACE: Report file location: {report_output.get('report_file_location')}"
            )
            logger.info(
                f"   📁 Report file: {report_output.get('report_file_location')}"
            )
            # Further checks on report_content_summary can be added here
        elif report_output.get("status") == "no_sessions_available":
            logger.warning(
                "ΛTRACE: Report generation skipped as no sessions were available (possibly prior run failed)."
            )
            logger.warning(
                "   ⚠️ No sessions available for report (prior run might have failed)."
            )
        else:
            logger.error(f"ΛTRACE: Report generation had issues: {report_output}")
            logger.warning(
                f"   ⚠️ Report generation status: {report_output.get('status')}, Error: {report_output.get('error_details', report_output.get('error'))}"
            )
            # Not failing the whole suite for a report generation issue if tests ran.

    except Exception as e:
        logger.error(
            f"ΛTRACE: autotest.report() failed with exception: {e}", exc_info=True
        )
...
async def test_performance_validation() -> bool:
    ...
    test_cmds = [
        ...
        (
            "ls -la . > /dev/null" if os.name != "nt" else "dir > NUL"
    ...
    for idx, cmd_str in enumerate(test_cmds, 1):
        ...
        try:
            op = await capture(
                command_to_run=cmd_str, timeout_duration_seconds=5
            )  # Using renamed args
            captured_ops_list.append(op)
            status_symbol = "✅" if op.status == "completed" else "❌"
            logger.info(
                f"   {status_symbol} Test {idx} ('{cmd_str[:30]}...'): {op.duration_ms:.2f}ms, Status: {op.status}"
            )
            logger.info(
                f"ΛTRACE: Perf validation command {idx} status: {op.status}, duration: {op.duration_ms:.2f}ms."
            )
        except Exception as e:
            logger.error(
                f"ΛTRACE: Perf validation command {idx} ('{cmd_str}') failed: {e}",
                exc_info=True,
            )
    ...
    if not captured_ops_list:
        logger.error(
            "ΛTRACE: No operations were successfully captured for performance validation."
        )
        return False
    ...
    op_durations = [
        op.duration_ms
        for op in captured_ops_list
        if op.status == "completed"
        and op.duration_ms is not None
        and op.duration_ms > 0
    ]
    successful_ops_count = len(
        [op for op in captured_ops_list if op.status == "completed"]
    )

    if op_durations:
        avg_duration_val = (
            np.mean(op_durations) if np else sum(op_durations) / len(op_durations)
        )
        ...
        logger.info(
            f"ΛTRACE: Performance Stats - AvgDur: {avg_duration_val:.2f}ms, MaxDur: {max_duration_val:.2f}ms, MinDur: {min_duration_val:.2f}ms, Success: {success_rate_val:.1f}%"
        )
        ...
        sub_100ms_percent = (
            (sub_100ms_ops_count / len(op_durations)) * 100 if op_durations else 0
        )
        logger.info(
            f"ΛTRACE: Sub-100ms operations: {sub_100ms_ops_count}/{len(op_durations)} ({sub_100ms_percent:.1f}%)"
        )
        logger.info(
            f"   🎯 Sub-100ms operations: {sub_100ms_ops_count}/{len(op_durations)} ({sub_100ms_percent:.1f}%)"
        )
        ...
        if sub_100ms_percent >= 75:  # Adjusted target
            ...
            logger.warning(
                "   ⚠️  NEEDS IMPROVEMENT: Consider optimization for sub-100ms operations."
            )
        return sub_100ms_percent >= 75  # Test passes if target met
    else:
        logger.warning(
            "ΛTRACE: No valid durations collected for performance statistics."
        )
...
async def test_error_handling() -> bool:
    ...
    try:
        invalid_cmd_op = await capture(
            command_to_run="this_is_not_a_real_command_xyz123abc"
        )
        if (
            invalid_cmd_op.status == "failed" and invalid_cmd_op.exit_code != 0
        ):  # More specific check
            logger.info(
                f"ΛTRACE: Invalid command handled as 'failed' with exit code {invalid_cmd_op.exit_code}. Error: {invalid_cmd_op.error[:100]}..."
            )
            logger.info("   ✅ Invalid command properly handled as failed.")
            logger.info(
                f"   📝 Error message (first 100 chars): {invalid_cmd_op.error[:100]}..."
            )
        else:
            logger.error(
                f"ΛTRACE: Invalid command not properly detected as failed. Status: {invalid_cmd_op.status}, ExitCode: {invalid_cmd_op.exit_code}"
            )
            logger.warning(
                f"   ⚠️  Invalid command not properly detected. Status: {invalid_cmd_op.status}, ExitCode: {invalid_cmd_op.exit_code}"
            )
            all_steps_passed = False
    except Exception as e:  # Should be caught by capture
        logger.error(
            f"ΛTRACE: Exception during invalid command test: {e}", exc_info=True
        )
    ...
    try:
        timeout_op = await capture(
            command_to_run=sleep_command, timeout_duration_seconds=2
        )
        if timeout_op.status == "timeout":
            logger.info(
                f"ΛTRACE: Timeout properly handled. Duration: {timeout_op.duration_ms:.0f}ms (expected ~2000ms)."
            )
            logger.info("   ✅ Timeout properly handled.")
            logger.info(
                f"   ⏰ Duration: {timeout_op.duration_ms:.0f}ms (expected ~2000ms)"
            )
        else:
            logger.error(
                f"ΛTRACE: Timeout not properly handled. Status: {timeout_op.status}, Duration: {timeout_op.duration_ms}ms."
            )
...
async def main_validation_runner():  # Renamed
    ...
    logger.info("=" * 60)
    logger.info(
        "This script will now perform a comprehensive validation of the testing system's capabilities..."
    )
    ...
    try:
        ...
        logger.critical(
            f"ΛTRACE: CRITICAL ERROR during main validation execution: {e_critical}",
            exc_info=True,
        )
        logger.critical(
    ...
    num_passed_suites = sum(
        1 for _, result_flag in validation_suite_results if result_flag
    )
    ...
    for suite_name, result_flag in validation_suite_results:
        ...
        logger.info(
            f"ΛTRACE: Validation Suite '{suite_name}' result: {'PASSED' if result_flag else 'FAILED'}"
        )
    ...
    logger.info(
        f"ΛTRACE: Final Tally - Passed: {num_passed_suites}, Total: {total_suites_run}."
    )

    if num_passed_suites == total_suites_run:
        logger.info(
            "🎉 CONGRATULATIONS! All validation suites passed. The Automatic Testing System appears robust."
        )
        logger.info("ΛTRACE: All validation suites PASSED.")
        return True
    else:
        logger.warning(
            "⚠️ ATTENTION: One or more validation suites failed. Please review the logs and output for details."
        )
...
if __name__ == "__main__":
    ...
    if sys.version_info < (
        3,
        7,
    ):  # Async/await syntax requires 3.5+, but many libraries target 3.7+
        logger.critical(
            "ΛTRACE: Python 3.7+ is required for this validation script due to async/await usage and type hinting."
        )
    ...
    logger.info(
        f"ΛTRACE: main_validation_runner finished. Overall success: {overall_success_status}."
    )
