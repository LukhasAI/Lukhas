...
class ProperConsolidator:
    ...
    def analyze_directory_structure(self) -> Dict[str, List[Path]]:
        ...
        rules = {
            ...
            "core/identity/": ["core/identity/vault/*.py"],
    ...
    def plan_moves(
        self, consolidation_targets: Dict[str, List[Path]]
    ...
    def execute_moves(self, moves: List[Tuple[Path, Path]]) -> Dict[str, int]:
        ...
        stats = {"moved": 0, "errors": 0, "directories_removed": 0}
    ...
    def cleanup_empty_directories(self) -> int:
        ...
        check_dirs = [
            ...
            "core/visuals",
            "core/identity/vault",
            "personality/emotional_system",
            "personality/narrative_engine",
            "personality/creative_core",
            "personality/brain",
            "quantum/attention",
            "quantum/economics",
        ...
        for dir_path in check_dirs:
            ...
            if path.exists() and path.is_dir():
                ...
                py_files = [
                    f for f in files if f.suffix == ".py" and f.name != "__init__.py"
                ]
                ...
                if not py_files:  # No Python files except maybe __init__.py
                    if not self.dry_run:
                        shutil.rmtree(path)
                    print(
                        f"{'Would remove' if self.dry_run else 'Removed'} empty directory: {path}"
                    )
                    removed += 1
    ...
    def run(self):
        ...
        for target_dir in consolidation_targets:
            moves_to_target = [
                (s, d) for s, d in moves if str(d).startswith(target_dir)
            ]
            if moves_to_target:
                print(f"\n{target_dir}")
        ...
        print(
            f"{'Would update' if self.dry_run else 'Updated'} imports in {updated_files} files"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Properly consolidate LUKHAS directory structure"
    )
    parser.add_argument(
        "--execute",
        action="store_true",
