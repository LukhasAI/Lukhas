...
from typing import (  # Set, Optional not directly used by this file but good for consistency
    Any,
    Dict,
    List,
    Set,
...
class TopologyManager:
    ...
    def analyze_topology(self) -> TopologyMetrics:
        ...
        self.logger.debug(
            f"ΛTRACE: Basic metrics: Nodes={node_count}, Connections={connection_count}, Density={connection_density:.4f}"
        )
        ...
        self.logger.debug(
            f"ΛTRACE: Advanced metrics: ClusteringCoeff={clustering_coeff:.4f}, AvgPathLength={avg_path_len:.4f}, NetworkEfficiency={network_eff:.4f}"
        )
    ...
    def _calculate_clustering_coefficient(self) -> float:
        ...
        for node_id_str in self.network.nodes:
            ...
            if max_possible_triangles > 0:
                ...
                self.logger.debug(
                    f"ΛTRACE: Node '{node_id_str}' has <2 possible triangles, local coeff is 0."
                )
    ...
    def _calculate_network_efficiency_from_metrics(
        ...
        self.logger.debug(
            f"ΛTRACE: Efficiency factors: Connectivity={connectivity_factor:.2f}, Performance={performance_factor:.2f}, Structure={structure_factor:.2f}, PathPenalty={path_penalty:.2f}"
        )
    ...
    def assess_network_health(self, current_metrics: TopologyMetrics) -> NetworkHealth:
        ...
        self.logger.debug(
            f"ΛTRACE: Health components: ErrorScoreComp={error_score_component:.2f}, ActivityScoreComp={activity_score_component:.2f}, EfficiencyScoreComp={efficiency_score_component:.2f}. Overall Health Score: {overall_health_score:.2f}"
        )
        ...
        if avg_network_degree > 0:  # Only if average degree is meaningful
            for node_id_str, degree in node_degrees_map.items():
                if degree > max(
                    ...
                    severity = (
                        min(degree / (avg_network_degree * overconnected_factor), 1.0)
                    ...
                    identified_bottlenecks.append(bottleneck_info)
                    self.logger.debug(
                        f"ΛTRACE: Bottleneck (Overconnected): Node '{node_id_str}', Degree: {degree} (Avg: {avg_network_degree:.2f})"
                    )

        # Sort bottlenecks by severity for prioritization
        sorted_bottlenecks = sorted(
            identified_bottlenecks, key=lambda x: x["severity_score"], reverse=True
        )
        self.logger.info(
