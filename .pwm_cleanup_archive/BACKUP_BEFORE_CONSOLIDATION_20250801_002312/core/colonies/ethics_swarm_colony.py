...
class EthicsSwarmColony(BaseColony):
    ...
    async def _generate_collapse_hash(
        ...
        combined_string = json.dumps(
            {"decision": decision_data, "context": request.context}, sort_keys=True
        )
    ...
    async def _update_swarm_memory(
        self, request: EthicalDecisionRequest, response: EthicalDecisionResponse
    ):
            ...
            try:
                ...
                await self.emit_event(
                    "ethical_drift_monitored",
                    {
                        ...
                        in [EthicalDriftLevel.CONCERNING, EthicalDriftLevel.CRITICAL],
                    },
                )
