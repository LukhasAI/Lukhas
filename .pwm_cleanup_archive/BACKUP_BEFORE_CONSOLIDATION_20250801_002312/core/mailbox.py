    ...
    async def add(self, message: ActorMessage, reason: str):
        ...
        async with self._lock:
            self.messages.append(
                {"message": message, "reason": reason, "timestamp": time.time()}
            )
...
class PriorityMailbox(Mailbox):
    ...
    async def put(
        self, message: ActorMessage, priority: MessagePriority = MessagePriority.NORMAL
    ) -> bool:
    ...
    class PriorityActor(MailboxActor):
        def __init__(self, actor_id: str):
            super().__init__(
                ...
                mailbox_config={"max_size": 100, "starvation_prevention": True},
            )
