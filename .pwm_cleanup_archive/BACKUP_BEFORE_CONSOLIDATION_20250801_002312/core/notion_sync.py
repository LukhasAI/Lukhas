    ...
    return {
        ...
        "code": {
            ...
            "rich_text": [
                {
                    "type": "text",
                    "text": {"content": content.strip()[:2000]},  # max block length
                }
            ],
        },
    }
    ...
    try:
        ...
        try:
            ...
            response = client.chat.completions.create(
                model="gpt-4", messages=[{"role": "user", "content": summary_prompt}]
            )
                    ...
                    summary_block = {
                        ...
                        "paragraph": {
                            "rich_text": [
                                {
                                    "type": "text",
                                    "text": {"content": f"🧠 GPT Summary: {summary}"},
                                }
                            ]
                        },
                    }
            ...
            audit_block = {
                ...
                "paragraph": {
                    "rich_text": [
                        {
                            "type": "text",
                            "text": {"content": f"🔏 Synced by {lambda_id}"},
                        }
                    ]
                },
            }
...
Examples:
  ...
  python notion_sync.py --LukhasDoc-sync /path/to/doc.json --notion-page-id your-page-id
    ...
    parser.add_argument(
        "--ui", action="store_true", help="Launch modern Streamlit UI (modular)"
    )
    parser.add_argument(
        "--legacy-ui",
        action="store_true",
    ...
    parser.add_argument(
        "--sync", choices=["manual", "auto", "daily"], help="Run sync operation"
    )
    parser.add_argument(
    ...
    parser.add_argument("--ΛDoc-sync", type=str, help="Sync ΛDoc output file to Notion")
    parser.add_argument(
        "--notion-page-id", type=str, help="Target Notion page ID for ΛDoc sync"
    )
    parser.add_argument(
        "--LukhasDoc-sync", type=str, help="Sync LukhasDoc output file to Notion"
    )
    parser.add_argument(
        "--notion-page-id", type=str, help="Target Notion page ID for LukhasDoc sync"
    )
