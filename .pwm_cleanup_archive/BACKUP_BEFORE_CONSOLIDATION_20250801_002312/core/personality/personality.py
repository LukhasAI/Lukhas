    ...
    def update_shyness(self, partner_id: str, interaction_quality: float):
        ...
        if self.federated_model:
            try:
                ...
                logger.warning(
                    "ΛPERSONALITY: Federated learning unavailable", error=str(e)
                )
...
class EtiquetteEngine:
    ...
    def set_cultural_preference(self, user_id: str, style: CulturalStyle):
        ...
        logger.info(
            "ΛPERSONALITY: Cultural preference set", user_id=user_id, style=style.value
        )
...
class PersonalityManager:
    ...
    def get_personality_profile(
        self, user_id: str, context: InteractionContext = InteractionContext.CASUAL
    ) -> Dict[str, Any]:
        """
    ...
    def update_from_interaction(
        self, user_id: str, interaction_quality: float, mood_impact: float = 0.0
    ):
