...
class IdentityAwareBaseColony(BaseColony if BASE_COLONY_AVAILABLE else ABC):
    ...
    async def unregister_user_context(self, user_id: str):
        ...
        keys_to_remove = [
            key
            for key in self.tier_authorization_cache.keys()
    ...
    def get_identity_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "supported_tier_levels": [
                tier.name for tier in self.tier_capability_matrix.keys()
            ],
...
def create_identity_aware_colony(
    colony_id: str, capabilities: List[str], colony_class: Optional[type] = None
