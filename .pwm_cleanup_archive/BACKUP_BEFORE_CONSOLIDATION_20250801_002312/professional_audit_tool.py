...
class ProfessionalAuditor:
    ...
    def _check_scatter(self, file_path: str, system: str) -> str:
        ...
        if (
            system == "dream"
            and current_dir not in ["dream", "creativity"]
            or system == "memory"
            and current_dir != "memory"
            or system in ["consciousness", "bio", "quantum", "identity"]
            and current_dir != system
        ):
            return "scattered"
            ...
            if data["scatter_status"] == "scattered":
                ...
                if data["system"] == "dream":
                    data["proposed_location"] = (
                        f"dream/{'/'.join(file_path.split('/')[1:])}"
                    )
                else:
                    data["proposed_location"] = (
                        f"{data['system']}/{'/'.join(file_path.split('/')[1:])}"
                    )
    ...
    def _identify_personality_files(self):
        ...
        for file_path, data in self.files_data.items():
            # Check filename
            if any(
                pattern in file_path.lower() for pattern in self.personality_patterns
            ):
            ...
            for export in data["exports"]:
                if any(
                    pattern in export.lower() for pattern in self.personality_patterns
                ):
                    data["personality_critical"] = True
                    self.personality_files.add(file_path)
                    break
    ...
    def _assess_commercial_potential(self):
        ...
        for file_path, data in self.files_data.items():
            ...
            high_score = sum(
                1
                for indicator in self.commercial_indicators["high"]
                if indicator in file_path.lower()
                or any(indicator in func for func in data["functions"])
            )

            medium_score = sum(
                1
                for indicator in self.commercial_indicators["medium"]
                if indicator in file_path.lower()
            )

            low_score = sum(
                1
                for indicator in self.commercial_indicators["low"]
                if indicator in file_path.lower()
            )

            # Determine potential
            if high_score >= 2 or (
                file_path.startswith("api/") and not data["personality_critical"]
            ):
    ...
    def _generate_consolidation_plan(self) -> Dict[str, Any]:
        ...
        for file_path, data in self.files_data.items():
            if (
                data["size"] < 100
                and len(data["functions"]) == 0
                and len(data["classes"]) == 0
    ...
    def _create_report(self, consolidation_plan: Dict[str, Any]) -> Dict[str, Any]:
        ...
        report = {
            "summary": {
                ...
                "scattered_files": sum(
                    1
                    for d in self.files_data.values()
                    if d["scatter_status"] == "scattered"
                ),
                "personality_critical_files": len(self.personality_files),
                "high_commercial_potential": sum(
                    1
                    for d in self.files_data.values()
                    if d["commercial_potential"] == "high"
                ),
    ...
    def _group_personality_files(self) -> List[List[str]]:
        ...
        voice_group = [
            f
            for f in self.personality_files
...
def main():
    ...
    with open("AUDIT_SUMMARY.md", "w") as f:
        ...
        f.write(
            f"- Personality Critical: {report['summary']['personality_critical_files']}\n"
        )
        f.write(
            f"- High Commercial Potential: {report['summary']['high_commercial_potential']}\n\n"
        )
