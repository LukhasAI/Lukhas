...
from candidate.core.endocrine.hormone_system import get_endocrine_system
...
class EndocrineObservabilityEngine:
    ...
    def __init__(
        ...
        self.snapshot_retention = self.config.get(
    ...
    async def create_snapshot(
        ...
        snapshot = EndocrineSnapshot(
            hormone_levels=hormone_levels or {},
            system_metrics=system_metrics or {},
        )
        # Compute a quick coherence estimate based on provided data
        try:
            cortisol = snapshot.hormone_levels.get("cortisol", 0.5)
            adrenaline = snapshot.hormone_levels.get("adrenaline", 0.5)
            dopamine = snapshot.hormone_levels.get("dopamine", 0.5)
            serotonin = snapshot.hormone_levels.get("serotonin", 0.5)
            positive = (dopamine + serotonin) / 2.0
            stress = (cortisol + adrenaline) / 2.0
            balance = 1.0 - abs(stress - positive)
            perf = snapshot.system_metrics.get("performance", 0.7)
            snapshot.coherence_score = max(0.0, min(1.0, balance * 0.6 + perf * 0.4))
        except Exception:
    ...
    async def assess_homeostasis(
        ...
        if not hormones:
            return {"balance_score": 0.6}
        values = list(hormones.values())
        avg = sum(values) / len(values)
        variance = sum((v - avg) ** 2 for v in values) / max(1, len(values))
        # Lower variance implies better balance; normalize inversely
        balance_score = max(0.0, min(1.0, 1.0 - min(1.0, variance)))
        return {"balance_score": balance_score}
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            try:
                from candidate.orchestration.signals.homeostasis_controller import (
            ...
            logger.error(
                "Failed to initialize EndocrineObservabilityEngine", error=str(e)
            )
            return False
    ...
    async def _capture_endocrine_snapshot(self) -> EndocrineSnapshot:
        ...
        try:
            ...
            if self.homeostasis_controller:
                snapshot.homeostasis_state = str(
                    self.homeostasis_controller.current_state
                )
    ...
    async def _analyze_plasticity_triggers(
        ...
        if (
            cortisol > thresholds["cortisol_high"]
            or adrenaline > thresholds["adrenaline_high"]
            or combined_stress > thresholds["combined_stress"]
        ):
        ...
        perf_thresholds = self.trigger_thresholds[
            PlasticityTriggerType.PERFORMANCE_OPTIMIZATION
        ]
        if (
        ...
        social_thresholds = self.trigger_thresholds[
            PlasticityTriggerType.SOCIAL_ENHANCEMENT
        ]
        ...
        recovery_thresholds = self.trigger_thresholds[
            PlasticityTriggerType.RECOVERY_CONSOLIDATION
        ]
        if (
        ...
        emotion_thresholds = self.trigger_thresholds[
            PlasticityTriggerType.EMOTIONAL_REGULATION
        ]
    ...
    async def _process_plasticity_trigger(
        ...
        try:
            ...
            logger.error(
                "Error processing plasticity trigger",
                trigger_type=trigger.trigger_type.value,
                error=str(e),
            )
    ...
    async def _apply_default_adaptation(
        ...
        try:
            if trigger.trigger_type == PlasticityTriggerType.STRESS_ADAPTATION:
                ...
                adaptation_applied = (
                    "Regulated emotional processing, balanced mood responses"
                )

            trigger.adaptation_applied = adaptation_applied
            logger.info(
                "Applied default adaptation",
                trigger_type=trigger.trigger_type.value,
            ...
            logger.error(
                "Error applying default adaptation",
                trigger_type=trigger.trigger_type.value,
                error=str(e),
            )
            return False
    ...
    async def _calculate_coherence_score(self) -> float:
        ...
        try:
            ...
            positive_hormones = (
                ...
                + hormone_levels.get("oxytocin", 0.5)
            ) / 3
    ...
    def register_adaptation_handler(
        self, trigger_type: PlasticityTriggerType, handler: Callable
    ):
