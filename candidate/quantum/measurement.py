...
class QuantumMeasurement:
    ...
    def collapse(
        ...
        if mode == "argmax":
            selected_index = max(
                range(len(adjusted_probabilities)), key=adjusted_probabilities.__getitem__
            )
        else:
        ...
        post_amplitudes = [
            amp * math.sqrt(1.0 - decoherence) for amp in state.amplitudes
        ]
        if post_amplitudes:
        ...
        post_state = SuperpositionState(
            options=state.options, amplitudes=post_amplitudes, metadata=post_metadata
        )

        selected_label = self._option_label(state.options[selected_index])

        self._history.record_measurement(
            state, context, selected_label, adjusted_probabilities[selected_index]
        )
    ...
    def _apply_bias(
        ...
        if not base_probabilities:
            base_probabilities = [1.0 / len(state.options)] * len(state.options) if state.options else []

        bias_map = dict(context.get("bias", {}))

        historical_bias = self._history.estimate_future_bias(state, context)
        for label, bias in historical_bias.items():
            bias_map[label] = bias_map.get(label, 0.0) + bias

        preferred = context.get("preferred_option")
        if preferred is not None:
            label = str(preferred)
            bias_strength = float(context.get("preferred_weight", 2.0))
            bias_map[label] = bias_map.get(label, 0.0) + bias_strength
        ...
        if total == 0 and weighted:
            logger.debug(
                "All biased probabilities zero; reverting to uniform distribution"
            )
    ...
    def _sample_index(self, probabilities: list[float]) -> int:
        ...
        for index, probability in enumerate(probabilities):
            ...
            if threshold <= cumulative:
                return index
        return len(probabilities) - 1 if probabilities else 0
    ...
    def _option_label(self, option: Mapping[str, Any]) -> str:
        ...
        try:
            return str(hash(frozet(option.items())))
        except TypeError:
