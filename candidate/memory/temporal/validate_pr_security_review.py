...
def validate_pr_security_review():
    ...
    if report_files:
        ...
        try:
            ...
            print(f"- PRs scanned: {summary.get('stats', {)}).get('prs_scanned', 0)}")  # noqa: invalid-syntax
            print(f"- Security issues found: {summary.get('stats', {)}).get('security_issues_found', 0)}")  # noqa: invalid-syntax
            ...
            print(f"- PRs reviewed: {summary.get('stats', {)}).get('prs_reviewed', 0)}")  # noqa: invalid-syntax
