...
class PatternDetector:
    ...
    def _detect_code_patterns(
        ...
        for file_path, count in file_modifications.most_common(10):
            if count >= 5:  # Modified 5+ times
                occurrences = [
                    ...
                    if any(
                        f["path"] == file_path and f["action"] == "M"
                        for f in c["files"]
                    )
                ]
        ...
        for prefix, commits in message_patterns.items():
            if len(commits) >= 5:
                occurrences = [
                    {"timestamp": c["timestamp"], "context": c["message"]}
    ...
    def _detect_time_patterns(
        ...
        late_night_work = sum(hour_activity[h] for h in range(22, 24)) + sum(
            hour_activity[h] for h in range(0, 6)
        )
    ...
    def _detect_error_patterns(
        ...
        error_entries = [
            e
            for e in entries
            if e.metadata.get("category") in ["failure", "challenge"]
            or any(
                word in e.content.lower()
                for word in ["error", "failed", "broke", "bug", "issue"]
            )
        ]
        ...
        for error_type, entries in error_types.items():
            if len(entries) >= 3:
                occurrences = [
                    {"timestamp": e.timestamp, "context": e.content[:100]}
                    for e in entries[:10]
                ]
    ...
    def _suggest_error_fix(self, error_type: str) -> str:
        ...
        return suggestions.get(
            error_type, "Analyze root cause and implement preventive measures"
        )
    ...
    def _detect_workflow_patterns(
                    ...
                    if commit["timestamp"] > decision.timestamp:
                        ...
                        if decision_files & commit_files:  # Intersection
                            ...
                            decision_implementation_times.append(
                                {
                                    "decision": decision.metadata.get(
                                        "title", "Unknown"
                                    ),
        ...
        if len(decision_implementation_times) >= 3:
            avg_time = sum(
                d["time"].total_seconds() for d in decision_implementation_times
            ) / len(decision_implementation_times)
            ...
            pattern = Pattern(
                ...
                automation_potential=False,
                suggested_action=(
                    "Track implementation progress more closely"
                    if avg_time > 259200
                    else None
                ),  # 3 days
            )
        ...
        if len(high_switch_days) >= 3:
            occurrences = [
                {
                    "timestamp": d["timestamp"],
                    "context": f"Worked on {len(d['topics'])} different topics: {', '.join(list(d['topics']}[:3]}...",  # noqa: invalid-syntax
                )}  # noqa: invalid-syntax
                for d in high_switch_days[:10]  # noqa: invalid-syntax
            ]  # noqa: invalid-syntax
            ...
            if avg_emotions:
                ...
                if dominant_emotion[1] > 0.7:  # Strong pattern
                    occurrences = [
                        ...
                        and dominant_emotion[0] in e.emotional_vector
                    ][:10]
            ...
            if len(emotion_triggers) >= 5:
                ...
                if common_words:
                    ...
                    pattern = Pattern(
                        ...
                        impact=(
                            "negative"
                            if emotion in ["frustration", "concern"]
                            else "positive"
                        ),
                        automation_potential=False,
    ...
    def _detect_file_patterns(
        ...
        if common_prefixes:
            examples = [
                ...
                if Path(f["path"]).name.startswith(common_prefixes[0])
            ]
            occurrences = [
                {"timestamp": e["timestamp"], "context": Path(e["path"]).name}
    ...
    def _detect_tool_usage_patterns(
        ...
        for tool, mentions in tool_mentions.items():
            if len(mentions) >= 5:
                occurrences = [
                    {"timestamp": m.timestamp, "context": m.content[:100]}
                ...
                impact = (
                    "negative"
                    if len(problem_mentions) > len(mentions) * 0.5
                    else "neutral"
                )
                ...
                pattern = Pattern(
                    ...
                    automation_potential=True,
                    suggested_action=(
                        f"Create {tool} aliases or automation scripts"
                        if impact == "negative"
                        else None
                    ),
                )
    ...
    def suggest_automations(self) -> list[dict[str, Any]]:
        ...
        for pattern in self.detected_patterns:
            if pattern.automation_potential and pattern.impact in [
                "negative",
                "neutral",
            ]:
                automation = {
    ...
    def _suggest_implementation(self, pattern: Pattern) -> str:
        ...
        return implementations.get(
            automation_type, "Analyze pattern and create custom automation"
        )
...
"""
        ...
        negative_patterns = [
            p for p in self.detected_patterns if p.impact == "negative"
        ]
        positive_patterns = [
        ...
        if any(p.frequency == "daily" for p in negative_patterns):
            report += (
                "4. **Fix daily friction points** - Small daily improvements compound\n"
            )
