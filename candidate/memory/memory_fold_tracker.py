...
class MemoryFoldTracker:
    """
    Tracks recurring symbolic patterns, collapses, and Constellation voids
    ...
    def detect_symbolic_recursion(self, sessions: Optional[list[dict]] = None) -> dict[str, Any]:
        ...
        # Constellation void detection
        constellation_voids = self._detect_trinity_voids(sessions)
        ...
        return {
            ...
            "constellation_voids": constellation_voids,
            "risk_assessment": self._assess_recursion_risk(
                glyph_recursions, persona_recursions, collapses, constellation_voids
            ),
        }
    ...
    def _detect_collapses(self, sessions: list[dict]) -> list[dict]:
        ...
        for i in range(1, len(sessions)):
            ...
            constellation_collapse = curr.get("constellation_coherence", 1) < self.collapse_indicators["constellation_void"]

            drift_critical = curr.get("drift_score", 0) > self.collapse_indicators["drift_critical"]

            if entropy_spike or constellation_collapse or drift_critical:
                collapses.append(
                    {
                        ...
                        "type": (
                            "entropy_spike"
                            if entropy_spike
                            else ("constellation_collapse" if constellation_collapse else "drift_critical")
                        ),
                        "metrics": {
                            "entropy": curr.get("entropy", 0),
                            "constellation": curr.get("constellation_coherence", 0),
    ...
    def _detect_trinity_voids(self, sessions: list[dict]) -> list[dict]:
        ...
        for i, session in enumerate(sessions):
            glyphs = set(session.get("glyphs", []))
            constellation_present = glyphs.intersection(self.constellation_core)

            if not constellation_present:
                voids.append(
                    {
                        "session": session.get("session_id", f"session_{i}"),
                        "missing": list(self.constellation_core),
    ...
    def _assess_recursion_risk(
        ...
        if len(voids) > 3:
            risk_score += 0.2
            risk_factors.append(f"Constellation voids ({len(voids)} sessions)")
    ...
    def suggest_stabilization_glyphs(self, analysis: dict[str, Any]) -> dict[str, Any]:
        ...
        if "Constellation voids" in str(risk.get("risk_factors", [])):
            suggestions.extend(list(self.constellation_core))
            rationale.append("Constellation glyphs to restore framework alignment")
    ...
    def _prioritize_glyphs(self, glyphs: list[str], analysis: dict) -> list[str]:
        ...
        # Constellation first if missing
        constellation_voids = analysis.get("constellation_voids", [])
        if constellation_voids:
            constellation_first = [g for g in glyphs if g in self.constellation_core]
            others = [g for g in glyphs if g not in self.constellation_core]
            return constellation_first + others
    ...
    def analyze_glyph_evolution(self, sessions: Optional[list[dict]] = None) -> dict[str, Any]:
        ...
        for i in range(0, len(sessions), window_size):
            ...
            constellation_count = sum(1 for g in window_glyphs if g in self.constellation_core)

            glyph_timeline.append(
                {
                    "window": f"sessions_{i}-{min(i + window_size - 1, len(sessions) - 1}",  # noqa: invalid-syntax
                    "total_glyphs": total_glyphs,  # noqa: invalid-syntax
                    "unique_glyphs": unique_glyphs,  # noqa: invalid-syntax
                    "constellation_ratio": (constellation_count / total_glyphs if total_glyphs > 0 else 0),  # noqa: invalid-syntax
                    "top_glyphs": [g for g, _ in glyph_counts.most_common(3)],  # noqa: invalid-syntax
                }  # noqa: invalid-syntax
            )  # noqa: invalid-syntax

        # Detect trends
        constellation_trend = "stable"  # noqa: invalid-syntax
        if len(glyph_timeline) >= 2:
            ...
            if last_trinity > first_trinity + 0.1:
                constellation_trend = "improving"
            elif last_trinity < first_trinity - 0.1:
                constellation_trend = "declining"

        return {
            "status": "analyzed",
            "timeline": glyph_timeline,
            "constellation_trend": constellation_trend,
