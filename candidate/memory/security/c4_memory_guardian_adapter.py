...
class GuardianMemoryOperation:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        return {
            ...
            "operation_type": (
                self.operation_type.value if hasattr(self.operation_type, "value") else self.operation_type
            ),
...
class C4MemoryGuardianAdapter:
    ...
    def _generate_operation_id(self) -> str:
        ...
        return f"guardian_op_{int(time.time())_{random_suffix}"  # noqa: invalid-syntax
    ...
    def _create_security_context(
        ...
        return MemorySecurityContext(
            user_id=user_id,
            session_id=f"c4_session_{int(time.time())",  # noqa: invalid-syntax
    ...
    def _log_operation(self, operation: GuardianMemoryOperation, result: dict[str, Any]):
        ...
        log_entry = {
            ...
            "user_id": (
                operation.security_context.user_id if hasattr(operation.security_context, "user_id") else "unknown"
            ),
            "security_level": (
