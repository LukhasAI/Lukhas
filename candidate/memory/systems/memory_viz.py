...
def format_flamegraph(flamegraph_lines, flamegraph_script=None):
    ...
    if not os.path.exists(flamegraph_script):
        ...
        logger.info(f"Downloading flamegraph.pl to: {flamegraph_script}")
...
def compare(before, after, format_flamegraph=format_flamegraph):
    ...
    logger.debug(f"only_before = {[a for a, _ in (before_segs - after_segs)}]}")  # noqa: invalid-syntax
    logger.debug(f"only_after = {[a for a, _ in (after_segs - before_segs)}]}")  # noqa: invalid-syntax
...
def trace(data):
    ...
    def format(entries):
        ...
        def _name():
            ...
            next_name += 1
            return f"{chr(ord('a'} + m}{'' if r == 0 else r)}"  # noqa: invalid-syntax
