...
if not fold_state:  # noqa: F821
        return 0.0
avg_res = sum(item.get("resonance", 0.0) for item in fold_state) / len(fold_state)  # noqa: F821
avg_entropy = sum(item.get("entropy", 0.0) for item in fold_state) / len(fold_state)  # noqa: F821
...
for item in fold_state:  # noqa: F821
        adjusted = dict(item)
...
return [item.get("entropy", 0.0) for item in fold_state]  # noqa: F821
