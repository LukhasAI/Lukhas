...
class ReasoningAgent:
    ...
    async def _retrieve_relevant_memories(self, query: ReasoningQuery) -> list[tuple[str, Any]]:
        ...
        if query.emotional_context > 0.7:
            memories = [m for m in memories if self._get_memory_emotion(m) > 0.5]
    ...
    async def _store_insight(self, insight: ReasoningInsight):
        ...
        self.memory.remember(
            content,
            {"type": "reasoning_insight", "id": f"insight_{insight.insight_id}"},
        )
...
class ReasoningColony:
    ...
    def _calculate_insight_similarity(self, insight1: ReasoningInsight, insight2: ReasoningInsight) -> float:
        ...
        # Weighted average
        similarity = symbol_overlap * 0.4 + word_overlap * 0.4 + type_similarity * 0.2
