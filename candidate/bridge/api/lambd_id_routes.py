...
__all__ = ["lambd_id_bp", "lambda_id_bp"]
# ═══════════════════════════════════════════════════════════════════════════
...
# Attempt to import controller and core services (though controller should
...
logger.info(
    f"ΛTRACE: Flask Blueprint 'lambd_id_v1' created with url_prefix: {lambd_id_bp.url_prefix}"
)
...
# For now, assuming it's either configured with an app elsewhere or works
# standalone on blueprint routes.
limiter = Limiter(
...
logger.info("ΛTRACE: Flask-Limiter initialized for lambd_id_bp with default limits.")
...
def validate_lambda_id_route():  # Renamed:
    ...
    try:
        ...
        logger.info(
            f"ΛTRACE ({req_id}): /validate response for ΛiD '{lambda_id_to_validate}'. Valid: {result.get('valid', False)}, Details: {result.get('validation_details')}"
        )
        return (
            jsonify(result),
            200,
...
def health_check_route():  # Renamed:
    ...
    try:
        ...
        http_status = (
            200 if response_payload["status"] == "healthy" else 503
        )  # 503 Service Unavailable if degraded
        logger.info(
            f"ΛTRACE ({req_id}): /health response. Overall Status: {response_payload['status']}, HTTP Status: {http_status}"
        )
        return jsonify(response_payload), http_status

    except Exception as e:
        logger.error(
            f"ΛTRACE ({req_id}): Unhandled error in /health endpoint: {e}",
            exc_info=True,
        )
        return (
            jsonify(
...
# Human-readable comment: Custom error handler for rate limit exceeded on
# this blueprint.
...
def handle_rate_limit_exceeded_on_bp(e):  # Renamed:
    ...
    error_description = getattr(
        e, "description", "Rate limit exceeded for this endpoint."
    )
    logger.warning(
        f"ΛTRACE ({req_id}): Rate limit exceeded for {request.remote_addr} on {request.path}. Description: {error_description}"
    )
    return (
        jsonify(
            {
...
def handle_not_found_on_bp(_e):  # noqa: ARG001 - Flask error handler protocol compliance
    """Handles 404 Not Found errors for routes under lambd_id_bp."""
    req_id = _get_req_id("err404")
    logger.warning(
        f"ΛTRACE ({req_id}): Resource not found within lambd_id_bp at {request.path}."
    )
    return (
        jsonify(
            {
...
def handle_method_not_allowed_on_bp(_e):  # noqa: ARG001 - Flask error handler protocol compliance
    """Handles 405 Method Not Allowed errors for routes under lambd_id_bp."""
    req_id = _get_req_id("err405")
    logger.warning(
        f"ΛTRACE ({req_id}): Method {request.method} not allowed for {request.path} within lambd_id_bp."
    )
    return (
        jsonify(
            {
