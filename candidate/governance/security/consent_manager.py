...
class ConsentRequest:
    ...
    def __post_init__(self):
        ...
        if self.triad_impact is None:
            self.triad_impact = {"identity": 0.0, "consciousness": 0.0, "guardian": 0.0}
...
class ConsentManager(GlyphIntegrationMixin):
    ...
    DEFAULT_ESCALATION_RULES = [
        ...
        },
        {
            ...
            "condition": "triad_impact.get('identity', 0) > 0.7",
        ...
        },
        {
            ...
            "condition": "triad_impact.get('consciousness', 0) > 0.8",
            "escalation_level": EscalationLevel.HIGH,
    ...
    def _create_default_trust_paths(self):
        ...
        default_paths = [
            ...
            TrustPath(
                path_id="triad_framework_protected",
                ...
                last_validated=time.time(),
                validation_count=0,
                symbolic_signature=self.TRUST_SYMBOLS["triad_protected"],
                metadata={"triad_protected": True, "governance_approved": True},
                governance_approved=True,
            ),
        ]
    ...
    def _rebuild_requester_stats(self):
        ...
        for entry in self.consent_history:
            ...
            if "triad_impact" in entry:
                stats["triad_impact_history"].append(entry["triad_impact"])

            # Keep only recent history
            if len(stats["trust_history"]) > 20:
                stats["trust_history"] = stats["trust_history"][-20:]
            if len(stats["triad_impact_history"]) > 20:
                stats["triad_impact_history"] = stats["triad_impact_history"][-20:]
    ...
    async def process_consent_request(self, request: ConsentRequest) -> ConsentRequest:
        ...
        try:
            ...
            triad_analysis = await self._analyze_triad_impact(request)
            request.triad_impact = triad_analysis["impact_scores"]
            ...
            if escalation_result:
                ...
                escalation_entry = {
                    ...
                    "triad_impact": request.triad_impact,
                }
                ...
                triad_risk = max(request.triad_impact.values())
                adjusted_trust = request.trust_score * governance_weight * (1.0 - triad_risk * 0.3)
    ...
    async def _validate_governance_compliance(self, request: ConsentRequest) -> dict[str, Any]:
        ...
        triad_protected = self._is_triad_protected_resource(request)
        if triad_protected and not request.context.get("triad_authorized", False):
            return {
                "approved": False,
                "reason": "Trinity Framework authorization required",
                "violations": ["triad_authorization_required"],
            }
    ...
    def _is_triad_protected_resource(self, request: ConsentRequest) -> bool:
        """Check if resource is protected by Trinity Framework"""
        resource = request.target_resource.lower()
        triad_resources = ["identity", "consciousness", "guardian", "constellation", "core", "system"]
        return any(term in resource for term in triad_resources)

    async def _analyze_triad_impact(self, request: ConsentRequest) -> dict[str, Any]:
    ...
    def _update_requester_stats(self, request: ConsentRequest):
        ...
        if request.triad_impact:
            stats["triad_impact_history"].append(request.triad_impact)
    ...
    def _path_applies_to_request(self, path: TrustPath, request: ConsentRequest) -> bool:
        ...
        if path.metadata.get("triad_protected"):
            return basic_match and self._is_triad_protected_resource(request)
    ...
    def _evaluate_enhanced_condition(self, condition: str, context: dict) -> bool:
        ...
        try:
            ...
            if "triad_impact.get(" in condition:
                if "triad_impact.get('identity', 0) > 0.7" in condition:
                    return context["triad_impact"].get("identity", 0) > 0.7
                elif "triad_impact.get('consciousness', 0) > 0.8" in condition:
                    return context["triad_impact"].get("consciousness", 0) > 0.8
                elif "triad_impact.get('guardian', 0) > 0.6" in condition:
                    return context["triad_impact"].get("guardian", 0) > 0.6
        ...
        for action in actions:
            try:
                if action == "require_multi_factor":
                    ...
                    request.context["triad_impact_assessment_required"] = True
                else:
    ...
    def _generate_symbolic_response(self, request: ConsentRequest) -> list[str]:
        ...
        if request.triad_impact and max(request.triad_impact.values()) > 0.7:
            if request.triad_impact["identity"] > 0.7:
                base_symbols.append("⚛️")
            if request.triad_impact["consciousness"] > 0.7:
                base_symbols.append("🧠")
            if request.triad_impact["guardian"] > 0.7:
                base_symbols.append("🛡️")
    ...
    def get_requester_trust_summary(self, requester: str) -> dict:
        ...
        triad_history = stats["triad_impact_history"]
        ...
        avg_triad_impact = {"identity": 0.0, "consciousness": 0.0, "guardian": 0.0}
        if triad_history:
            for impact in triad_history:
                for component in avg_triad_impact:
                    avg_triad_impact[component] += impact.get(component, 0.0)
            for component in avg_triad_impact:
                avg_triad_impact[component] /= len(triad_history)
        ...
        return {
            ...
            "trust_trend": trend,
            "average_triad_impact": avg_triad_impact,
    ...
    def get_enhanced_consent_statistics(self) -> dict:
        ...
        for entry in self.consent_history:
            ...
            triad_impact = entry.get("triad_impact", {})
            if any(impact > 0.7 for impact in triad_impact.values()):
                triad_high_impact += 1
        ...
        return {
            ...
            "triad_high_impact_rate": triad_high_impact / total_requests,
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            triad_analysis = await self._analyze_triad_impact(test_request)
...
if __name__ == "__main__":
    ...
    async def demo():
        ...
        for request in test_requests:
            ...
            print(
                f"   Trinity impact: I:{result.triad_impact['identity']:.1f} C:{result.triad_impact['consciousness']:.1f} G:{result.triad_impact['guardian']:.1f}"
            )
        ...
        print(f"   Trinity high impact rate: {stats['triad_high_impact_rate']:.2f}")
