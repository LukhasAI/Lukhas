...
class SecurityComplianceChecker:
    ...
    def generate_compliance_report(self) -> str:
        ...
        report.extend(["", "#)  #  🔍 Detailed Results", ""]  # noqa: invalid-syntax

        for check_name, check_result in results["checks"].items():  # noqa: invalid-syntax
            report.extend(
                [
                    f"##"_', ' ').title()}",'  # noqa: invalid-syntax
                    f"**Status:** {check_result['status']}",
                    "",
                ]
            )

            if "details" in check_result:  # noqa: invalid-syntax
                report.append("**Details:**")  # noqa: invalid-syntax
                for key, value in check_result["details"].items():  # noqa: invalid-syntax
                    report.append(f"- {key.replace('_', ' ').title()}: {value}")
                report.append("")  # noqa: invalid-syntax

            if "recommendations" in check_result:  # noqa: invalid-syntax
                report.append("**Recommendations:**")  # noqa: invalid-syntax
                for rec in check_result["recommendations"]:  # noqa: invalid-syntax
                    report.append(f"- {rec}")
                report.append("")  # noqa: invalid-syntax

        report.extend(  # noqa: invalid-syntax
            [
                "## 🎯 Next Steps",
                "",
                "### Immediate Actions",
