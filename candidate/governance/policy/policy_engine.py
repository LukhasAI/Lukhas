...
class PolicyRuleEngine:
    ...
    def _check_rule_conflicts(self, rule: PolicyRule) -> list[str]:
        ...
        for existing_rule_id, existing_rule in self.rules.items():
            ...
            if (rule.policy_type == existing_rule.policy_type and
                rule.scope == existing_rule.scope and
                self._conditions_similar(rule.conditions, existing_rule.conditions)):
...
class PolicyEnforcementEngine:
    ...
    async def evaluate_policies(
        ...
        try:
            ...
            return PolicyEvaluationResult(
                ...
                action_reason=f"Policy evaluation error: {str(e)}",
                confidence=0.5,
                evaluation_time=evaluation_time,
    ...
    async def _evaluate_rule(
        ...
        try:
            ...
            return {
                "triggered": False,
                "violation": None,
                "warning": f"Rule evaluation error: {str(e)}",
                "recommendations": []
            }
    ...
    async def _determine_final_action(
        ...
        critical_violations = [v for v in violations if v.severity in [PolicyPriority.EMERGENCY, PolicyPriority.CRITICAL]]
    ...
    async def _update_metrics(self, result: PolicyEvaluationResult):
        ...
        for rule_id in result.triggered_rules:
            if rule_id not in self.metrics["policy_effectiveness"]:
                self.metrics["policy_effectiveness"][rule_id] = {"triggers": 0, "effectiveness": 0.0}
