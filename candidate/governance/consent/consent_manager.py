...
class AdvancedConsentManager:
    ...
    def __init__(self, config: Optional[dict[str, Any]] = None):
        ...
        self.metrics = {
            ...
            "consent_by_regime": {},
            "withdrawal_rate": 0.0,
            "last_updated": datetime.now(timezone.utc).isoformat(),  # noqa: F821
        }
    ...
    async def withdraw_consent(
        ...
        try:
            ...
            withdrawal_time = datetime.now(timezone.utc)  # noqa: F821
    ...
    async def validate_consent(
        ...
        try:
            ...
            # Check expiration
            if consent.expires_at and datetime.now(timezone.utc) > consent.expires_at:  # noqa: F821
                # Update status to expired
                consent.status = ConsentStatus.EXPIRED
                consent.updated_at = datetime.now(timezone.utc)  # noqa: F821
    ...
    async def _create_consent_record(
        ...
        current_time = datetime.now(timezone.utc)  # noqa: F821
    ...
    async def _trigger_data_processing_updates(self, user_id: str, withdrawn_consents: list[ConsentRecord]):
        ...
        for consent in withdrawn_consents:
            ...
            consent.guardian_validations.append(
                f"Data processing stopped immediately upon withdrawal at {datetime.now(timezone.utc)}"  # noqa: F821
            )
