...
class PLS:
    ...
    def _extract_soothing_signals(self, signals: dict[str, Any]) -> float:
        ...
        if "text" in signals:
            text = str(signals["text"]).lower()
            soothing_keywords = ["calm", "peaceful", "gentle", "soothing", "comfort", "safe", "serene"]
    ...
    def _extract_temporal_pressure(self, signals: dict[str, Any]) -> float:
        ...
        if "text" in signals:
            text = str(signals["text"]).lower()
            urgency_keywords = ["urgent", "asap", "immediately", "deadline", "hurry", "quick", "fast"]
    ...
    def _extract_agency_signals(self, signals: dict[str, Any]) -> float:
        ...
        if "text" in signals:
            text = str(signals["text"]).lower()
            active_keywords = ["choose", "decide", "control", "manage", "direct", "lead"]
