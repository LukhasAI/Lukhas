...
def get_human_proposals(
    ...
    try:
        ...
        proposals = engine.propose_tone_adaptations(
            target_file="qi/safety/policy_packs/global/mappings.yaml", user_focus=user
        )
    ...
    try:
        ...
        dashboard = {
            ...
            "evaluation": {
                "latest": latest_eval,
                "status": (
                    "pass" if latest_eval and latest_eval.get("summary", {}).get("num_failures", 1) == 0 else "fail"
                ),
            },
            "production": {
...
def root():
    return {
        "message": "LUKHAS Unified Ops Cockpit API",
        "version": "1.0.0",
        "docs": "/docs",
        "ui": "/ui",
    }
