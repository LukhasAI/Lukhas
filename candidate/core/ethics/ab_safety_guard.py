...
class ABSafetyGuard:
    ...
    def __init__(self, config: Optional[ABSafetyConfig] = None):
        ...
        self.circuit_failures = {"candidate": 0, "control": 0}
        self.circuit_open_until = {"candidate": None, "control": None}

        # Metrics tracking for rollback detection
        self.recent_decisions = {"candidate": [], "control": []}
    ...
    def _check_rollback_conditions(self) -> None:
        ...
        candidate_decisions = self.recent_decisions.get("candidate", [])
        ...
        # Update metrics
        if METRICS_AVAILABLE:
            SAFETY_GUARD_CRITICAL_BLOCK_RATE.labels(lane="candidate").set(block_rate)
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "enforcement": {
                    "candidate": self.config.candidate_enforcement.value,
                    "control": self.config.control_enforcement.value
                },
                "circuit_breaker": {
                    "candidate_open": self._is_circuit_open("candidate"),
                    "control_open": self._is_circuit_open("control"),
                    "candidate_failures": self.circuit_failures.get("candidate", 0),
                    "control_failures": self.circuit_failures.get("control", 0)
                },
                "recent_metrics": {
                    "candidate": calc_rates(candidate_decisions),
