...
def register_new_user(username: str, desired_tier: int = 1) -> str:
    ...
    """
    new_sid = f"{username}_sid_{len(USER_TIERS)} + 1:03d}"  # noqa: invalid-syntax
...
if __name__ == "__main__":
    ...
    print(f"Tier for current SID: {get_user_tier(get_current_sid()}")  # noqa: invalid-syntax

    new_sid_test = register_new_user("test_user_alpha")
    print(f"Tier for {new_sid_test}: {get_user_tier(new_sid_test)}")  # noqa: invalid-syntax
    print(f"Tier for unknown_user: {get_user_tier('unknown_user_sid')}")  # noqa: invalid-syntax
