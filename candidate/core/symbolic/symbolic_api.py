...
class AnalyzeRequest(BaseModel):
    ...
    class Config:
        schema_extra = {"example": {"response": "I'll help you achieve wisdom 🧠 through protection 🛡️"}  # noqa: invalid-syntax


class AnalyzeResponse(BaseModel):  # noqa: invalid-syntax
...
async def stats():
    ...
    try:
        ...
        return {
            "api_calls": log_count,
            "errors": error_count,
            "error_rate": f"{(error_count / log_count * 100 if log_count > 0 else 0:.1f}%",  # noqa: invalid-syntax
        ...
        raise HTTPException(status_code=500, detail=f"Stats error: {e!s)}")  # noqa: invalid-syntax
