...
class CapabilityEvaluationFramework:
    ...
    async def assess_capability_domain(self, 
        ...
        try:
            ...
            self.logger.error(f"Capability assessment failed for {domain.value}: {str(e)}")
            raise
    ...
    async def generate_capability_profile(self, 
        ...
        try:
            ...
            self.logger.error(f"Capability profile generation failed: {str(e)}")
            raise
    ...
    def _calculate_measurement_consistency(self, measurements: List[CapabilityMeasurement]) -> float:
        ...
        consistencies = []
        for domain, domain_measurements in domain_groups.items():
    ...
    async def _handle_agi_threshold_detection(self, profile: CapabilityProfile):
        ...
        for callback in self.alert_callbacks:
            try:
                ...
                self.logger.error(f"AGI alert callback failed: {str(e)}")
