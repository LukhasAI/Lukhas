...
class MemoryReflectionSystem:
    ...
    def _assess_cascade_risk(self, signal: dict[str, Any]) -> float:
        ...
        recent_failures = sum(
            1 for entry in self.reflection_history[-10:] if entry.get("status") == "failed"
        )
    ...
    def _calculate_pair_coherence(self, fold1: MemoryFold, fold2: MemoryFold) -> float:
        ...
        # Emotional coherence
        emotion_coherence = (
            1 - np.linalg.norm(np.array(fold1.vad_encoding) - np.array(fold2.vad_encoding)) / 3.0
        )
    ...
    def _generate_reflection_insights(
        self, folds: list[MemoryFold], signal: dict[str, Any], context: ReflectionContext
    ) -> list[dict[str, Any]]:
    ...
    def _create_insight_fold(
        ...
        new_fold = MemoryFold(
            ...
            causal_weight=insight["confidence"]
