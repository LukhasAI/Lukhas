...
from .enhanced_matriz_adapter import SignalType, EnhancedMatrizAdapter
from .consciousness_data_flow import (
...
class ConsciousnessNetworkMonitor:
    ...
    async def _calculate_network_health(self) -> NetworkHealth:
        ...
        consciousness_coherence = flow_state_score
        
        # 2. Bio-Adaptation Health (pattern emergence and effectiveness)
        bio_efficiency = network_status.get("metrics", {}).get("bio_adaptation_efficiency", 0.0)
        bio_adaptation_health = bio_efficiency
        
        # 3. Trinity Framework Compliance
        trinity_compliance_score = adapter_metrics.get("trinity_compliance_rate", 0.0)
        
        # 4. Cascade Prevention Effectiveness  
        cascade_prevention_score = flow_metrics.cascade_prevention_rate
        
        # 5. Performance Score (latency and throughput)
        performance_score = await self._calculate_performance_score(flow_metrics)
