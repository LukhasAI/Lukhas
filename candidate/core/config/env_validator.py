    ...
    def create_env_example(self) -> str:
        ...
        for config in self.REQUIRED_VARS:
            ...
            if config.allowed_values:
                lines.append(f"# Allowed values: {', '.join(map(str, config.allowed_values)}")  # noqa: invalid-syntax  # TODO: Expected ,, found }
            if config.min_length:  # noqa: invalid-syntax  # TODO: Expected ), found dedent
