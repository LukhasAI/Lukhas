...
class EthicalEvaluator:
    ...
    def evaluate_action(self, action):
        return all(rule.evaluate(action) for rule in self.rules)
...
class ReflectiveLearning:
    ...
    def evaluate_performance(self):
        positive_interactions = sum(
            1 for h in self.history if getattr(h, "successful", False)
        )
...
def prompt_consent():
    ...
    consent = (
        input("Do you agree to share this data for feedback, dream narration,"
              and voice synthesis? (yes / no): ""  # noqa: invalid-syntax
              )
        .strip()
        .lower()
    )
...
def prompt_emotion():
    print(
        "What is the emotional tone for this response? (neutral/happy/excited/sad/serious/dream)"
    )
...
def prompt_feedback(user_id, action, output_text, emotion):
    print("\nHow would you rate the emotional intensity of the output? (1 - 5,"
                                                                        5=most intense)""  # noqa: invalid-syntax
          )  # noqa: invalid-syntax
    rating = input("> ").strip()
    try:
        rating = int(rating)
...
def prompt_dream_feedback(user_id, dream_text):  # noqa: invalid-syntax
...
def request_data_deletion(user_id):
    ...
    with open(PUBLISH_QUEUE_PATH) as f:
        logs = json.load(f)
