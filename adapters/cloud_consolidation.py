...
class CloudConsolidationService:
    ...
    async def analyze_consolidation(
        self, request: ConsolidationRequest, capability_tokens: dict[str, str]
    ) -> ConsolidationPlan:
        """
    ...
    async def _analyze_files(self, files: list[ResourceMetadata], request: ConsolidationRequest) -> ConsolidationPlan:
        ...
        large_files = [f for f in files if f.size and f.size > large_threshold]
    ...
    async def _find_duplicates(self, files: list[ResourceMetadata]) -> list[DuplicateGroup]:
        ...
        for (size, name), file_group in size_groups.items():
            if len(file_group) > 1:
                ...
                for f in file_group:
                    service_tag = next(
                        (tag for tag in (f.tags or []) if tag.startswith("service:")),
                        "service:unknown",
                    )
...
async def demonstrate_consolidation():
    ...
    tokens = {"drive": "mock_capability_drive_gonzo", "dropbox": "mock_capability_dropbox_gonzo"}
