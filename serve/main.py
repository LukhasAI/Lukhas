...
matriz_traces_router = _safe_import_router("MATRIZ.traces_router", "router") or _safe_import_router(
    "matriz.traces_router", "router"
)
...
app = FastAPI(
    ...
    contact={"name": "LUKHAS AI Team", "url": "https://github.com/LukhasAI/Lukhas"},
...
class StrictAuthMiddleware(BaseHTTPMiddleware):
    ...
    def _auth_error(self, message: str) -> Response:
        ...
        error_response = {
            "error": {"message": {"error": error_detail}, "type": error_detail["type"], "code": error_detail["code"]}
        }
...
async def create_response(request: dict) -> dict[str, Any]:
    ...
    if ASYNC_ORCH_ENABLED and _RUN_ASYNC_ORCH is not None:
        ...
        metrics_snapshot = (
            orchestrator_result.get("orchestrator_metrics") if isinstance(orchestrator_result, dict) else None
        )
        if metrics_snapshot:
        ...
        if orchestrator_answer:
            ...
            logger.info(
                "Async MATRIZ orchestrator returned error; retaining stub response: %s", orchestrator_result["error"]
            )
    return {
        "id": rid,
        "object": "chat.completion",
