...
try:
    ...
    from agi_core.orchestration.capability_matrix import CapabilityMatrix, CapabilityDimension
    from agi_core.orchestration.cost_optimizer import CostOptimizer, OptimizationStrategy, CostConstraints
...
async def initialize_orchestration_components():
    ...
    global agi_model_router, agi_consensus_engine, agi_capability_matrix, agi_cost_optimizer
    ...
    try:
        ...
        agi_capability_matrix = CapabilityMatrix()
        agi_cost_optimizer = CostOptimizer()
        agi_model_router = ModelRouter()
        agi_consensus_engine = ConsensusEngine()
    ...
    try:
        ...
        if agi_cost_optimizer and request.cost_constraints:
            ...
            recommendations.extend([
                f"Most cost-efficient: {optimized_models[0][0]}" if optimized_models else "No models meet cost constraints",
                f"Best quality/cost ratio found" if optimized_models else "Consider relaxing cost constraints"
            ])
        ...
        if not recommendations:
            recommendations = [
                ...
                "Consider constellation context for better alignment" if not request.constellation_filter else "Constellation alignment applied"
            ]
    ...
    try:
        ...
        if AGI_ORCHESTRATION_AVAILABLE:
            ...
            if agi_consensus_engine:
                consensus_stats = agi_consensus_engine.get_consensus_stats() if hasattr(agi_consensus_engine, 'get_consensus_stats') else {}
                stats["consensus"] = {
                    "available_methods": ["majority_vote", "weighted_quality", "confidence_threshold", "iterative_refinement", "dream_synthesis"],
