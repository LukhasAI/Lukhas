...
class PersonaSimilarityEngine:
    ...
    def __init__(self, persona_profile_path: str = "symbolic_persona_profile.yaml"):
        ...
        logger.info(f"🧬 Persona Similarity Engine initialized")
        logger.info(f"   Loaded {len(self.personas)} personas")
        logger.info(f"   Embedding style: symbolic_keywords + persona_traits")
        ...
        try:
            ...
            for name, profile in self.personas.items():
                ...
                if not (has_glyphs and has_traits and has_emotions):
                    missing = []
                    if not has_glyphs: missing.append('glyphs')
                    if not has_traits: missing.append('traits/dominant_traits')
                    if not has_emotions: missing.append('emotional_resonance')
    ...
    def _create_session_embedding(self, symbolic_trace: Dict[str, Any]) -> np.ndarray:
        ...
        if entropy < 0.3:
            if f"emotion_valence_positive" in self.feature_map:
                embedding[self.feature_map[f"emotion_valence_positive"]] = 0.7
            if f"emotion_arousal_low" in self.feature_map:
                embedding[self.feature_map[f"emotion_arousal_low"]] = 0.7
        elif entropy > 0.7:
            if f"emotion_valence_negative" in self.feature_map:
                embedding[self.feature_map[f"emotion_valence_negative"]] = 0.7
            if f"emotion_arousal_high" in self.feature_map:
                embedding[self.feature_map[f"emotion_arousal_high"]] = 0.7
    ...
    def rank_personas(self, symbolic_trace: Dict[str, Any], 
                     top_n: int = 3) -> List[PersonaMatch]:
        """
    ...
    def _calculate_trait_alignment(self, symbolic_trace: Dict[str, Any], 
    ...
    def _generate_explanation(self, symbolic_trace: Dict[str, Any], 
                            persona: Dict[str, Any], score: float, 
                            glyph_overlap: int) -> str:
    ...
    def evolve_persona(self, drift_history: List[Dict[str, Any]], 
                      current_persona: str) -> Dict[str, Any]:
        """
    ...
    def _generate_batch_recommendations(self, stability: float, 
        ...
        if isinstance(thresholds, dict) and thresholds.get('max', 0) > 0.7:
            recommendations.append(f"Dominant persona '{dominant}' has high drift threshold - ensure Guardian oversight")
        
        return recommendations
    
    def export_similarity_report(self, symbolic_trace: Dict[str, Any], 
                               output_path: str = "similarity_report.json") -> str:
        """
