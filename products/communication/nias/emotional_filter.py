...
class EmotionalFilter:
    ...
    async def create_user_profile(
        ...
        protection_level: EmotionalProtectionLevel = EmotionalProtectionLevel.STANDARD,
    ) -> EmotionalProfile:
    ...
    async def _check_dream_residue_protection(
        self,
        message: SymbolicMessage,
        ...
        if message.metadata and message.metadata.get("emergency", False):
            return {
                "approved": True,
                "reason": "Emergency override during dream residue",
            }

        # Allow gentle, therapeutic messages
        therapeutic_rule = self.therapeutic_rules.get("dream_recovery", {})
        if message.emotional_tone in therapeutic_rule.get(
            "allowed_tones", []
        ) and message.intensity <= therapeutic_rule.get("max_intensity", 0.2):
            return {
                "approved": True,
                "reason": "Therapeutic message approved during dream residue",
            }
    ...
    async def _check_stress_protection(
        self,
        message: SymbolicMessage,
        emotional_state: EmotionalVector,
        profile: EmotionalProfile,
    ...
    async def _check_emotional_tone_compatibility(
        self,
        message: SymbolicMessage,
        emotional_state: EmotionalVector,
        profile: EmotionalProfile,
        ...
        if emotional_state.focus >= 0.8:  # High focus state
            if message.emotional_tone not in [
    ...
    async def _check_intensity_capacity(
        self,
        message: SymbolicMessage,
        emotional_state: EmotionalVector,
        profile: EmotionalProfile,
    ...
    async def _get_delivery_recommendation(
        self,
        message: SymbolicMessage,
        emotional_state: EmotionalVector,
        profile: EmotionalProfile,
        ...
        if (
            message.voice_tag
            and emotional_state.stress < 0.3
            and profile.protection_level != EmotionalProtectionLevel.GUARDIAN
        ):
