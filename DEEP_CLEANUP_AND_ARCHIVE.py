...
class WorkspaceCleanup:
    ...
    def __init__(self):
        ...
        self.archive_dir = Path.home() / "lukhas-archive" / f"archive_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        self.stats = {
            "files_archived": 0,
            "files_cleaned": 0,
            "bytes_freed": 0,
        ...
        for py_file in self.workspace.rglob("*.py"):
            ...
            try:
                ...
                for imp in imports:
                    ...
                    if not imp.startswith(("lukhas", "lambda", "governance", "consciousness", "memory")):
                        continue
        ...
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            for file in self.workspace.rglob("*"):
                if file.is_file():
                    ...
                    if any(skip in str(file) for skip in ["__pycache__", ".pyc", ".pyo", ".DS_Store", ".git"]):
                        continue
...
Setup.py:
   ✅ Updated with current structure
    ...
    def run_deep_cleanup(self):
        ...
        import_issues = self.check_import_paths()
