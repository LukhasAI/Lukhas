...
Production-ready REST API with WebSocket support for MATADA cognitive system
...
class QueryRequest(BaseModel):
    ...
    include_nodes: bool = Field(default=True, description="Include MATADA nodes in response")
...
class QueryResponse(BaseModel):
    ...
    matada_nodes: Optional[List[Dict[str, Any]]] = Field(None, description="Generated MATADA nodes")
...
class SystemInfo(BaseModel):
    ...
    matada_graph_size: int = Field(..., description="Number of nodes in MATADA graph")
...
async def initialize_orchestrator():
    ...
    logger.info("Initializing MATADA Cognitive Orchestrator...")
...
async def lifespan(app: FastAPI):
    ...
    logger.info("MATADA-AGI FastAPI server started")
    
    yield
    
    # Shutdown
    await cleanup_orchestrator()
    logger.info("MATADA-AGI FastAPI server stopped")


# Create FastAPI application
app = FastAPI(
    title="MATADA-AGI Cognitive API",
    description="""
    Production-ready REST API for MATADA Autonomous General Intelligence system.
    
    This API provides access to cognitive processing capabilities through MATADA nodes:
    ...
    All processing follows the MATADA format for full interpretability and governance.
    """,
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
    ...
    servers=[
        ...
        {"url": "https://api.matada-agi.example.com", "description": "Production server"}
    ]
)
...
async def process_query(
    ...
    """
    Process a cognitive query through MATADA nodes
    ...
    try:
        ...
        if request.include_nodes:
            response_data["matada_nodes"] = result.get("matada_nodes", [])
...
async def system_info(orch: CognitiveOrchestrator = Depends(get_orchestrator)):
    ...
    return SystemInfo(
        nodes=nodes,
        matada_graph_size=len(orch.matada_graph),
...
async def get_matada_graph(
    orch: CognitiveOrchestrator = Depends(get_orchestrator),
    limit: int = 100
):
    """Get the MATADA graph nodes (limited for performance)"""
    nodes = list(orch.matada_graph.values())[-limit:]
    return {
        "total_nodes": len(orch.matada_graph),
...
async def get_causal_chain(
    ...
    """Get the causal chain for a specific MATADA node"""
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    await websocket.send_json({
        ...
        "data": {
            "client_id": client_id,
            "message": "Connected to MATADA-AGI WebSocket",
                ...
                if ws_message.type == "ping":
                    ...
                    await websocket.send_json({
                        ...
                        "data": {
                            ...
                            "matada_nodes": result.get("matada_nodes", []),
                    ...
                    await websocket.send_json({
                        ...
                        "data": {
                            "nodes": nodes,
                            "matada_graph_size": len(orchestrator.matada_graph) if orchestrator else 0,
...
def run_server(
    ...
    """
    logger.info(f"Starting MATADA-AGI FastAPI server on {host}:{port}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="MATADA-AGI FastAPI Server")
