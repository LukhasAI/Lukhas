...
A production-ready interactive visualization system for MATADA cognitive nodes
...
class NodeTypeConfig:
    """Configuration for MATADA node type visualization."""
...
class LinkTypeConfig:
    """Configuration for MATADA link type visualization."""
...
class MATADAGraphViewer:
    """
    Production-ready interactive visualization system for MATADA cognitive graphs.
    ...
    def __init__(self, 
        ...
        """
        Initialize the MATADA graph viewer.
        ...
        self.node_data = {}  # Store full MATADA node data
        ...
        logger.info(f"Initialized MATADA Graph Viewer ({width}x{height})")
    
    def add_node(self, matada_node: Dict[str, Any]) -> None:
        """
        Add a MATADA node to the graph.
        
        Args:
            matada_node: Complete MATADA format node dictionary
        ...
        try:
            ...
            if not self._validate_matada_node(matada_node):
                raise ValueError("Invalid MATADA node format")
            
            node_id = matada_node['id']
            node_type = matada_node['type']
            state = matada_node['state']
            timestamps = matada_node['timestamps']
            
            # Store full node data
            self.node_data[node_id] = matada_node
            ...
            self.graph.add_node(
                ...
                label=self._generate_node_label(matada_node),
                size=self._calculate_node_size(matada_node),
                color=NodeTypeConfig.get_color(node_type),
                shape=NodeTypeConfig.get_shape(node_type)
            )
            
            # Add links from this node
            for link in matada_node.get('links', []):
    ...
    def add_nodes_batch(self, matada_nodes: List[Dict[str, Any]]) -> Tuple[int, int]:
        """
        Add multiple MATADA nodes efficiently.
        
        Args:
            matada_nodes: List of MATADA node dictionaries
    ...
    def search_nodes(self, 
        ...
        for node_id, data in self.graph.nodes(data=True):
            ...
            if has_links is not None:
                has_any_links = (self.graph.in_degree(node_id) > 0 or 
    ...
    def create_interactive_plot(self, 
                              ...
                              title: str = "MATADA Cognitive Graph") -> go.Figure:
        """
    ...
    def create_temporal_animation(self, 
                                 ...
                                 layout: str = 'force_directed',
                                 title: str = "MATADA Temporal Evolution") -> go.Figure:
        """
                    ...
                    'buttons': [
                        {
                            ...
                            'method': 'animate',
                            'args': [None, {
                                'frame': {'duration': time_step_ms, 'redraw': True},
                                'fromcurrent': True,
    ...
    def create_statistics_dashboard(self) -> go.Figure:
        ...
        try:
            ...
            fig.update_layout(
                title=dict(
                    text="MATADA Graph Statistics Dashboard",
                    x=0.5,
                    font=dict(size=20, color='#2C3E50')
                ),
                width=self.width,
    ...
    def calculate_graph_metrics(self) -> Dict[str, Union[int, float]]:
        ...
        if (cache_key in self._analysis_cache and 
            current_time - self._cache_timestamp < 300):  # 5 minute cache
    ...
    def export_graph(self, 
                    ...
                    layout_type: str = 'force_directed') -> bool:
        """
        ...
        try:
            ...
            if format.lower() == 'json':
                # Custom JSON format with full MATADA data
                export_data = {
                    'graph': nx.node_link_data(export_graph),
                    'matada_nodes': self.node_data,
                    'metadata': {
    ...
    def import_graph(self, filepath: Union[str, Path], format: str = 'json') -> bool:
        ...
        try:
            ...
            if format.lower() == 'json':
                ...
                # Import MATADA node data if available
                if 'matada_nodes' in data:
                    self.node_data = data['matada_nodes']
    ...
    def _validate_matada_node(self, node: Dict[str, Any]) -> bool:
        """Validate MATADA node format."""
    ...
    def _generate_node_label(self, matada_node: Dict[str, Any]) -> str:
        """Generate a display label for a node."""
        node_type = matada_node['type']
        node_id = matada_node['id'][:8]
        confidence = matada_node['state'].get('confidence', 0)
        
        return f"{node_type}\n{node_id}...\nConf: {confidence:.2f}"
    
    def _calculate_node_size(self, matada_node: Dict[str, Any]) -> float:
        """Calculate node size based on importance."""
        base_size = 20
        
        # Size based on confidence and salience
        confidence = matada_node['state'].get('confidence', 0.5)
        salience = matada_node['state'].get('salience', 0.5)
    ...
    def _compute_layout(self, layout_type: str) -> Dict[str, Tuple[float, float]]:
        ...
        try:
            if layout_type == 'force_directed':
                ...
                pos = nx.nx_agraph.graphviz_layout(self.graph, prog='dot') if 'nx_agraph' in dir(nx) else nx.spring_layout(self.graph)
    ...
    def _create_empty_plot(self, title: str) -> go.Figure:
        ...
        fig.add_annotation(
            text="No data to display<br>Add MATADA nodes to visualize the graph",
            xref="paper", yref="paper",
            x=0.5, y=0.5,
...
if __name__ == "__main__":
    ...
    # Example MATADA nodes for testing
    example_nodes = [
        {
            "version": 1,
            "id": "node_001",
    ...
    print("MATADA Graph Visualization System Demo")
    print("=" * 50)
    
    # Create viewer
    viewer = MATADAGraphViewer(width=1000, height=700)
