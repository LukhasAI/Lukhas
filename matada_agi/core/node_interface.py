#!/usr/bin/env python3
"""
MATADA Cognitive Node Interface

This module defines the base interface that all MATADA cognitive nodes must implement.
Every node MUST emit MATADA format nodes for complete interpretability and governance.

The MATADA format ensures:
...
@dataclass
class NodeState:
    """Standard state structure for MATADA nodes"""
...
@dataclass
class NodeLink:
    """Connection between MATADA nodes"""
    target_node_id: str
...
class CognitiveNode(ABC):
    """
    Abstract base class for all MATADA cognitive nodes.
    
    Every cognitive node must:
    1. Process input deterministically
    2. Emit complete MATADA format nodes
    3. Support interpretability through tracing
    4. Validate its own outputs
    
    The MATADA format ensures complete auditability and governance.
    """
    ...
    def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Process input data and return result with MATADA node.
        
        This method MUST:
        1. Process the input deterministically
        2. Create a complete MATADA format node
        3. Return both the result and the MATADA node
    ...
    def get_trace(self) -> List[Dict]:
        ...
        Returns:
            List of all MATADA nodes created by this processor
        """
        return self.processing_history.copy()
    
    def create_matada_node(
        self,
        node_type: str,
        ...
        """
        Create a complete MATADA format node.
        
        This is a helper method that ensures all nodes conform to the MATADA schema.
        
        Args:
            node_type: Type from allowed MATADA types
        ...
        self.processing_history.append(matada_node)
        
        return matada_node
    
    def validate_matada_node(self, node: Dict[str, Any]) -> bool:
        """
        Validate that a node conforms to MATADA schema.
    ...
    def create_link(
        ...
        """
        Create a link to another MATADA node.
