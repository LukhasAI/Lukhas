...
This document describes the base `CognitiveNode` interface that all MATADA nodes must implement for complete interpretability and governance.

## Overview

The MATADA (Memory, Attention, Thought, Action, Decision, Awareness) cognitive architecture ensures complete traceability of all cognitive processes through standardized node formats. Every cognitive operation must emit MATADA format nodes that can be audited, traced, and governed.
...
```python
from matada_agi.core import CognitiveNode, NodeState
...
```python
from matada_agi.core import CognitiveOrchestrator
