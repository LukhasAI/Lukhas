...
try:
    from .node_interface import CognitiveNode, NodeState, NodeReflection
except ImportError:
    # For direct execution
    import sys
    from pathlib import Path
    sys.path.insert(0, str(Path(__file__).parent.parent.parent))
    from matada_agi.core.node_interface import CognitiveNode, NodeState, NodeReflection
...
class MathReasoningNode(CognitiveNode):
    ...
    2. Creating complete MATADA format nodes
    ...
    def process(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Process mathematical expressions and return results with MATADA node.
        
        Args:
            input_data: Dict with 'query' containing mathematical expression
            
        Returns:
            Dict with answer, confidence, matada_node, and processing_time
        """
    ...
    def validate_output(self, output: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not self.validate_matada_node(output['matada_node']):
                return False
            ...
            result = matada_node['state'].get('result')
...
if __name__ == "__main__":
    ...
    print("MATADA Math Reasoning Node Test")
    print("=" * 40)
    ...
    for i, query in enumerate(test_queries, 1):
        ...
        # Show MATADA node structure
        matada_node = result['matada_node']
        print(f"MATADA Node ID: {matada_node['id']}")
        print(f"Node Type: {matada_node['type']}")
        print(f"State: confidence={matada_node['state']['confidence']:.2f}, "
              f"salience={matada_node['state']['salience']:.2f}")
