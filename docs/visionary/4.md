...
Epic—let’s wire “Dreams” end-to-end without breaking the clean layering you just set up. Below are drop-in files you can paste into your repo:
	•	a tiny compatibility bridge that forwards any legacy dream_bridge_adapter calls to the new consciousness.simulation.api
	•	a Memory Inbox writer that persists dream_shard artifacts behind a capability token (defaults to in-memory; optional FS sink via env flag)
	•	optional canary tests to keep CI honest

I’ve kept everything dependency-light, deterministic, and zero side-effects by default.

⸻

1) Compatibility Bridge (legacy → new API)
...
The inbox is write-only and requires memory.inbox.dreams.write. This keeps “Dreams” decoupled from adapters and ensures only explicitly authorized flows can persist artifacts.
...
def test_bridge_persist_denied_without_scope():
    ...
    try:
        ...
        # depending on your preference, you can let it bubble—here we allow bubble in test
...
def test_writer_redacts_seed_and_writes_inmemory():
    ...
    result = {"trace_id": "LT-abc12345", "shards": [{"proposal": {"ul_tags": ["ΛSIM"]}, "risks": [], "scores": {}}]}
    before = len(_debug_inmemory_dump())
...
5) CI/Config quick notes
	•	Keep SIMULATION_ENABLED=false by default in prod; enable in CI for tests:
	•	e.g. in CI before tests: export SIMULATION_ENABLED=true
	•	(Optional) Enable FS sink locally:
	•	export MEMORY_INBOX_FS_ENABLED=true
	•	export MEMORY_INBOX_ROOT=var/memory_inbox
	•	No adapters are imported from consciousness/simulation/* — tests enforce this.
