...
2. Generate **E402 batch 1**:
   ...
   awk -F: '/ E402 /{print $1}' "$SWEEP/ruff.out" | sort -u | head -20 > /tmp/e402_batch1.txt
   ```
3. Open E402 batch 1 PR using the prepared template (see checklists)
...
- **RUF100 Hidden Risk:** Removing `# noqa` can expose latent F401 issues → skip until F401 cleanup done.
- **Batch Size Discipline:** ≤20 files per E402 PR keeps review fast.
- **Dry-Run First:** Colony planner generates CSV + commands before any moves.
- **Monitoring Early:** Deploy health audit + Grafana before major refactors.

---

## 🔗 Reference Links

- PR Review Checklists: see Phase-B templates (safe auto-fix & E402 batch)
