    ...
    def route_task(self, task_type: TaskType, prompt: str, priority: str = "balanced") -> dict:
        ...
        if priority == "cost":
            ...
            best_service_id, best_service = max(
                suitable_services, key=lambda x: speed_ranking.get(x[1].response_time, 1)
            )
...
def get_ai_router_status() -> dict:
    ...
    most_expensive_service = (
        max(available_service_costs.keys(), key=lambda k: available_service_costs[k])
        if available_service_costs
        else "none"
    )
