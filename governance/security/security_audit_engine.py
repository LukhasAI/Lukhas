            ...
            for pattern in patterns:
                ...
                for match in matches:
                    ...
                    finding = SecurityFinding(
                        ...
                        level=SecurityLevel.HIGH if secret_type in ['private_key', 'aws_key'] else SecurityLevel.MEDIUM,
            ...
            for line in content.split('\n'):
                ...
                if package.lower() in self.known_vulnerabilities:
                    ...
                    if version in vulnerable_versions:
                        finding = SecurityFinding(
                            id=f"dep_{hashlib.md5(f'{package}:{version}'.encode()).hexdigest()[:8]}",
            ...
            for vuln_type, patterns in self.dangerous_patterns.items():
                for pattern in patterns:
                    ...
                    for match in matches:
                        ...
                        finding = SecurityFinding(
                            id=f"code_{hashlib.md5(f'{file_path}:{line_num}:{vuln_type}'.encode()).hexdigest()[:8]}",
                            type=VulnerabilityType.CODE_INJECTION if 'injection' in vuln_type else VulnerabilityType.CRYPTO_WEAKNESS,
                            level=SecurityLevel.HIGH,
            ...
            for pattern_type, patterns in self.audit_patterns.items():
                for pattern in patterns:
                    ...
                    for match in matches:
                        ...
                        findings.append(SecurityFinding(
                            id=f"adapter_{pattern_type}_{hashlib.md5(f'{adapter_path}:{line_num}'.encode()).hexdigest()[:8]}",
                            type=VulnerabilityType.POLICY_VIOLATION,
                            level=SecurityLevel.HIGH if pattern_type in ['dangerous_operations', 'privilege_escalation'] else SecurityLevel.MEDIUM,
...
class SecurityAuditEngine:
    ...
    async def _scan_for_secrets(self) -> List[SecurityFinding]:
        ...
        scan_extensions = {'.py', '.js', '.ts', '.json', '.yaml', '.yml', '.env', '.config'}
            ...
            for file in files:
                ...
                if file_path.suffix.lower() in scan_extensions:
                    try:
                        ...
                        file_findings = self.secret_detector.scan_content(content, str(file_path))
    ...
    async def _scan_dependencies(self) -> List[SecurityFinding]:
        ...
        for req_file in requirements_files:
            ...
            if req_path.exists():
                file_findings = await self.dependency_scanner.scan_requirements(str(req_path))
                findings.extend(file_findings)
    ...
    async def _audit_access_controls(self) -> List[SecurityFinding]:
        ...
        identity_configs = list(self.project_root.rglob('*identity*.yaml')) + \
        ...
        for config_file in identity_configs:
            try:
                ...
                self.logger.warning(f"Could not audit access control file {config_file}: {e}")
    ...
    async def _audit_adapter_security(self) -> List[SecurityFinding]:
        ...
        for adapter_file in adapter_files:
            ...
            adapter_findings = await self.adapter_auditor.audit_adapter_security(str(adapter_file))
            findings.extend(adapter_findings)
    ...
    async def _check_guardian_status(self) -> Dict[str, Any]:
        ...
        if not self.guardian_system:
            return {'status': 'unavailable', 'reason': 'Guardian System not initialized'}
    ...
    def _check_compliance_status(self, findings: List[SecurityFinding]) -> Dict[str, bool]:
