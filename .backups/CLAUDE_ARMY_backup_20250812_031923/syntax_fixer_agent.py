...
class SyntaxFixerAgent:
    ...
    def find_syntax_errors(self) -> List[Tuple[Path, int, str]]:
        ...
        for p in Path('.').rglob('*.py'):
            if any(skip in str(p) for skip in ['.git', '__pycache__', '.pwm_cleanup_archive', '.agent_results']):
                continue
            try:
    ...
    def fix_file(self, file_path: Path, line_no: int, error_msg: str) -> bool:
        ...
        try:
            ...
            # Fix 2: Unmatched parenthesis
                ...
                # Count parentheses
                open_count = error_line.count('(') + error_line.count('[') + error_line.count('{')
                close_count = error_line.count(')') + error_line.count(']') + error_line.count('}')
            ...
            # Fix 3: Invalid syntax - missing colon
                ...
                if re.match(r'^\s*(def|class|if|for|while|elif|else|try|except|finally|with)\s+.*[^:]$', error_line):
                    lines[line_no - 1] = error_line + ':'
                    fixed = True
                # Check for malformed function definition
                    ...
                    lines[line_no - 1] = re.sub(r'def\s+(\w+)\(:$', r'def \1(self):', error_line)
                    if lines[line_no - 1] == error_line:  # If no change, try without self
                        lines[line_no - 1] = re.sub(r'def\s+(\w+)\(:$', r'def \1():', error_line)
            ...
            if fixed:
                ...
                try:
                    ...
                    self.failed_count += 1
                    self.results.append(f"❌ Failed to fix {file_path}:{line_no} - {error_msg}")
                    return False
            else:
                self.failed_count += 1
                self.results.append(f"⚠️ No fix available for {file_path}:{line_no} - {error_msg}")
                return False
