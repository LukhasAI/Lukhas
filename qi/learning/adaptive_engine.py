...
LEARN_DIR = os.path.join(STATE, "learning"); _ORIG_MAKEDIRS(LEARN_DIR, exist_ok=True)
    ...
    def evolve_node_parameters(self, *, target_file: str, tasks_focus: Optional[List[str]] = None) -> List[CandidateConfig]:
        ...
        for t,_ in low:
            for delta in ( -0.05, -0.02, 0.02, 0.05 ):
                ...
                cands.append(CandidateConfig(id=cid, base_path=target_file, patch=patch, meta={"task": t, "kind":"param_shift","delta":delta}))
    ...
    def discover_new_node_combinations(self, *, target_file: str, tasks_focus: Optional[List[str]] = None) -> List[CandidateConfig]:
        ...
        for t,_ in low:
            for tool in ("reranker", "web_search", "policy_explain"):
                ...
                cands.append(CandidateConfig(id=cid, base_path=target_file, patch=patch, meta={"task":t,"kind":"tool_enable","tool":tool}))
        self._queue_candidates(cands); return cands
    ...
    def propose_best(self, *, config_targets: List[str]) -> List[str]:
        ...
        for cand in top:
            ...
            for tgt in config_targets:
                ...
                prop = ChangeProposal(
                    id=pid, ts=_now(), author="system:adaptive", risk="medium", ttl_sec=3600,
