...
PDIR = os.path.join(STATE, "autonomy", "proposals"); _ORIG_MAKEDIRS(PDIR, exist_ok=True)
ADIR = os.path.join(STATE, "audit"); _ORIG_MAKEDIRS(ADIR, exist_ok=True)
...
def _sha(obj: Any) -> str: return hashlib.sha256(json.dumps(obj, sort_keys=True).encode()).hexdigest()
...
def _attest(steps: List[Dict[str, Any]]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        return {"chain_path": at.chain_path, "signature_b64": at.signature_b64, "public_key_b64": at.public_key_b64, "root_hash": at.root_hash}
    except Exception:
        return None
...
def plan_proposals(signals: Dict[str, Any], *, config_targets: List[str]) -> List[ChangeProposal]:
    ...
    if mean < 0.85 or fails > 0:
        ...
        try:
            ...
            try:
                import yaml; loaded = yaml.safe_load(old) or {}
                # naive merge
        ...
        # attestation
        prop.attestation = _attest([{"phase":"plan","signals":signals},{"phase":"patch","target":target,"patch":patch}])
        props.append(prop)
...
def list_proposals() -> List[Dict[str, Any]]:
    ...
    for fn in sorted(os.listdir(QDIR)):
        ...
        try: out.append(_read_json(os.path.join(QDIR, fn)))
...
def apply(proposal_id: str, subject_user: str = "system") -> Dict[str, Any]:
    ...
    if _HAS_SANDBOX:
        ...
        mgr.grant(subject=f"user:{subject_user}", caps=[f"fs:write:{os.path.abspath(target)}", f"fs:read:{os.path.abspath(target)}"], ttl_sec=600)
    ...
    if _HAS_RECEIPTS:
        try:
            rec = emit_receipt(
                ...
                started_at=p["ts"], ended_at=_now(),
                user_id=subject_user,
                jurisdiction="ops", context="config_change",
...
def main():
    ...
    args = ap.parse_args()
    if args.cmd == "observe":
        print(json.dumps(observe_signals(args.eval_dir), indent=2)); return
    if args.cmd == "plan":
        sig = observe_signals()
        props = plan_proposals(sig, config_targets=args.targets)
        print(json.dumps([asdict(p) for p in props], indent=2)); return
    if args.cmd == "list":
        print(json.dumps(list_proposals(), indent=2)); return
    if args.cmd == "approve":
        print(json.dumps(approve(args.id, args.by, args.reason), indent=2)); return
    if args.cmd == "reject":
        print(json.dumps(reject(args.id, args.by, args.reason), indent=2)); return
    if args.cmd == "apply":
        print(json.dumps(apply(args.id, args.as_user), indent=2)); return
