...
ADMIN_ALLOWED_AUDIENCES = _parse_env_list("ROUTING_ADMIN_ALLOWED_AUDIENCES", "lukhas-routing-admin")
...
async def get_admin_user(request: Request, credentials: HTTPAuthorizationCredentials = Security(security)):
...
async def validate_configuration(request: ConfigurationValidationRequest, admin_user: dict = Depends(get_admin_user)):
...
async def preview_routing(request: RoutingPreviewRequest, admin_user: dict = Depends(get_admin_user)):
    ...
    try:
        ...
        routing_context = RoutingContext(
            session_id=request.session_id, request_type=request.request_type, metadata=request.routing_hints
        )

        # Find matching rule
        rule = config_manager.get_rule_for_request(request.request_type, routing_context.metadata)
        ...
        routing_result = await routing_engine.route_request(rule, routing_context, provider_health)
...
async def simulate_routing(request: RoutingSimulationRequest, admin_user: dict = Depends(get_admin_user)):
    """Simulate routing decisions across multiple scenarios"""
    try:
        simulation_results = {"scenarios": [], "summary": {}, "timestamp": time.time()}
        ...
        for scenario_idx, scenario in enumerate(request.scenarios):
            ...
            for _ in range(request.iterations):
                ...
                rule = config_manager.get_rule_for_request(routing_context.request_type, routing_context.metadata)
            ...
            for dist in ["provider_distribution", "strategy_distribution"]:
                ...
                if total > 0:
                    scenario_results[dist] = {k: round((v / total) * 100, 2) for k, v in scenario_results[dist].items()}
...
async def trigger_health_check(request: HealthCheckRequest, admin_user: dict = Depends(get_admin_user)):
...
async def manage_circuit_breaker(
    provider: str, request: CircuitBreakerRequest, admin_user: dict = Depends(get_admin_user)
):
    ...
    try:
        ...
        if request.action == "reset":
            ...
            if provider in routing_engine.circuit_breakers:
                routing_engine.circuit_breakers[provider].state = (
                    CircuitBreakerState.OPEN
                )  # noqa: F821  # TODO: CircuitBreakerState
