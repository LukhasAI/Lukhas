...
class ObservabilityCIIntegration:
    ...
    async def post_deployment_validation(self, context: DeploymentContext) -> DeploymentReport:
        ...
        try:
            ...
            regression_analysis = await self._analyze_performance_regression(baseline_metrics, current_metrics)
            report.performance_metrics = regression_analysis

            # Check for rollback triggers
            rollback_triggers = await self._check_rollback_triggers(context.service, current_metrics, slo_results)
    ...
    async def _validate_current_slos(self, service: str) -> List[SLOValidationResult]:
        ...
        service_slos = {name: slo for name, slo in self.alert_manager.slos.items() if slo.service == service}
    ...
    async def _analyze_performance_regression(
        ...
        analysis = {"regressions_detected": False, "improvements_detected": False, "comparisons": {}}
    ...
    async def _check_rollback_triggers(
        self, service: str, current_metrics: Dict[str, Any], slo_results: List[SLOValidationResult]
    ) -> List[str]:
        ...
        service_critical_alerts = [
            a for a in active_alerts if a.slo.service == service and a.severity.value == "critical"
        ]
    ...
    def _get_recent_deployments(self, service: str, hours: int = 24) -> List[DeploymentReport]:
        ...
        return [
            ...
            if (deployment.context.service == service and deployment.context.timestamp >= cutoff_time)
        ]
    ...
    def generate_observability_report(self, context: DeploymentContext) -> Dict[str, Any]:
        ...
        report = {
            ...
            "observability_status": {
                ...
                "burn_rates": {k: v for k, v in burn_rate_status.items() if v["service"] == context.service},
                "active_alerts": [
                    {"alert_id": alert.alert_id, "severity": alert.severity.value, "message": alert.message}
    ...
    def _generate_deployment_recommendations(self, context: DeploymentContext) -> List[str]:
        ...
        # Check burn rates
        burn_rate_status = self.alert_manager.get_burn_rate_status()
        service_burn_rates = {k: v for k, v in burn_rate_status.items() if v["service"] == context.service}
        ...
        if recent_deployments:
            success_rate = len([d for d in recent_deployments if d.status == DeploymentStatus.SUCCESS]) / len(
                recent_deployments
            )
