...
class BruteForceDetector(ThreatDetector):
    ...
    def __init__(self, max_attempts: int = 5, time_window_minutes: int = 15, lockout_duration_minutes: int = 30):
...
class SecurityMonitor:
    ...
    def __init__(self, buffer_size: int = 10000, processing_threads: int = 4, guardian_integration: bool = True):
    ...
    def _start_processing(self):
        ...
        for i in range(self.processing_threads):
            thread = threading.Thread(target=self._process_events, name=f"SecurityMonitor-{i}", daemon=True)
            thread.start()
    ...
    def create_event(
        ...
        event_id = hashlib.sha256(f"{event_type.value}{source_ip}{user_id}{time.time()}".encode()).hexdigest()[:16]
...
if __name__ == "__main__":
    ...
    try:
        ...
        event = monitor.create_event(
            ...
            source_ip="10.0.0.1",
            user_id="normal_user",
            details={"resource_type": "admin_panel", "location": {"country": "RU"}},  # Unusual location
        )
        monitor.submit_event(event)
        ...
        for threat_id, threat in active_threats.items():
            print(f"  - {threat.name}: {threat.threat_level.value} " f"(confidence: {threat.confidence_score:.2f})")
