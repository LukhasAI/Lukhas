...
class WebAuthnOIDCSession:
    ...
    def is_valid(self) -> bool:
        ...
        return self.webauthn_validated and self.oidc_validated and self.guardian_approved and self.risk_score < 50.0
...
class WebAuthnOIDCIntegration:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.discovery_provider = DiscoveryProvider(self.config.get("issuer", "https://ai"), self.config)
        ...
        logger.info(
            "WebAuthn-OIDC Integration initialized",
            fail_closed=self.fail_closed,
            token_target_ms=self.token_generation_target_ms,
        )

    async def initiate_webauthn_oidc_flow(
        self, authorization_params: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            oidc_validation = await self.oidc_security.validate_authorization_request(authorization_params, context)
    ...
    async def complete_webauthn_authentication(
        self, session_id: str, webauthn_response: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            webauthn_validation = await self._validate_webauthn_response(session, webauthn_response, context)
            ...
            if session.is_valid():
                ...
                return {"status": "success", "authorization_code": authorization_code, "session_id": session_id}
    ...
    async def generate_oidc_tokens(
        self, authorization_code: str, token_request: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            token_validation = await self.oidc_security.validate_token_request(token_request, context)
            ...
            if latency_ms > self.token_generation_target_ms:
                logger.warning(
                    "Token generation latency exceeded target",
                    latency_ms=latency_ms,
    ...
    async def _validate_webauthn_response(
        self, session: WebAuthnOIDCSession, response: Dict[str, Any], context: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Validate WebAuthn authentication response"""
        # Use existing WebAuthn security hardening
        validation_result = await self.webauthn_security.validate_authentication_response(response, context)
        ...
        if validation_result.get("valid"):
            ...
            if credential_id and session.lambda_id:
                binding_valid = await self._verify_credential_binding(credential_id, session.lambda_id)
    ...
    async def _validate_with_guardian(self, session: WebAuthnOIDCSession, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"approved": risk_score < 50.0, "risk_score": risk_score, "risk_factors": risk_factors}
    ...
    async def _generate_jwt_token(self, claims: Dict[str, Any]) -> str:
        ...
        token = jwt.encode(claims, private_key, algorithm="RS256", headers={"kid": "lukhas-integration-2024"})
    ...
    async def _log_integration_event(self, event: Dict[str, Any]):
        ...
        logger.info(
            "WebAuthn-OIDC integration event",
    ...
    async def get_integration_metrics(self) -> Dict[str, Any]:
        ...
        latencies = [e.get("latency_ms", 0) for e in self.integration_events if e.get("latency_ms")]

        success_events = [e for e in self.integration_events if e.get("success") is True]
