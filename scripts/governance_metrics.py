...
def calculate_metrics(manifest: Dict) -> Dict:
    ...
    sorted([(owner, count) for owner, count in by_owner.items() if owner == "unknown"], key=lambda x: -x[1])[:5]
...
def main():
    ...
    print("=" * 80)
    print(
        f"Owner Coverage: {metrics['owner_coverage']['percentage']}% ({metrics['owner_coverage']['with_owners']}/{metrics['total_docs']})"
    )
