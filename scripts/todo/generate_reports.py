...
- **P0 tasks age**: ⚠️ Check for any P0 tasks >1 day old
...
"""
        ...
        print(f"{GREEN}✓{RESET} Weekly report generated: {report_path}")
...
|--------|-------|--------|
| Total Tasks | {total} | - |
| Active Tasks | {active} | {'⚠️ High' if active > 100 else '✅ Normal'} |
...
"""
        ...
        if completion_rate < 20:
            content += "- 🚨 **URGENT**: Very low completion rate. Review and close completed tasks.\n"
        if active > 100:
            content += "- ⚠️ **HIGH**: Too many active tasks. Consider archiving or closing stale tasks.\n"
        if completion_rate > 80:
            content += "- ✅ **EXCELLENT**: Maintain current momentum!\n"

        content += "\n---\n\n**Report generated by**: `scripts/todo/generate_reports.py`\n"

        report_path.write_text(content)
        print(f"{GREEN}✓{RESET} Health report generated: {report_path}")
