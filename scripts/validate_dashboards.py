...
class DashboardValidator:
    ...
    def __init__(self, dashboard_dir: str = "config/grafana/dashboards", grafana_url: str = "http://localhost:3000"):
    ...
    def _validate_panels(self, dashboard_name: str, panels: List[Dict]) -> bool:
        ...
        for i, panel in enumerate(panels):
            ...
            if "targets" not in panel:
                self.validation_errors.append(f"{dashboard_name} Panel '{panel_title}': Missing targets")
                valid = False
                continue
            ...
            if not targets:
                self.validation_warnings.append(f"{dashboard_name} Panel '{panel_title}': No query targets")
                continue
    ...
    def _validate_target(self, dashboard_name: str, panel_title: str, target_idx: int, target: Dict) -> bool:
        """Validate query target configuration"""
        expr = target.get("expr", target.get("query", ""))

        if not expr:
            self.validation_errors.append(f"{dashboard_name} Panel '{panel_title}' Target {target_idx}: Empty query")
            return False
        ...
        if not has_required_metric and "up" not in expr:
            self.validation_warnings.append(f"{dashboard_name} Panel '{panel_title}': Query may not use T4 metrics")
        ...
        if isinstance(datasource, dict):
            ...
            if ds_type != "prometheus":
                self.validation_warnings.append(f"{dashboard_name} Panel '{panel_title}': Non-Prometheus datasource")
    ...
    def _validate_sla_panel(self, dashboard_name: str, panel_title: str, panel: Dict) -> bool:
        ...
        if not thresholds.get("steps"):
            self.validation_warnings.append(f"{dashboard_name} Panel '{panel_title}': SLA panel missing thresholds")
            return False
        ...
        if not sla_matched:
            self.validation_warnings.append(f"{dashboard_name} Panel '{panel_title}': Thresholds don't match T4 SLAs")
    ...
    def validate_metrics_availability(self) -> bool:
        ...
        try:
            ...
            if missing_metrics:
                self.validation_warnings.append(f"Metrics not available: {', '.join(missing_metrics)}")
