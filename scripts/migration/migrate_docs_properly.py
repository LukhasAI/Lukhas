...
def find_documentation_files():
    ...
    for file_path in all_files:
        ...
        if len(file_path.parts) <= 2 or any(
            part in str(file_path) for part in ["README", "CHANGELOG", "CONTRIBUTING", "LICENSE", "SECURITY"]
        ):
            doc_files.append(file_path)
...
def categorize_file(filepath):
    ...
    if any(
        x in path_str for x in ["concept", "architecture", "theory", "framework", "design", "philosophy", "principle"]
    ):
        return "concepts"
...
def determine_metadata(filepath, category):
    ...
    # Determine facets
    facets = {"layer": ["gateway"], "domain": ["symbolic"], "audience": ["dev", "user"]}
