...
BODY1=$(jq -rn --arg p "$PATH1" --arg t "# Lukhas MCP Write Path\ncreated at $TS\n" \
  '{jsonrpc:"2.0",id:1,method:"tools/call",params:{name:"create_file",arguments:{path:$p,contents:$t}}}')
rpc "$BODY1" | jq .

echo
echo "2) APPEND A LINE"
BODY2=$(jq -rn --arg p "$PATH1" --arg t "appended at $TS\n" \
  '{jsonrpc:"2.0",id:2,method:"tools/call",params:{name:"append_file",arguments:{path:$p,contents:$t}}}')
rpc "$BODY2" | jq .

echo
echo "3) LIST DIR (filter our file)"
BODY3=$(jq -rn --arg dir "docs" '{jsonrpc:"2.0",id:3,method:"tools/call",params:{name:"list_dir",arguments:{path:$dir,glob:"TEST_MCP_WRITE*.md"}}}')
...
# First get the SHA256 of the renamed file
CURRENT_SHA=$(shasum -a 256 "$PATH2" 2>/dev/null | cut -d' ' -f1)
if [ -n "$CURRENT_SHA" ]; then
  BODY5=$(jq -rn --arg p "$PATH2" --arg sha "$CURRENT_SHA" '{jsonrpc:"2.0",id:5,method:"tools/call",params:{name:"delete_file",arguments:{path:$p,expectSha256:$sha}}}')
  rpc "$BODY5" | jq .
else
  echo "File not found for deletion (may have been cleaned up already)"
fi
...
BODY6=$(jq -rn --arg p "$COCKPIT_PATH" --arg t "$HTML" \
  '{jsonrpc:"2.0",id:6,method:"tools/call",params:{name:"write_file",arguments:{path:$p,contents:$t}}}')
rpc "$BODY6" | jq .

echo
echo "✅ SMOKE TEST COMPLETE!"
echo "Open: https://acb519bafa80.ngrok-free.app/cockpit.html"