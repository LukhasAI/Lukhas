...
class ModuleInventoryGenerator:
    ...
    def scan_directory(self, directory: Path, lane: str) -> None:
        ...
        for root, dirs, files in os.walk(directory):
            ...
            if "__init__.py" in files:
                ...
                self.stats["matriz_nodes"][matriz_node] = self.stats["matriz_nodes"].get(matriz_node, 0) + 1
    ...
    def _analyze_module(self, module_path: Path, relative_path: Path, lane: str) -> Dict[str, Any]:
        ...
        subdirs = [d for d in module_path.iterdir() if d.is_dir() and (d / "__init__.py").exists()]
        ...
        # Check for tests
        has_tests = (module_path / "tests").exists() or (module_path.parent / "tests" / module_path.name).exists()
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate complete module inventory for MATRIZ audit")
    parser.add_argument(
        "--scan",
        nargs="+",
    ...
    parser.add_argument("--base-path", type=str, default=".", help="Base repository path (default: current directory)")
