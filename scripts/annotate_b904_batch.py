#!/usr/bin/env python3
"""
Batch annotate B904 violations with T4 annotations.
Targets raise-without-from-inside-except patterns.
These need review for proper exception chaining.
"""
...
T4_ANNOTATION = """# T4: code=B904 | ticket=GH-1031 | owner=consciousness-team | status=planned
...
def get_b904_violations():
    """Get all B904 violations from T4 check."""
    ...
    return [v for v in data.get("unannotated", []) if v["code"] == "B904"]
...
def main():
    ...
    print(f"Found {len(violations)} B904 violations to annotate\n")
