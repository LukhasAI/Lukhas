...
def generate_navigation():
    ...
    if howto_dir.exists():
        for md_file in sorted(howto_dir.rglob("*.md")):
            ...
            if not any(
                file_title.lower() in item.get(next(iter(item.keys())), "").lower()
                for item in howto_nav
