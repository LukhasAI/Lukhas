...
from identity import get_current_user, AuthContext, require_t3_or_above
...
async def get_guardian_logs(
    ...
    if user.tier != "T5":
        raise HTTPException(
            status_code=403,
            detail="Guardian logs require T5 access"
        )
    ...
    try:
        ...
        return {
            ...
            "guardian_summary": {
                ...
                "average_drift": sum(log.get("drift_score", 0) for log in enriched_guardian_logs) / len(enriched_guardian_logs) if enriched_guardian_logs else 0
            },
            "guardian_glyphs": ["🛡️", "⚔️", "🔮", "⚡", "🌟"]
        }
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to fetch guardian logs: {str(e)}"
        )
        ...
        if format == "json":
            ...
            for log in enriched_logs:
                symbolic_logs.append({
                    ...
                    "time": log.get("timestamp", "").split("T")[1].split(".")[0] if "T" in log.get("timestamp", "") else ""
                })
