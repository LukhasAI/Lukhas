...
class FlexibleAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            return JSONResponse({"error": f"Invalid token: {e}"}, status_code=403)
    ...
    return JSONResponse(
        {
            ...
            "auth_methods": ["Authorization: Bearer <token>", "X-API-Key: <token>", "?token=<token>"],
        }
    )
...
async def jwks_endpoint(request):
    ...
    return JSONResponse(
        {"keys": [{"kty": "oct", "k": OAUTH_SECRET, "alg": "HS256", "use": "sig", "kid": "test-key-1"}]}
    )
...
# Available tools
TOOLS = {"list_directory": list_directory_tool, "read_file": read_file_tool}
