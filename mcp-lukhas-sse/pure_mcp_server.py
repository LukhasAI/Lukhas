...
AVAILABLE_TOOLS = [
    ...
    },
    {
        ...
        "parameters": {
            "type": "object",
            "properties": {
                "file_path": {"type": "string", "description": "Path to file (security-validated, max 1MB)"}
            },
            "required": ["file_path"],
        },
    },
...
async def constellation_health_check(arguments: dict = None) -> Dict[str, Any]:
    ...
    return {
        ...
        "integration_status": {"chatgpt_connector": "ready", "sse_transport": "active", "http_endpoints": "available"},
    }
        ...
        for item in sorted(path_obj.iterdir()):
            try:
                ...
                item_data = {
                    ...
                    "is_lukhas_component": any(
                        keyword in item.name.lower() for keyword in ["lukhas", "constellation", "consciousness", "mcp"]
                    ),
                }
    ...
    try:
        ...
        if file_size > MAX_FILE_SIZE:
            return {
                ...
                "file_info": {"size_mb": round(file_size / 1024 / 1024, 2), "limit_mb": 1.0},
            }
        ...
        lukhas_indicators = {
            ...
            "consciousness_patterns": any(
                term in content.lower() for term in ["consciousness", "cognitive", "awareness"]
            ),
            "http_endpoints": any(term in content for term in ["SSE", "Server-Sent Events", "starlette", "uvicorn"]),
...
async def call_tool(request):
    ...
    try:
        ...
        if tool_name not in TOOL_FUNCTIONS:
            return JSONResponse(
                {"error": f"Unknown tool: {tool_name}", "available_tools": list(TOOL_FUNCTIONS.keys())}, status_code=400
            )
...
async def chatgpt_openapi_spec(request):
    ...
    spec = {
        ...
        "servers": [
            {"url": "https://lukhas-mcp-production.up.railway.app", "description": "LUKHAS AI Production Server"}
        ],
        "paths": {
            "/tools/call": {
                "post": {
