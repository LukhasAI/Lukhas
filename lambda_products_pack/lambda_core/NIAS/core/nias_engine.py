...
class NIASEngine:
    ...
    async def _phase_emotional_gating_fallback(
        ...
        gating_config = self.emotional_gating_config.get(
            emotional_state,
            self.emotional_gating_config[EmotionalState.NEUTRAL.value]
        )
            ...
            if self.dast_adapter:
                ...
                if user_id:
                    try:
                        ...
                        logger.debug(f"DΛST symbolic processing for {user_id}: {len(symbolic_context.get('symbolic_tags', []))} symbols, coherence {symbolic_context.get('coherence_score', 0):.2f}")
    ...
    async def check_emotional_state(self, user_context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            ).isoformat() if not gating_config.get("allow_delivery", True) else None,
        ...
        for defer_id, deferred_data in ready_messages:
            try:
                ...
                await self.process_message(
                    deferred_data["message"],
                    deferred_data["user_context"]
                )
            except Exception as e:
