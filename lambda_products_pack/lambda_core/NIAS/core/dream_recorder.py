...
class DreamRecorder:
    ...
    async def _update_symbolic_resonance(self, seed_id: str):
        ...
        positive_interactions = sum(1 for i in interactions if i["interaction_type"] in ["view", "like", "share", "purchase"])
    ...
    async def get_brand_performance(self, brand_id: str) -> Dict[str, Any]:
        ...
        avg_resonance = sum(s.get("symbolic_resonance", 0) for s in brand_seeds if s.get("symbolic_resonance") is not None) / max(1, total_seeds)
    ...
    async def get_dream_analytics(self, user_id: Optional[str] = None, days: int = 30) -> Dict[str, Any]:
        ...
        for seed in recent_seeds:
            ...
            if brand not in brand_breakdown:
                brand_breakdown[brand] = {"count": 0, "resonance_sum": 0, "resonance_count": 0}
