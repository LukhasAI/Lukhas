...
class TierManager:
    ...
    async def register_user(self, user_id: str, tier: str = "T3", subscription_data: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        if action == "message_delivery":
            ...
            if daily_limit != "unlimited":
                ...
                if usage.get("daily_messages_today", 0) >= daily_limit:
                    return {"allowed": False, "reason": "daily_limit_exceeded", "limit": daily_limit}
            
            if monthly_limit != "unlimited":
                if usage.get("monthly_messages", 0) >= monthly_limit:
                    return {"allowed": False, "reason": "monthly_limit_exceeded", "limit": monthly_limit}
            ...
            if usage.get("api_calls_today", 0) >= api_limit:
                return {"allowed": False, "reason": "api_daily_limit_exceeded", "limit": api_limit}
    ...
    async def get_tier_analytics(self, days: int = 30) -> Dict[str, Any]:
        ...
        for tier, usage_list in usage_by_tier.items():
            if usage_list:
                ...
                avg_usage[tier] = {"avg_messages": 0, "avg_interactions": 0, "avg_feedback": 0}
