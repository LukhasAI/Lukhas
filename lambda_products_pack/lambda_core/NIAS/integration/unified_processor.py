...
class UnifiedMessageProcessor:
    ...
    async def _phase_dast_analysis(self, context: UnifiedProcessingContext) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"DΛST provided {len(symbolic_context.get('symbolic_tags', []))} symbols for {context.user_id}")
    ...
    async def _phase_abas_decision(self, context: UnifiedProcessingContext) -> Dict[str, Any]:
        ...
        try:
            ...
            if context.symbolic_context:
                ...
                enhanced_context["focus_score"] = context.symbolic_context.get("context_scores", {}).get("focus_score", 0.5)
            ...
            logger.debug(f"ΛBAS decision for {context.user_id}: {attention_result.get('approved')} ({attention_result.get('emotional_state')})")
    ...
    def _finalize_processing(self, context: UnifiedProcessingContext) -> Dict[str, Any]:
        ...
        self.processing_stats["average_processing_time_ms"] = (
            (current_avg * (total_requests - 1) + context.total_processing_time_ms) / total_requests
        )
        ...
        logger.info(f"Unified processing completed: {context.session_id} - {result.get('status')} ({context.total_processing_time_ms:.1f}ms)")
        return result
    ...
    def _fallback_symbolic_context(self, context: UnifiedProcessingContext) -> Dict[str, Any]:
        ...
        return {
            ...
            "context_scores": {"focus_score": 0.5, "coherence_score": 0.5, "stability_score": 0.5},
