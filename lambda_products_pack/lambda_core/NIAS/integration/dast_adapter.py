...
try:
    from dast_core import DΛST, SymbolicTag, ContextSnapshot, SymbolCategory, SymbolSource
    DAST_AVAILABLE = True
...
class NIASDastAdapter:
    ...
    async def update_symbolic_context_from_message(
        ...
        try:
            ...
            logger.debug(f"Updated DΛST context for {user_id} with {len(symbols_to_add)} symbols from message interaction")
            return True
