...
- Make it feel like their own dream, not an external message"""
    ...
    async def _generate_image(self, visual_prompt: str) -> Optional[str]:
        ...
        try:
            ...
            logger.info(f"Generated dream image with DALL-E 3")
            return image_url
    ...
    def _calculate_emotional_profile(self, narrative: str, context: DreamContext) -> Dict[str, float]:
        ...
        mood_profiles = {
            DreamMood.NOSTALGIC: {"joy": 0.4, "calm": 0.6, "stress": 0.0, "longing": 0.7},
            DreamMood.ASPIRATIONAL: {"joy": 0.7, "calm": 0.4, "stress": 0.0, "longing": 0.5},
            DreamMood.COMFORTING: {"joy": 0.5, "calm": 0.8, "stress": 0.0, "longing": 0.2},
            DreamMood.ADVENTUROUS: {"joy": 0.8, "calm": 0.3, "stress": 0.0, "longing": 0.4},
            DreamMood.SERENE: {"joy": 0.4, "calm": 0.9, "stress": 0.0, "longing": 0.1},
            DreamMood.CELEBRATORY: {"joy": 0.9, "calm": 0.4, "stress": 0.0, "longing": 0.2},
            DreamMood.WHIMSICAL: {"joy": 0.7, "calm": 0.5, "stress": 0.0, "longing": 0.3}
        }
    ...
    async def _validate_ethics(self, narrative: str, visual_prompt: str, context: DreamContext) -> float:
        ...
        pressure_words = ["hurry", "limited", "act now", "don't miss", "last chance", "expires soon"]
