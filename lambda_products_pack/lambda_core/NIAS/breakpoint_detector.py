...
class NaturalBreakpointDetector:
    ...
    def __init__(self):
        ...
        self.completion_indicators = [
            "submit", "save", "done", "complete", "finish",
            "send", "post", "publish", "confirm", "success"
        ]
    ...
    def check_breakpoint(self) -> Tuple[bool, Optional[BreakpointType], Dict[str, Any]]:
        ...
        if self._check_user_permission():
            return True, BreakpointType.PERMISSION_GRANTED, {
                "confidence": 1.0,
                "user_initiated": True
            }
        ...
        if task_completed:
            return True, BreakpointType.TASK_COMPLETION, {
                "confidence": 0.95,
                "task_data": task_data,
        ...
        if is_paused:
            return True, BreakpointType.NATURAL_PAUSE, {
                "confidence": 0.85,
        ...
        if at_boundary:
            return True, BreakpointType.CONTENT_BOUNDARY, {
                "confidence": 0.8,
        ...
        if is_idle and self.user_permissions.get("during_idle", True):
            return True, BreakpointType.IDLE_THRESHOLD, {
                "confidence": 0.7,
        ...
        if self._check_transition_state():
            return True, BreakpointType.BETWEEN_SECTIONS, {
                "confidence": 0.75,
                "natural_break": True
            }
    ...
    def record_ad_display(
        ...
        if user_response == "dismissed_quickly":
            ...
            self.current_workflow.interruption_score = max(0, 
                self.current_workflow.interruption_score - 0.1)
    ...
    def _is_task_completion(self, action_type: str, context: Dict) -> bool:
        ...
        if context:
            ...
            if context.get("completed") == True:
                return True
            if "result" in context and context["result"] in ["success", "done", "complete"]:
                return True
        
        # Check for form submission patterns
        if action_type == "submit" and self.current_workflow.state == WorkflowState.TYPING:
            return True
    ...
    def _analyze_best_times(self) -> List[str]:
        ...
        successful_displays = [
            record for record in self.breakpoint_history
    ...
    def _generate_timing_suggestions(self) -> List[str]:
        ...
        if len(self.breakpoint_history) > 10:
            success_rate = sum(1 for r in self.breakpoint_history[-10:] 
                             if r.get("user_response") == "engaged") / 10
            if success_rate < 0.3:
