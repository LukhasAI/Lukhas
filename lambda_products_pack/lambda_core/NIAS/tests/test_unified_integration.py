...
async def test_unified_integration():
    ...
    print(f"📊 Processing Stats: {health['processing_stats']['total_requests']} total requests")
    ...
    for i, scenario in enumerate(test_scenarios, 1):
        ...
        try:
            ...
            print(f"   🔄 Unified Processing: {result.get('unified_processing', False)}")
            ...
            if symbolic_context:
                print(f"   🎨 Symbolic Context:")
                print(f"     - Primary Activity: {symbolic_context.get('primary_activity', 'N/A')}")
            ...
            if attention_decision:
                print(f"   🧠 Attention Decision:")
                print(f"     - Approved: {attention_decision.get('approved', False)}")
                print(f"     - Emotional State: {attention_decision.get('emotional_state', 'N/A')}")
                print(f"     - Attention State: {attention_decision.get('attention_state', 'N/A')}")
                print(f"     - Confidence: {attention_decision.get('confidence', 0):.2f}")
                print(f"     - ΛBAS Integration: {attention_decision.get('abas_integration', False)}")
                ...
                if not attention_decision.get('approved'):
                    ...
                    if attention_decision.get('defer_until'):
                        print(f"     - Defer Until: {attention_decision.get('defer_until')}")
            ...
            if result.get('status') == 'delivered':
                ...
                if widget_config:
                    print(f"   🎛️ Widget Generated: {widget_config.get('type', 'unknown')}")
    ...
    print(f"   Active Integrations: {metrics['integrations_active']}/{metrics['total_integrations']}")
...
async def test_integration_failure_scenarios():
    ...
    user_context = {
        "user_id": "test_failure_scenarios",
        "tier": "T2"
    }
    ...
    try:
        ...
        print(f"   Fallback Mechanisms: {'activated' if result.get('unified_processing') else 'inactive'}")
