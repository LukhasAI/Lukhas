...
async def test_dast_integration():
        ...
        try:
            ...
            if session:
                ...
                if 'symbolic_processing' in phases:
                    ...
                    print(f"   DΛST Integration: {symbolic_phase.get('dast_integration', False)}")
                    print(f"   Symbolic Tags: {symbolic_phase.get('symbolic_tags_count', 0)}")
                    print(f"   Primary Activity: {symbolic_phase.get('primary_activity', 'N/A')}")
                    print(f"   Coherence Score: {symbolic_phase.get('coherence_score', 0):.2f}")
                    print(f"   Message Coherence: {symbolic_phase.get('message_coherence', 0):.2f}")
                    if symbolic_phase.get('lambda_fingerprint'):
                        print(f"   Λ Fingerprint: {symbolic_phase.get('lambda_fingerprint')}")
                
                # Check delivery phase for DAST context updates
                if 'delivery' in phases:
                    delivery_phase = phases['delivery']
                    print(f"   DΛST Context Updated: {delivery_phase.get('dast_context_updated', False)}")
            ...
            if 'message' in result:
                ...
                if symbolic_processing.get('dast_integration'):
                    print(f"   Symbolic Enhancement:")
                    print(f"     - Colors: {len(symbolic_processing.get('recommended_colors', []))} colors")
                    print(f"     - Elements: {len(symbolic_processing.get('recommended_symbols', []))} elements")
                    print(f"     - Tone: {symbolic_processing.get('recommended_tone', 'neutral')}")
                    print(f"     - Symbolic Tags: {symbolic_processing.get('symbolic_tags', [])}")
    ...
    if hasattr(nias_engine, 'dast_adapter') and nias_engine.dast_adapter:
        ...
        try:
            ...
            print(f"   Symbolic Context Retrieved:")
            print(f"   - Primary Activity: {symbolic_context.get('primary_activity')}")
            print(f"   - Active Symbols: {len(symbolic_context.get('symbolic_tags', []))}")
            print(f"   - Focus Score: {symbolic_context.get('context_scores', {}).get('focus_score', 0):.2f}")
            print(f"   - Coherence Score: {symbolic_context.get('coherence_score', 0):.2f}")
            print(f"   - Recommended Tone: {symbolic_context.get('recommended_tone')}")
            print(f"   - DΛST Integration: {symbolic_context.get('dast_integration', False)}")
            ...
            if suggestions:
                ...
                for suggestion in suggestions[:2]:  # Show first 2
                    print(f"   - {suggestion.get('activity')}: {suggestion.get('reason')}")
    ...
    if hasattr(nias_engine, 'abas_adapter') and nias_engine.abas_adapter:
        ...
        print(f"   ΛBAS: {abas_status.get('integration_mode', 'unknown')} mode, {abas_status.get('registered_users', 0)} users")
    
    if hasattr(nias_engine, 'dast_adapter') and nias_engine.dast_adapter:
        dast_status = nias_engine.dast_adapter.get_integration_status()
        print(f"   DΛST: {dast_status.get('integration_mode', 'unknown')} mode, {dast_status.get('registered_users', 0)} users")
...
async def test_dast_symbolic_coherence():
    ...
    for context_type, message_type in coherence_tests:
        try:
            ...
            print(f"   {context_type.capitalize()} + {message_type}: coherence {coherence:.2f}")
