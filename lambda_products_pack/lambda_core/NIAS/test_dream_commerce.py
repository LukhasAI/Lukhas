...
from lambda_products_pack.lambda_core.NIAS.consent_manager import (
    ConsentManager, ConsentLevel, ConsentScope, DataSource, AIGenerationType
)
from lambda_products_pack.lambda_core.NIAS.user_data_integrator import (
    UserDataIntegrator, UserDataProfile
)
from lambda_products_pack.lambda_core.NIAS.vendor_portal import (
    VendorPortal, VendorTier, DreamSeedType
)
        ...
        return [
            {
                ...
                "data": {
                    ...
                    "shopping_cart": [
                        {"item": "Cozy Cashmere Sweater", "price": 189.99, "added": "2 days ago"},
                        {"item": "Winter Boots", "price": 220.00, "added": "5 days ago"}
                    ],
                    "calendar_events": [
                        {"event": "Emma's Birthday Party", "date": "2024-12-15", "type": "celebration"},
                        {"event": "Holiday Shopping", "date": "2024-12-20", "type": "reminder"}
                    ],
                    "browsing_history": [
            ...
            
            {
                ...
                "data": {
                    ...
                    "calendar_events": [
                        {"event": "Doctor appointment", "date": "2024-12-10", "type": "medical"},
                        {"event": "Loan payment due", "date": "2024-12-12", "type": "financial"}
                    ],
                    "browsing_history": [
                        "debt consolidation",
            ...
            
            {
                ...
                "data": {
                    ...
                    "calendar_events": [
                        {"event": "Soccer practice", "date": "2024-12-11", "type": "sports"},
            ...
            
            {
                ...
                "data": {
                    ...
                    "shopping_cart": [
                        {"item": "Blood Pressure Monitor", "price": 45.99, "added": "3 days ago"},
                        {"item": "Reading Glasses", "price": 32.00, "added": "1 week ago"}
                    ],
                    "calendar_events": [
                        {"event": "Doctor checkup", "date": "2024-12-14", "type": "medical"},
                        {"event": "Pharmacy pickup", "date": "2024-12-16", "type": "medical"}
                    ],
                    "browsing_history": [
                        "medicare plans",
                        "senior discounts",
            ...
            
            {
                ...
                "data": {
                    ...
                    "shopping_cart": [],
                    "calendar_events": [
                        {"event": "Gamblers Anonymous", "date": "2024-12-12", "type": "recovery"}
                    ],
                    "browsing_history": [
                        "online poker",
                        "sports betting odds",
...
class NIASDreamCommerceTest:
    ...
    def __init__(self):
        ...
        if not self.openai_available:
            ...
            logger.info("💡 To use real OpenAI generation, set OPENAI_API_KEY environment variable")
    ...
    async def test_consent_management(self):
        ...
        profile = await self.consent_manager.get_comprehensive_consent_profile(test_user)
        
        print(f"\n📊 Consent Profile Summary:")
        print(f"   - Data sources granted: {sum(1 for v in profile['data_sources'].values() if v)}")
        print(f"   - AI generation enabled: {profile['ai_generation'].get('openai_api_enabled', False)}")
    ...
    async def test_ethical_blocks(self):
        ...
        for profile in user_profiles:
            ...
            print(f"   Emotional State: Stress={profile['emotional_state']['stress']:.1f}")
            ...
            should_block = "BLOCK" in profile["expected_result"] or "CAUTION" in profile["expected_result"]
            was_blocked = result["status"] in ["deferred", "consent_required", "blocked"]
            ...
            if should_block and was_blocked:
                ...
                print(f"   ❌ Incorrect: Should {'block' if should_block else 'allow'}, but {'blocked' if was_blocked else 'allowed'}")
                if not should_block and was_blocked:
                    self.test_results["allowed_incorrectly"] += 1
            
            self.log_test(
                f"Ethical check for {profile['name']}",
                was_blocked,
                should_block
            )
    ...
    async def test_full_commerce_flow(self):
        ...
        if "vendor_id" in vendor_result:
            ...
            if "seed_id" in seed_result:
                ...
                init_result = await self.orchestrator.initiate_dream_commerce(test_user_id)
                ...
                if init_result["status"] == "initiated":
                    ...
                    print(f"   Click tracked: {interaction_result.get('status', 'unknown')}")
                    
                    if interaction_result.get("affiliate_link"):
                        print(f"   Affiliate link: {interaction_result['affiliate_link'][:50]}...")
                        print(f"   One-click ready: {interaction_result.get('one_click_ready', False)}")
                    
                    # 9. Get system metrics
                    metrics = self.orchestrator.get_system_metrics()
                    print(f"\n4️⃣ System Metrics:")
                    print(f"   Active sessions: {metrics['active_sessions']}")
                    print(f"   Dreams delivered: {metrics['metrics']['dreams_delivered']}")
                    print(f"   Ethical blocks: {metrics['metrics']['ethical_blocks']}")
                    
                    self.log_test("End-to-end flow", True, True)
                else:
                    print(f"   ⚠️  Could not initiate: {init_result.get('reason', 'unknown')}")
        ...
        if self.test_results['failed'] > 0:
            ...
            for detail in self.test_results['details']:
                if detail['status'] == "❌ FAIL":
                    print(f"   - {detail['test']}")
                    print(f"     Expected: {detail['expected']}, Got: {detail['result']}")
