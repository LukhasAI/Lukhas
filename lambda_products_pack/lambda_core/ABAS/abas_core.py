...
class ΛBAS:
    ...
    async def register_user(self, user_id: str, 
                           initial_state: AttentionState = AttentionState.AVAILABLE) -> bool:
    ...
    async def update_attention_metrics(self, user_id: str, 
                                     metrics: AttentionMetrics) -> bool:
        ...
        try:
            ...
            self.attention_history[user_id] = [
                (timestamp, metric) for timestamp, metric in self.attention_history[user_id]
    ...
    async def _determine_attention_state(self, user_id: str, 
                                       metrics: AttentionMetrics) -> AttentionState:
        """Determine attention state from metrics"""
        
        # Flow state detection
        if (metrics.focus_level >= 0.8 and 
            metrics.cognitive_load >= 0.6 and 
            metrics.cognitive_load <= 0.8 and
            metrics.attention_residue < 0.2):
            return AttentionState.FLOW_STATE
        
        # Overload detection
        if (metrics.cognitive_load >= 0.85 or
            metrics.attention_residue >= 0.7 or
            metrics.multitask_penalty >= 0.5):
            return AttentionState.OVERLOADED
        
        # Recovery state
        if (metrics.cognitive_load <= 0.3 and 
            metrics.attention_residue >= 0.4):
            return AttentionState.RECOVERING
        
        # Focused state
        if (metrics.focus_level >= 0.7 and 
            metrics.cognitive_load >= 0.5 and
    ...
    async def request_attention(self, user_id: str, 
                              request: AttentionRequest) -> AttentionDecision:
        """
    ...
    async def _check_boundary_violations(self, user_id: str, 
                                       request: AttentionRequest) -> Dict[str, Any]:
        ...
        for boundary in active_boundaries:
            ...
            if boundary.type == BoundaryType.COGNITIVE:
                ...
                if (current_metrics.flow_probability >= boundary.threshold and
                    request.interruptibility < 0.5):
                    violation = True
    ...
    async def _is_boundary_active(self, boundary: AttentionBoundary, 
                                 metrics: AttentionMetrics) -> bool:
        """Check if boundary is currently active"""
    ...
    async def _check_temporal_boundary(self, boundary: AttentionBoundary,
                                     request: AttentionRequest) -> bool:
    ...
    async def _evaluate_request_for_state(self, request: AttentionRequest,
                                        state: AttentionState,
                                        metrics: AttentionMetrics) -> Dict[str, Any]:
        ...
        if state == AttentionState.FLOW_STATE:
            ...
            if request.urgency >= 0.9:
                ...
                return {
                    ...
                    "defer_minutes": 25,
                    "alternatives": ["Schedule for next break", "Convert to ambient notification"]
                }
            else:
                return {
                    "decision": "block",
            ...
            if request.cognitive_cost >= 0.5:
                ...
                return {
                    ...
                    "transformations": ["Reduce cognitive complexity", "Provide summary only"]
                }
    ...
    async def _adjust_boundaries_for_state_change(self, user_id: str,
...
if __name__ == "__main__":
    async def demo():
        ...
        print(f"   Integration Mode: ΛBAS Standalone")
