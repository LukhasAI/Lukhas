Absolutely—let’s add the admin-only inbox reader and clarify the deprecation plan so you keep momentum without breaking callers.
...
def search(
    *,
    ...
    """
    Lightweight search across all artifacts (admin). FS can be large—use filters.
    """
    _require_scope(cap_token, READ_SCOPE)
...
def test_admin_roundtrip_inmemory():
    tok = admin_token()
    seed = {"goal": "Greet ΛID users", "context": {}, "constraints": {"budgets": {}, "flags": {}}}
    res  = {"trace_id": "LT-test-1", "shards": [{"proposal": {"ul_tags": ["ΛSIM"]}, "risks": [], "scores": {}}]}
    ...
    q = search(cap_token=tok, ul_tag="ΛSIM", goal_contains="ΛID")
    assert len(q) >= 1

    deleted = purge_trace("LT-test-1", cap_token=tok)
    assert deleted >= 1

🔐 Posture: Reader is admin-only, gated by memory.inbox.dreams.read_admin. Purge requires memory.inbox.dreams.purge_admin. This keeps the inbox write-mostly while giving ops a safe window for debugging or data subject requests.

⸻

2) Tiny fix in the writer (typo)

In the writer you pasted earlier, rename _REDAC_KEYS → _REDACT_KEYS inside _redact_seed():
...
🧭 Deprecation plan (answering your question)

“Are we deprecating the API files we have now?”

Short answer:
	•	Do not deprecate the new canonical lane: consciousness.simulation.api (keep as the source of truth).
	•	Keep legacy entry points (like any prior dream_bridge_adapter or consciousness/dreams/api.py) as shims for now, with warnings + metrics, and set a removal date only after callers are migrated.

Why: You want stability for existing call-sites while you finish consolidation. The bridge we added already forwards to the canonical API; that’s the right pattern.

Recommended policy
	1.	Canonical API (stay)
	•	Keep: consciousness/simulation/api.py
	•	Import direction: adapters → simulation, never the reverse (enforced by import-linter).
	2.	Legacy APIs (graceful sunset)
If you still have older files (examples):
	•	consciousness/dreams/api.py
	•	bridge/dreams.py or other historical adapters
...
from consciousness.simulation.api import *  # noqa
	...
	3.	CI visibility
	•	In your CI test step, treat DeprecationWarning as error for new usages only (or log a budget):

PYTHONWARNINGS=default::DeprecationWarning pytest -q


	•	Or add a warning budget via pytest-warning-control plugin (e.g., max 0 new).
...
forbidden_modules = adapters
	...
	5.	Removal date
	•	Publish in DEPRECATIONS.md with a clear date (e.g., 2025-11-01), and auto-emit a log line at import so you can find lingering dependencies.

⸻

🔧 Optional: tiny admin CLI (nice to have)
...
🧪 Quick sanity checklist
	•	SIMULATION_ENABLED=true in CI for tests that exercise the lane.
	•	MEMORY_INBOX_FS_ENABLED=true + MEMORY_INBOX_ROOT=var/memory_inbox if you want file output locally.
	•	Import-linter rules updated to prevent reverse imports.
	•	DEPRECATIONS.md created with the 2025-11-01 removal target for any legacy APIs you shim.
...
Admin reader capabilities:
	•	Filter/export by node.type, provenance.tenant, provenance.consent_scopes, timestamps.created_ts.
	•	CLI pattern: tools/dreams_admin_cli.py export-nodes --trace LT-... > nodes.ldjson
	•	Refuse export when consent scopes are insufficient; emit denial rationale with Λ-trace ID.
