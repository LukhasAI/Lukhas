...
from gpt_oss_brain import GPTOSSBrainSpecialist, GPTOSSModelLoader
...
class QRGAdapter:
    ...
    def _extract_reasoning_chain(self, reasoning: Dict[str, Any]) -> List[str]:
        ...
        for sentence in sentences:
            ...
            if sentence and ("therefore" in sentence.lower() or 
                           "because" in sentence.lower() or
                           "consequently" in sentence.lower()):
    ...
    def _evaluate_evidence_strength(self, reasoning: Dict[str, Any]) -> float:
        ...
        # Evidence indicators
        strong_evidence = ["research shows", "studies indicate", "data reveals", "analysis demonstrates"]
...
class NIASAdapter:
    ...
    def _calculate_intelligence_metrics(self, reasoning: Dict[str, Any]) -> Dict[str, float]:
        ...
        metrics = {
            ...
            "creative": len([i for i in insights if "creativ" in i.lower() or "innovat" in i.lower()]) / max(len(insights), 1),
...
class ABASAdapter:
    ...
    def _enhance_with_business_analysis(self, gpt_result: Dict[str, Any], request: LambdaProductRequest) -> Dict[str, Any]:
    ...
    def _analyze_business_risks(self, reasoning: Dict[str, Any]) -> List[Dict[str, str]]:
        ...
        if "market" in raw_output and "risk" in raw_output:
            risks.append({"type": "Market Risk", "level": "Medium", "description": "Market volatility potential"})
        
        if "competitive" in raw_output:
            risks.append({"type": "Competitive Risk", "level": "High", "description": "Competitive pressure"})
        
        if "operational" in raw_output:
            risks.append({"type": "Operational Risk", "level": "Low", "description": "Operational challenges"})
    ...
    def _update_average_processing_time(self, processing_time_ms: int):
        ...
        if self.metrics["average_processing_time"] == 0:
            ...
            self.metrics["average_processing_time"] = (
                0.9 * self.metrics["average_processing_time"] + 
