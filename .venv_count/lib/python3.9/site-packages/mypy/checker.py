        ...
        for base in defn.info.bases:
            ...
            if candidate and has_new:
                self.fail(
                    ...
                    f'found extra "{base.str_with_options(self.options)}"',
                    defn,
                )
            elif candidate:
                has_new = True
        ...
        if "." not in name:
            ...
            if last in n.names:
                ...
                if fullname in SUGGESTED_TEST_FIXTURES:
                    suggestion = f", e.g. add '[{parts[0]} fixtures/{SUGGESTED_TEST_FIXTURES[fullname]}]' to your test"
                else:
                    suggestion = ""
                raise KeyError(
                    f"Could not find builtin symbol '{last}' (If you are running a "
                    "test case, use a fixture that "
                    f"defines this symbol{suggestion})"
                )
            else:
