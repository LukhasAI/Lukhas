...
def generate_native_function(
    ...
    for r in all_values(fn.arg_regs, fn.blocks):
        ...
        declarations.emit_line(
            f"{ctype}{REG_PREFIX}{names[r]}{init};"
        )
...
class FunctionEmitterVisitor(OpVisitor[None]):
    ...
    def visit_goto(self, op: Goto) -> None:
        if op.label is not self.next_block:
            self.emit_line(f"goto {self.label(op.label)};")
    ...
    def visit_branch(self, op: Branch) -> None:
        ...
        if false is self.next_block:
            if op.traceback_entry is None:
                ...
                self.emit_lines(f"goto {self.label(true)};", "}")
        else:
            self.emit_line(f"if ({cond}) {{")
            self.emit_traceback(op)

            if true is not self.next_block:
                self.emit_line(f"goto {self.label(true)};")

            self.emit_lines("} else", f"    goto {self.label(false)};")

    def visit_return(self, op: Return) -> None:
        value_str = self.reg(op.value)
        self.emit_line(f"return {value_str};")
        ...
        if cl.get_method(op.attr, prefer_method=prefer_method):
            ...
            if not always_defined:
                ...
                if branch is not None:
                    if (
                        ...
                        self.emit_line(f"goto {self.label(branch.true)};")
                        merged_branch = branch
                        self.emitter.emit_line("}")
                if not merged_branch:
                    exc_class = "PyExc_AttributeError"
                    self.emitter.emit_line(
                        f'PyErr_SetString({exc_class}, "attribute {repr(op.attr)} of {repr(cl.name)} undefined");'
                    )
            ...
            if merged_branch:
                if merged_branch.false is not self.next_block:
                    self.emit_line(f"goto {self.label(merged_branch.false)};")
                self.op_index += 1
    ...
    def emit_method_call(self, dest: str, op_obj: Value, name: str, op_args: list[Value]) -> None:
        ...
        if is_direct:
            ...
            self.emit_line(
                f"{dest}CPY_GET_METHOD{version}({obj}, {self.emitter.type_struct_name(rtype.class_ir)}, {method_idx}, {rtype.struct_name(self.names)}, {mtype})({args}); /* {name} */"
            )
    ...
    def visit_raise_standard_error(self, op: RaiseStandardError) -> None:
        ...
        if op.value is not None:
            if isinstance(op.value, str):
                ...
                self.emitter.emit_line(
                    f"PyErr_SetObject(PyExc_{op.class_name}, {self.emitter.reg(op.value)});"
                )
            else:
    ...
    def visit_get_element_ptr(self, op: GetElementPtr) -> None:
        ...
        self.emit_line(
            f"{dest} = ({op.type._ctype})&(({op.src_type.name} *){src})->{op.field};"
        )
