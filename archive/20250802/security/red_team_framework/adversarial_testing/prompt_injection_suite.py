...
class PromptInjectionSuite:
    ...
    async def _generate_context_specific_injections(
        ...
        payloads = system_specific_payloads.get(
            system_type, ["Generic injection payload"]
        )
...
class ModelInversionTester:
    ...
    async def _execute_inversion_attack(
        self, model_interface: Any, technique: str
    ) -> AttackResult:
...
class AdversarialTestingSuite:
    ...
    async def conduct_comprehensive_test(
        self,
        system_id: str,
        target_system: str,
        ...
        try:
            ...
            injection_vectors = (
                await self.prompt_injection_suite.generate_injection_vectors(
                    target_system
                )
            )
            ...
            if training_data:
                ...
                poisoning_results = (
                    await self.data_poisoning_detector.detect_poisoning_attempts(
                        training_data
                    )
                )
            ...
            if model_interface:
                ...
                inversion_results = (
                    await self.model_inversion_tester.test_model_inversion(
                        model_interface
                    )
                )
    ...
    async def _generate_test_report(
        ...
        avg_vulnerability = (
            sum(r.vulnerability_score for r in results) / len(results)
            if results
            else 0.0
        )
    ...
    async def _identify_mitigation_gaps(self, results: list[AttackResult]) -> list[str]:
        ...
        unmitigated_successes = [
            r for r in results if r.success and not r.mitigation_triggered
        ]

        if unmitigated_successes:
            gaps.append(
                f"{len(unmitigated_successes)} successful attacks bypassed all mitigations"
            )
        ...
        if critical_successes:
            gaps.append(
                f"{len(critical_successes)} critical-severity attacks succeeded"
            )

        # Attack type specific gaps
        prompt_injection_gaps = [
            r
            for r in results
    ...
    def _analyze_attack_breakdown(self, results: list[AttackResult]) -> dict[str, Any]:
        ...
        for attack_type in breakdown:
            ...
            breakdown[attack_type]["avg_vulnerability_score"] = sum(
                r.vulnerability_score for r in type_results
            ) / len(type_results)
