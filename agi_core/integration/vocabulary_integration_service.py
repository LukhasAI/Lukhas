...
try:
    from agi_core.vocabulary import (
        agi_bridge, get_agi_symbol, format_agi_message, 
        get_vocabulary_context, translate_agi_to_dream, translate_agi_to_bio
    )
except ImportError:
    ...
    translate_agi_to_dream = lambda x: "🌙"
    translate_agi_to_bio = lambda x: "🧠"
...
class VocabularyIntegrationService:
    ...
    def create_enriched_event(self, operation: str, details: str = "", 
        ...
        if "cross_references" in full_context:
            for vocab_type, ref_key in full_context["cross_references"].items():
                if vocab_type == "dream":
    ...
    def export_vocabulary_analytics(self) -> Dict[str, Any]:
        ...
        cross_ref_types = {}
        for event in self.event_history:
            for ref_type in event.cross_references.keys():
