...
from ..memory.vector_memory import VectorMemoryStore, MemoryVector, MemoryType
from ..memory.dream_memory import DreamMemoryBridge, DreamPhase, DreamInsightType
        ...
        for tool_id, score in sorted_tools[:max_recommendations]:
            if score > 0.1:  # Only recommend if there's some intuitive basis
                rec = ToolRecommendation(
                    ...
                    primary_tool=tool_id,
                    reasoning=f"Intuitive selection based on pattern recognition and past success",
    ...
    async def _creative_tool_selection(self, context: Dict[str, Any],
        ...
        for tool_id, score in sorted_tools[:max_recommendations]:
            rec = ToolRecommendation(
                ...
                primary_tool=tool_id,
                reasoning=f"Creative selection emphasizing novelty and creative potential",
    ...
    async def _experimental_tool_selection(self, context: Dict[str, Any],
        ...
        for tool_id, score in sorted_tools[:max_recommendations]:
            rec = ToolRecommendation(
                ...
                primary_tool=tool_id,
                reasoning=f"Experimental selection for exploration and learning",
