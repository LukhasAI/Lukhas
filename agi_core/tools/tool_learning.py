...
class ToolLearner:
    ...
    async def _generate_recommendation(self, context: ToolLearningContext, insights: dict[str, Any]) -> ToolRecommendation:
        ...
        if insights["tool_rankings"]:
            best_tool = max(insights["tool_rankings"].keys(), key=lambda t: insights["tool_rankings"][t]["success_rate"])
    ...
    async def _recommend_usage_pattern(self, context: ToolLearningContext, insights: dict[str, Any]) -> ToolUsagePattern:
    ...
    async def _extract_learning_insights(self, context: ToolLearningContext, recommendation: ToolRecommendation) -> list[str]:
    ...
    def get_learning_insights(self) -> list[str]:
        ...
        if self.learning_experiences:
            ...
            insights.append(f"Overall learning success rate: {success_rate:.1%} ({successful_experiences}/{total_experiences})")
