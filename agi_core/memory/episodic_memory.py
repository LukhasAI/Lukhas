...
from .vector_memory import VectorMemoryStore, MemoryVector, MemoryType, MemoryImportance
...
class EpisodicMemorySystem:
    ...
    async def _create_episode_summary_memory(self, episode: Episode):
        ...
        if episode.success_score and episode.success_score > 0.8:
            importance = MemoryImportance.HIGH
        elif episode.learning_score and episode.learning_score > 0.8:
            importance = MemoryImportance.HIGH
        elif len(episode.key_insights) > 0 or len(episode.lessons_learned) > 0:
            importance = MemoryImportance.HIGH
    ...
    async def archive_old_episodes(self, days_threshold: int = None) -> int:
