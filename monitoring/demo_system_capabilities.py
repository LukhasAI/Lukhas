...
from endocrine_observability_engine import EndocrineObservabilityEngine, PlasticityTriggerType
...
from trigger_flow_example import TriggerFlowDemonstrator
...
class SystemCapabilitiesDemo:
    ...
    async def demo_scenario_1_stress_response(self):
        ...
        print(f"   🧬 Hormone Profile: Cortisol={stress_hormones['cortisol']:.3f}, Adrenaline={stress_hormones['adrenaline']:.3f}")
        ...
        print(f"   🔗 Bio-Symbolic Coherence: {avg_coherence:.3f} ({'ALIGNED' if avg_coherence > 0.6 else 'MISALIGNED'})")
    ...
    async def demo_scenario_2_performance_optimization(self):
        ...
        performance_snapshot = await self.endocrine_engine.create_snapshot(
            hormone_levels=performance_hormones,
        ...
        performance_triggers = [t for t in perf_triggers if t.trigger_type == PlasticityTriggerType.PERFORMANCE_OPTIMIZATION]
        ...
        context_metrics = await self.metrics_collector.collect_context_metrics(
            OperationalContext.LEARNING,
            {
                ...
                "system": {"cpu_percent": 85, "memory_percent": 70}  # System working hard but inefficiently
            }
        )
        ...
        print(f"      Performance: {performance_metrics['performance']:.3f} → {improved_metrics['performance']:.3f} (+{improved_metrics['performance'] - performance_metrics['performance']:.3f})")
        print(f"      Decision Confidence: {performance_metrics['decision_confidence']:.3f} → {improved_metrics['decision_confidence']:.3f}")
        print(f"      Response Time: {performance_metrics['response_time']:.3f} → {improved_metrics['response_time']:.3f}")
        
        improvement_score = (improved_metrics['performance'] - performance_metrics['performance']) / performance_metrics['performance']
    ...
    async def demo_scenario_3_social_enhancement(self):
        ...
        social_enhancement_triggers = [t for t in social_triggers if t.trigger_type == PlasticityTriggerType.SOCIAL_ENHANCEMENT]
        ...
        for trigger in social_enhancement_triggers:
            ...
            if plan:
                ...
                print(f"   🎭 Social Enhancement Plan:")
                print(f"      Focus: Enhanced empathy and social awareness")
                print(f"      Strategy: {plan.rule.strategy.value}")
        ...
        print(f"   🔗 Bio-Social Coherence: {sum(c.coherence_score for c in social_coherence) / len(social_coherence):.3f}")
        ...
        print(f"      Oxytocin: {social_hormones['oxytocin']:.3f} → {enhanced_oxytocin:.3f} (+{enhanced_oxytocin - social_hormones['oxytocin']:.3f})")
        print(f"      Empathy: {social_metrics['empathy_engagement']:.3f} → {enhanced_social_metrics['empathy_engagement']:.3f}")
        print(f"      Interaction Quality: {social_metrics['interaction_quality']:.3f} → {enhanced_social_metrics['interaction_quality']:.3f}")
        
        social_improvement = (enhanced_social_metrics['empathy_engagement'] - social_metrics['empathy_engagement']) / social_metrics['empathy_engagement']
    ...
    async def demo_scenario_4_adaptive_learning(self):
        ...
        learning_data = {
            "recent_experiments": [
                {"type": "stress_adaptation", "success": True, "improvement": 0.35, "duration": 45},
                {"type": "performance_optimization", "success": True, "improvement": 0.28, "duration": 30},
                {"type": "social_enhancement", "success": True, "improvement": 0.32, "duration": 60},
                {"type": "stress_adaptation", "success": False, "improvement": -0.05, "duration": 20},
                {"type": "creative_boost", "success": True, "improvement": 0.15, "duration": 90}
            ],
            "pattern_history": {
        ...
        consolidation_result = await self.learning_orchestrator.consolidate_learning({
            ...
            "pattern_discoveries": ["stress_work_correlation", "performance_dopamine_link", "social_oxytocin_boost"],
        ...
        print(f"   🎯 Learning Patterns Identified:")
        print(f"      • Stress adaptations most effective during work hours")
        print(f"      • Performance boosts require dopamine correlation")
        print(f"      • Social enhancements benefit from oxytocin simulation")
        ...
        print(f"   📈 System Learning Status: {'EXCELLENT' if learning_score > 0.7 else 'GOOD' if learning_score > 0.5 else 'DEVELOPING'}")
    ...
    async def demo_real_time_monitoring_dashboard(self):
        ...
        performance_history = [0.65, 0.68, 0.71, 0.69, 0.72, 0.75, 0.72]  # Last 7 cycles
        ...
        print(f"      Performance: {'▲' if perf_trend == 'IMPROVING' else '▶'} {' '.join(f'{p:.2f}' for p in performance_history[-5:])}")
        print(f"      Stress:      {'▼' if stress_trend == 'IMPROVING' else '▶'} {' '.join(f'{s:.2f}' for s in stress_history[-5:])}")
