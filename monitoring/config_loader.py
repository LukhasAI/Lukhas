...
def _apply_environment_overrides(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    config["unified_dashboard"]["port"] = int(os.getenv(
        "LUKHAS_DASHBOARD_PORT", str(config["unified_dashboard"].get("port", 3000))
    ))
    config["unified_dashboard"]["refresh_rate"] = int(os.getenv(
        "LUKHAS_DASHBOARD_REFRESH", str(config["unified_dashboard"].get("refresh_rate", 5))
    ))
    config["unified_dashboard"]["enable_auth"] = _str_to_bool(os.getenv(
        "LUKHAS_DASHBOARD_AUTH", str(config["unified_dashboard"].get("enable_auth", False))
    ))
    
    # Meta Dashboard Configuration
    config.setdefault("meta_dashboard", {})
    config["meta_dashboard"]["port"] = int(os.getenv(
        "LUKHAS_META_DASHBOARD_PORT", str(config["meta_dashboard"].get("port", 5042))
    ))
    config["meta_dashboard"]["refresh_rate"] = int(os.getenv(
        "LUKHAS_META_REFRESH", str(config["meta_dashboard"].get("refresh_rate", 15))
    ))
    
    # Metrics Collection
    config.setdefault("metrics_collection", {})
    config["metrics_collection"]["enabled"] = _str_to_bool(os.getenv(
        "LUKHAS_METRICS_ENABLED", str(config["metrics_collection"].get("enabled", True))
    ))
    config["metrics_collection"]["interval"] = int(os.getenv(
        "LUKHAS_METRICS_INTERVAL", str(config["metrics_collection"].get("interval", 1))
    ))
    config["metrics_collection"]["retention_hours"] = int(os.getenv(
        "LUKHAS_METRICS_RETENTION", str(config["metrics_collection"].get("retention_hours", 24))
    ))
    
    # Security Configuration
    config.setdefault("security", {})
    config["security"]["api_key_required"] = _str_to_bool(os.getenv(
        "LUKHAS_API_KEY_REQUIRED", str(config["security"].get("api_key_required", False))
    ))
    ...
    config["security"]["rate_limits"]["api_requests_per_minute"] = int(os.getenv(
    ...
    config["performance"]["database"]["url"] = os.getenv(
        "LUKHAS_DATABASE_URL", config["performance"]["database"].get("url", "sqlite:///monitoring.db")
    )
    config["performance"]["database"]["pool_size"] = int(os.getenv(
        "LUKHAS_DB_POOL_SIZE", str(config["performance"]["database"].get("pool_size", 10))
    ))
    ...
    config["data_storage"]["metrics_file"] = os.getenv(
        "LUKHAS_METRICS_FILE", config["data_storage"].get("metrics_file", "data/metrics.jsonl")
    )
    config["data_storage"]["alerts_file"] = os.getenv(
        "LUKHAS_ALERTS_FILE", config["data_storage"].get("alerts_file", "data/alerts.jsonl")
    )
    ...
    config["integrations"]["prometheus"]["enabled"] = _str_to_bool(os.getenv(
        "LUKHAS_PROMETHEUS_ENABLED", str(config["integrations"]["prometheus"].get("enabled", False))
    ))
    config["integrations"]["prometheus"]["port"] = int(os.getenv(
        "LUKHAS_PROMETHEUS_PORT", str(config["integrations"]["prometheus"].get("port", 9090))
    ))
    
    # Grafana
    config["integrations"].setdefault("grafana", {})
    config["integrations"]["grafana"]["enabled"] = _str_to_bool(os.getenv(
        "LUKHAS_GRAFANA_ENABLED", str(config["integrations"]["grafana"].get("enabled", False))
    ))
    config["integrations"]["grafana"]["host"] = os.getenv(
        "LUKHAS_GRAFANA_HOST", config["integrations"]["grafana"].get("host", "localhost")
    )
    config["integrations"]["grafana"]["port"] = int(os.getenv(
        "LUKHAS_GRAFANA_PORT", str(config["integrations"]["grafana"].get("port", 3001))
    ))
    
    # External Services
    config["integrations"].setdefault("datadog", {})
    config["integrations"]["datadog"]["enabled"] = _str_to_bool(os.getenv(
        "LUKHAS_DATADOG_ENABLED", str(config["integrations"]["datadog"].get("enabled", False))
    ))
    ...
    config["integrations"]["slack"]["enabled"] = _str_to_bool(os.getenv(
        "LUKHAS_SLACK_ENABLED", str(config["integrations"]["slack"].get("enabled", False))
    ))
    ...
    config["integrations"]["slack"]["channel"] = os.getenv(
        "SLACK_CHANNEL", config["integrations"]["slack"].get("channel", "#lukhas-alerts")
    )
    
    # PagerDuty
    config["integrations"].setdefault("pagerduty", {})
    config["integrations"]["pagerduty"]["enabled"] = _str_to_bool(os.getenv(
        "LUKHAS_PAGERDUTY_ENABLED", str(config["integrations"]["pagerduty"].get("enabled", False))
    ))
    config["integrations"]["pagerduty"]["routing_key"] = os.getenv(
        "PAGERDUTY_ROUTING_KEY", config["integrations"]["pagerduty"].get("routing_key", "")
    )
    ...
    if environment == "production":
        ...
        config["production"]["ssl_enabled"] = _str_to_bool(os.getenv(
            "LUKHAS_SSL_ENABLED", str(config["production"].get("ssl_enabled", False))
        ))
    ...
    config["development"]["debug_mode"] = _str_to_bool(os.getenv(
        "LUKHAS_DEBUG", str(config["development"].get("debug_mode", False))
    ))
    config["development"]["mock_data"] = _str_to_bool(os.getenv(
...
def load_validated_config(config_path: Optional[str] = None) -> tuple[Dict[str, Any], Dict[str, Any]]:
    """
