...
from lukhas_pwm.audit.tool_analytics import (
    ToolAnalytics, ToolCall, ToolIncident,
    enforce_tool_governance, get_analytics
)
from lukhas_pwm.openai.tooling import get_tool_names
...
def test_tool_tracking():
    ...
    call_id1 = analytics.start_tool_call(
        tool_name="retrieval",
        arguments={"query": "quantum computing", "k": 5}
    )
    time.sleep(0.1)  # Simulate execution time
    analytics.complete_tool_call(call_id1, status="success", result={"docs": 5})
    print(f"   ✅ Retrieval tool executed (100ms)")
    ...
    print(f"   ✅ Browser tool executed (50ms)")
    ...
    time.sleep(0.02)
    analytics.complete_tool_call(
        call_id3, 
        status="failed", 
        error="Sandbox unavailable"
    )
    print(f"   ⚠️ Code execution failed (20ms)")
...
def test_audit_bundle_integration():
    ...
    print(f"   Tools used: {[t['tool'] for t in audit_bundle['tool_analytics']['tools_used']]}")
    print(f"   Total duration: {sum(t['duration_ms'] for t in audit_bundle['tool_analytics']['tools_used'])}ms")
    ...
    print(f"   Attempted disallowed tool: code_exec")
    print(f"   Safety auto-tightened: balanced → strict")
    print(f"   Incident severity: high")
    print(f"   Action taken: blocked_and_tightened")
...
def test_api_endpoints():
    ...
    for endpoint in endpoints:
        ...
        if "example_response" in endpoint:
            ...
            print(f"      {json.dumps(endpoint['example_response'], indent=6)[:200]}...")
...
def main():
    ...
    # Run tests
    analytics = test_tool_tracking()
    incidents = test_security_incidents()
