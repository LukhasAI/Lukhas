...
class TestOIDCSecurityHardening:
    ...
    @pytest.mark.asyncio
    async def test_valid_authorization_request(self, security_hardening, valid_auth_params, request_context):
        """Test valid authorization request passes security validation"""
        result = await security_hardening.validate_authorization_request(valid_auth_params, request_context)
    ...
    @pytest.mark.asyncio
    async def test_missing_required_parameters(self, security_hardening, valid_auth_params, request_context):
        """Test missing required parameters trigger security events"""
        # Test missing client_id
        params = valid_auth_params.copy()
        del params["client_id"]

        result = await security_hardening.validate_authorization_request(params, request_context)

        assert result["valid"] is False
        assert result["security_response"] == SecurityResponse.BLOCK
        assert result["risk_score"] >= 80.0
        assert any(event.event_type == OIDCSecurityEventType.INVALID_CLIENT for event in result["events"])

    @pytest.mark.asyncio
    async def test_nonce_replay_attack(self, security_hardening, valid_auth_params, request_context):
        """Test nonce replay attack detection and blocking"""
        # First request should succeed
        result1 = await security_hardening.validate_authorization_request(valid_auth_params, request_context)
        assert result1["valid"] is True

        # Second request with same nonce should be blocked
        result2 = await security_hardening.validate_authorization_request(valid_auth_params, request_context)

        assert result2["valid"] is False
        assert result2["security_response"] == SecurityResponse.BLOCK
        assert any(event.event_type == OIDCSecurityEventType.NONCE_REPLAY for event in result2["events"])
        assert any(event.threat_level == SecurityThreatLevel.CRITICAL for event in result2["events"])

    @pytest.mark.asyncio
    async def test_rate_limiting_protection(self, security_hardening, valid_auth_params, request_context):
        ...
        for i in range(10):
            ...
            if i < 10:  # First 10 should succeed
                ...
                assert result["valid"] is False
                assert any(event.event_type == OIDCSecurityEventType.RATE_LIMIT_EXCEEDED for event in result["events"])

    @pytest.mark.asyncio
    async def test_invalid_redirect_uri_security(self, security_hardening, valid_auth_params, request_context):
        ...
        assert result["valid"] is False
        assert result["security_response"] == SecurityResponse.BLOCK
        assert any(event.event_type == OIDCSecurityEventType.INVALID_REDIRECT_URI for event in result["events"])
        assert any("HTTPS" in event.description for event in result["events"])

    @pytest.mark.asyncio
    async def test_pkce_validation_security(self, security_hardening, valid_auth_params, request_context):
        ...
        assert (
            len(
                [
                    ...
                    if event.event_type == OIDCSecurityEventType.PKCE_VALIDATION_FAILURE
                ]
            )
            > 0
        )
        ...
        assert any(event.event_type == OIDCSecurityEventType.PKCE_VALIDATION_FAILURE for event in result["events"])

    @pytest.mark.asyncio
    async def test_scope_security_validation(self, security_hardening, valid_auth_params, request_context):
        ...
        assert any(event.event_type == OIDCSecurityEventType.EXCESSIVE_SCOPE_REQUEST for event in result["events"])
    ...
    async def test_jwt_security_validation(self, security_hardening):
        ...
        header_b64 = base64.urlsafe_b64encode(json.dumps(none_token_header).encode()).decode().rstrip("=")
        payload_b64 = base64.urlsafe_b64encode(json.dumps(none_token_payload).encode()).decode().rstrip("=")
        ...
        assert any(event.event_type == OIDCSecurityEventType.JWT_ALGORITHM_ATTACK for event in result["events"])
    ...
    async def test_token_request_validation(self, security_hardening, request_context):
        ...
        result = await security_hardening.validate_token_request(token_params, request_context)

        assert result["valid"] is True
        assert result["security_response"] == SecurityResponse.ALLOW

        # Test authorization code reuse
        result2 = await security_hardening.validate_token_request(token_params, request_context)

        assert result2["valid"] is False
        assert any(event.event_type == OIDCSecurityEventType.AUTHORIZATION_CODE_REUSE for event in result2["events"])
    ...
    async def test_fail_closed_design(self, request_context):
        ...
        result = await fail_closed_security.validate_authorization_request(malicious_params, request_context)
        ...
        result2 = await fail_open_security.validate_authorization_request(malicious_params, request_context)
    ...
    @pytest.mark.asyncio
    async def test_security_metrics(self, security_hardening, valid_auth_params, request_context):
        """Test security metrics collection"""
        # Generate some security events
        await security_hardening.validate_authorization_request(valid_auth_params, request_context)
        ...
        await security_hardening.validate_authorization_request(malicious_params, request_context)
    ...
    @pytest.mark.asyncio
    async def test_performance_requirements(self, security_hardening, valid_auth_params, request_context):
...
class TestWebAuthnOIDCIntegration:
    ...
    @pytest.mark.asyncio
    async def test_webauthn_authentication_completion(
        self, integration, auth_params, webauthn_response, request_context
    ):
        ...
        with patch.object(
            integration.webauthn_security, "validate_authentication_response", new_callable=AsyncMock
        ) as mock_validate:
            mock_validate.return_value = {"valid": True, "user_verified": True, "credential_binding_valid": True}

            # Complete authentication
            result = await integration.complete_webauthn_authentication(session_id, webauthn_response, request_context)
    ...
    @pytest.mark.asyncio
    async def test_oidc_token_generation_with_webauthn_context(
        self, integration, auth_params, webauthn_response, request_context
    ):
    ...
    async def test_guardian_system_integration(self, integration, auth_params, request_context):
        ...
        init_result = await integration.initiate_webauthn_oidc_flow(auth_params, high_risk_context)
        session_id = init_result["session_id"]
    ...
    @pytest.mark.asyncio
    async def test_integration_performance(self, integration, auth_params, webauthn_response, request_context):
