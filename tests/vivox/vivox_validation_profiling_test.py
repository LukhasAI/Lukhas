...
from vivox import create_vivox_system, ActionProposal, PotentialState
...
def log_function_call(logger):
    ...
    def decorator(func):
        ...
        async def async_wrapper(*args, **kwargs):
            # Log function entry
            logger.debug(f"ENTER {func.__name__} | args: {args[1:] if args else 'none'} | kwargs: {kwargs}")
            ...
            try:
                ...
                logger.debug(f"EXIT {func.__name__} | elapsed: {elapsed:.3f}s | result: {result_summary}")
                
                return result
            except Exception as e:
                elapsed = time.time() - start_time
                logger.error(f"ERROR {func.__name__} | elapsed: {elapsed:.3f}s | error: {str(e)}")
                raise
...
class ValidationProfiler:
    ...
    def _add_logging_decorators(self):
        ...
        me.record_decision_mutation = log_function_call(me_logger)(me.record_decision_mutation)
        me.resonant_memory_access = log_function_call(me_logger)(me.resonant_memory_access)
        me.truth_audit_query = log_function_call(me_logger)(me.truth_audit_query)
        me._calculate_helix_position = log_function_call(me_logger)(me._calculate_helix_position)
        me._encode_emotional_dna = log_function_call(me_logger)(me._encode_emotional_dna)
        
        # Moral Alignment Engine logging
        mae = self.vivox["moral_alignment"]
        mae.evaluate_action_proposal = log_function_call(mae_logger)(mae.evaluate_action_proposal)
        ...
        mae._calculate_emotional_resonance = log_function_call(mae_logger)(mae._calculate_emotional_resonance)
        ...
        cil.simulate_conscious_experience = log_function_call(cil_logger)(cil.simulate_conscious_experience)
        cil.drift_monitor.measure_drift = log_function_call(cil_logger)(cil.drift_monitor.measure_drift)
        ...
        srm.structural_conscience_query = log_function_call(srm_logger)(srm.structural_conscience_query)
    ...
    async def validate_dynamic_outputs(self):
        ...
        for i in range(5):
            ...
            action_type = random.choice(["analyze", "create", "modify", "delete", "query"])
            ...
            memory = await self.vivox["memory_expansion"].memory_helix.get_entry(memory_id)
            ...
            print(f"   - Emotional DNA: V={valence:.2f}, A={arousal:.2f}, D={dominance:.2f}")
            print(f"   - Resonance Freq: {memory.emotional_dna.resonance_frequency:.3f}")
            print(f"   - 3D Position: ({memory.helix_coordinates[0]:.2f}, "
                  f"{memory.helix_coordinates[1]:.2f}, {memory.helix_coordinates[2]:.2f})")
            print(f"   - Crypto Hash: {memory.cryptographic_hash[:16]}...")
        
        # Validate uniqueness
        unique_hashes = set(m["hash"] for m in test_memories)
        unique_positions = set(str(m["helix_coordinates"]) for m in test_memories)
        ...
        for i in range(5):
            ...
            print(f"   - Ethical Alignment: {experience.drift_measurement.ethical_alignment:.3f}")
        ...
        self.collapses_captured.append({
            "selected_state": collapsed.selected_state.state_id if collapsed.selected_state else None,
        ...
        print(f"   - Selected: {collapsed.selected_state.state_id if collapsed.selected_state else 'None'}")
        print(f"   - Reason: {collapsed.collapse_reason}")
        
        # Test 5: Truth audit query
        print("\n5️⃣ Testing Truth Audit Query...")
        
        audit_result = await self.vivox["memory_expansion"].truth_audit_query("analyze_operation")
    ...
    def generate_validation_report(self):
        ...
        if self.decisions_captured:
            unique_fingerprints = set(d["moral_fingerprint"] for d in self.decisions_captured)
            ...
            print(f"   - Dissonance Range: {min(dissonance_values):.3f} - {max(dissonance_values):.3f}")
            print(f"   - Approved: {sum(1 for d in self.decisions_captured if d['approved'])}")
            print(f"   - Suppressed: {sum(1 for d in self.decisions_captured if not d['approved'])}")
        ...
        if self.experiences_captured:
            ...
            print(f"   - Drift Range: {min(drift_values):.3f} - {max(drift_values):.3f}")
            print(f"   - Average Coherence: {sum(e['coherence'] for e in self.experiences_captured) / len(self.experiences_captured):.3f}")
