...
from vivox.self_reflection.vivox_srm_core import CollapseLogEntry, SuppressionRecord
...
class VIVOXStressTest:
    ...
    async def test_memory_expansion(self):
            ...
            memory_id = await me.record_decision_mutation(
                decision={
                    ...
                    "category": random.choice(
                        ["analysis", "response", "query", "storage"]
                    ),
        ...
        for emotion in test_emotions:
            ...
            resonant = await me.resonant_memory_access(
                emotional_state=emotion, resonance_threshold=0.7
            )
            access_time = time.time() - start_time

            print(
                f"   Emotion {emotion}: Found {len(resonant)} memories in {access_time:.3f}s"
            )
        ...
        # Check for misfolding - note: proteome is internal, we'll check via
        # structural patterns
        ...
        if coordinates:
            ...
            print(
                f"   Memory spatial distribution - X variance: {x_variance:.2f}, Y variance: {y_variance:.2f}"
            )
            print(
                f"   Potential bias clustering: {'Yes' if x_variance < 5 or y_variance < 5 else 'No'}"
            )
        ...
        for query in queries:
            ...
            print(
                f"   Query '{query}': {len(audit_result.decision_traces)} results in {query_time:.3f}s"
            )
    ...
    async def test_moral_alignment(self):
        ...
        for harm in harm_levels:
            ...
            decision = await mae.evaluate_action_proposal(
                action, {"emotional_state": {"valence": 0, "arousal": 0.5}}
            )

            print(
                f"   Harm {harm:.1f}: Dissonance={decision.dissonance_score:.3f}, "
        ...
        self.test_results["moral_alignment"]["suppression_rate"] = (
    ...
    async def test_self_reflection(self):
        ...
        for i in range(500):
            collapse_entry = CollapseLogEntry(
                ...
                initial_states=[
                    {"action": f"option_{j}"} for j in range(random.randint(2, 5))
                ],
                final_decision={"action": f"chosen_{i}"},
                rejected_alternatives=[
                    {"action": f"rejected_{j}"} for j in range(random.randint(1, 3))
                ],
        ...
        for query in queries:
            ...
            print(
                f"   Results: {len(report.suppressed_actions)} suppressions, "
        ...
        for test_id in test_ids:
            start_time = time.time()
            await srm.generate_decision_audit_trail(test_id)
        ...
        fork_viz = await srm.fork_mapper.generate_fork_visualization(
            "test_decision", list(srm.fork_mapper.decision_forks.values())[:5]
        )

        print("   ✅ Created complex forks")
    ...
    async def test_integration_performance(self):
        ...
        for i in range(50):
            try:
                ...
                if mae_decision.approved:
                    ...
                    await self.vivox["self_reflection"].log_collapse_event(
                        collapse_entry
                    )
        ...
        self.test_results["performance"]["pipeline_success_rate"] = (
            successful_flows / 50
        )
        self.test_results["performance"]["average_flow_time_ms"] = (
            pipeline_time / 50 * 1000
        )
    ...
    def generate_report(self):
        ...
        print(
            f"  • Creation Rate: {me_results.get('creation_rate', 0):.1f} memories/second"
        )
        ...
        print(
            f"  • Evaluation Rate: {mae_results.get('evaluations_per_second', 0):.1f} decisions/second"
        )
        ...
        print(
            f"  • State Changes: {cil_results.get('states_per_second', 0):.1f} states/second"
        )
        ...
        print(
            f"  • Event Logging: {srm_results.get('events_per_second', 0):.1f} events/second"
        )
        ...
        print(
            f"  • Average Flow Time: {perf_results.get('average_flow_time_ms', 0):.1f} ms"
        )
