...
async def test_dream_orchestrator_improvements():
    ...
    from lambda_products_pack.lambda_core.NIAS.dependency_container import (
        DependencyContainer,
        ServiceLifecycle,
    )
    ...
    print(
        f"  • Service Recovery: {orchestrator.metrics['successful_recoveries']} successful recoveries"
    )
    print(
        f"  • Fallback Services: {orchestrator.metrics['service_fallbacks']} fallbacks used"
    )
    print("  • API Validation: ✅ Active")
    ...
    print(
        f"  • Services with Dependencies: {sum(1 for s in service_info.values() if s['dependencies'])}"
    )
