...
class DummyNode(MatrizNode):
    ...
    def handle(self, msg: MatrizMessage) -> MatrizResult:
        return MatrizResult(
            ok=True,
            payload={"echo": msg.payload, "processed": True},
            trace={"handled_by": self.name, "topic": msg.topic},
            guardian_log=[f"dummy_processed_{msg.topic}"]
        )
...
class FailingNode(MatrizNode):
    ...
    def handle(self, msg: MatrizMessage) -> MatrizResult:
        return MatrizResult(
            ok=False,
            reasons=["Simulated failure for testing"],
            payload={},
            trace={"error": "simulated", "handled_by": self.name},
            guardian_log=[f"dummy_failed_{msg.topic}"]
        )
...
class TestMatrizMessageValidation:
    ...
    def test_valid_message(self):
        ...
        msg = mk_test_message(
            topic=Topic.CONTRADICTION.value,
            lane="experimental",
            payload={"test": True}
        )
    ...
    def test_invalid_message_lane(self):
        ...
        msg_dict = {
            "lane": "invalid-lane",  # Invalid lane
            "topic": Topic.CONTRADICTION.value,
            "glyph": {"id": str(uuid4()), "kind": "intent"},
            "payload": {"test": True}
        }
    ...
    def test_invalid_message_unknown_topic(self):
        ...
        msg = mk_test_message(topic="test", lane="experimental")  # not in allowed ontology
        assert not validate_message(msg)
...
class TestMatrizResultValidation:
    ...
    def test_valid_result(self):
        ...
        result = MatrizResult(
            ...
            trace={"handled_by": "test"},
            guardian_log=["test_logged"]
        )
    ...
    def test_invalid_result_reasons(self):
        ...
        result = MatrizResult(
            ok=True,
            reasons="not-a-list",  # Should be list
            payload={},
            trace={},
            guardian_log=[]
        )
    ...
    def test_invalid_result_empty_guardian_log(self):
        ...
        result = MatrizResult(
            ...
            payload={},
            trace={},
            guardian_log=[]  # empty should be invalid per contract
        )
        assert not validate_result(result)
    ...
    def test_invalid_result_non_json_payload(self):
        ...
        result = MatrizResult(
            ok=True,
            reasons=[],
            payload={"bad": {1, 2, 3}},  # set is not JSON-serializable
            trace={},
            guardian_log=["crumb"]
        )
        assert not validate_result(result)
...
class TestNodeContractCompliance:
    ...
    def test_failing_node_contract_compliance(self):
        ...
        # Verify result structure even for failures
        assert validate_result(result)
...
class TestContractImmutability:
    ...
    def test_result_mutability(self):
        ...
        # Should be able to modify non-frozen dataclass
...
def test_contract_golden_example():
    ...
    # Use the exact example from T4 spec
