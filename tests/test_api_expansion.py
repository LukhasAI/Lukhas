...
def test_create_user():
    ...
    response = client.post("/identity/users", json={"username": "newuser", "email": "new@example.com", "password": "password"})
    assert response.status_code == 200
...
def test_consolidate_users():
    ...
    response = client.post("/identity/consolidate", json={"primary_user_id": "user_123", "secondary_user_id": "user_456"})
    assert response.status_code == 200
    assert response.json()["user_id"] == "user_123"
