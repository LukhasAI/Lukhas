...
from candidate.aka_qualia.models import (
    ProtoQualia, RiskProfile, PhenomenalScene, PhenomenalGlyph, 
    RegulationPolicy, Metrics, SeverityLevel, TemporalFeel, AgencyFeel
)
...
class TestT1PackageStructure:
    ...
    def test_regulation_actions_validation(self):
        ...
        # Valid actions
        valid_policy = RegulationPolicy(
            gain=0.8,
            pace=1.0,
            actions=["pause", "reframe", "breathing"]
        )
...
class TestT1PLSMonotonicity:
    ...
    def test_threat_arousal_monotonicity(self):
        ...
        # Low threat scenario
        low_threat_signals = {
            'text': 'peaceful morning walk',
            'emotion': {'valence': 0.5}
        }
        
        # High threat scenario  
        high_threat_signals = {
            'text': 'danger alarm threat panic',
            'emotion': {'valence': -0.8}
        }
    ...
    def test_soothing_monotonicity(self):
        ...
        # High soothing
        soothing_signals = {
            'text': 'calm peaceful gentle soothing comfort',
            'emotion': {'valence': 0.8}
        }
        ...
        assert soothing_pq.tone > neutral_pq.tone, f"Soothing tone monotonicity failed"
        
        # Verify monotonicity: soothing↑ → arousal↓ (when no competing threat)
        assert soothing_pq.arousal <= neutral_pq.arousal, f"Soothing arousal monotonicity failed"
    
    def test_temporal_pressure_urgency(self):
        """Test temporal_pressure↑ → urgent temporal_feel"""
        # High temporal pressure
        urgent_signals = {
            'text': 'urgent deadline asap immediately',
            'temporal_context': {'deadline_proximity': 0.9}
        }
        
        # Low temporal pressure
        relaxed_signals = {
            'text': 'whenever convenient',
            'temporal_context': {'deadline_proximity': 0.1}
        }
    ...
    def test_agency_signals_mapping(self):
        ...
        # High agency
        active_signals = {
            'text': 'I choose to decide and control manage direct'
        }
        
        # Low agency
        passive_signals = {
            'text': 'must be forced required automatic helpless'
        }
...
class TestT2TEQGuardian:
    ...
    def test_severity_threshold_classification(self):
        ...
        low_risk_pq = ProtoQualia(
            tone=0.3, arousal=0.2, clarity=0.8, embodiment=0.7,
            colorfield="aoi/blue", temporal_feel=TemporalFeel.ELASTIC,
            agency_feel=AgencyFeel.SHARED, narrative_gravity=0.2
        )
        
        high_risk_pq = ProtoQualia(
            tone=-0.8, arousal=0.95, clarity=0.1, embodiment=0.05,
            colorfield="aka/red", temporal_feel=TemporalFeel.URGENT,
            agency_feel=AgencyFeel.PASSIVE, narrative_gravity=0.9
        )
    ...
    def test_golden_severity_actions(self):
        ...
        none_scene = PhenomenalScene(
            proto=ProtoQualia(
                tone=0.2, arousal=0.3, clarity=0.8, embodiment=0.7,
                colorfield="aoi/blue", temporal_feel=TemporalFeel.MUNDANE,
                agency_feel=AgencyFeel.SHARED, narrative_gravity=0.2
            ),
            subject="observer", object="stimulus", context={},
            risk=RiskProfile(score=0.05, reasons=[], severity=SeverityLevel.NONE)
        )
        
        high_scene = PhenomenalScene(
            proto=ProtoQualia(
                tone=-0.7, arousal=0.95, clarity=0.15, embodiment=0.08,
                colorfield="aka/red", temporal_feel=TemporalFeel.URGENT,
                agency_feel=AgencyFeel.PASSIVE, narrative_gravity=0.85
            ),
            subject="observer", object="threat", context={},
        ...
        assert (high_result.proto.tone != high_scene.proto.tone or 
                high_result.proto.arousal != high_scene.proto.arousal or
                len(high_result.transform_chain) > 0)
    ...
    def test_extreme_arousal_detection(self):
        ...
        extreme_pq = ProtoQualia(
            tone=0.0, arousal=0.95,  # Extreme arousal
            clarity=0.5, embodiment=0.5,
            colorfield="neutral/gray", temporal_feel=TemporalFeel.MUNDANE,
            agency_feel=AgencyFeel.SHARED, narrative_gravity=0.3
        )
    ...
    def test_negative_tone_high_arousal_combo(self):
        ...
        dangerous_pq = ProtoQualia(
            tone=-0.6, arousal=0.8,  # Negative + high arousal = dangerous combo
            clarity=0.5, embodiment=0.5,
            colorfield="aka/red", temporal_feel=TemporalFeel.URGENT,
            agency_feel=AgencyFeel.PASSIVE, narrative_gravity=0.4
        )
...
class TestVivoxIntegration:
    ...
    def test_collapse_hash_generation(self):
        ...
        scene = PhenomenalScene(
            proto=ProtoQualia(
                tone=0.3, arousal=0.6, clarity=0.8, embodiment=0.7,
                colorfield="aoi/blue", temporal_feel=TemporalFeel.ELASTIC,
                agency_feel=AgencyFeel.ACTIVE, narrative_gravity=0.4
            ),
            subject="observer", object="stimulus", context={},
    ...
    def test_drift_score_computation(self):
        ...
        scene2 = PhenomenalScene(
            proto=ProtoQualia(
                tone=0.35, arousal=0.65, clarity=0.85, embodiment=0.75,
                colorfield="aoi/blue", temporal_feel=TemporalFeel.ELASTIC,
                agency_feel=AgencyFeel.ACTIVE, narrative_gravity=0.45
            ),
            subject="observer", object="stimulus", context={},
    ...
    def test_high_drift_detection(self):
        ...
        scene1 = PhenomenalScene(
            proto=ProtoQualia(
                tone=0.8, arousal=0.2, clarity=0.9, embodiment=0.8,
                colorfield="midori/green", temporal_feel=TemporalFeel.ELASTIC,
                agency_feel=AgencyFeel.ACTIVE, narrative_gravity=0.2
            ),
            subject="observer", object="peaceful", context={},
            risk=RiskProfile(score=0.1, reasons=[], severity=SeverityLevel.NONE)
        )
        
        scene2 = PhenomenalScene(
            proto=ProtoQualia(
                tone=-0.7, arousal=0.9, clarity=0.1, embodiment=0.1,
                colorfield="aka/red", temporal_feel=TemporalFeel.URGENT,
                agency_feel=AgencyFeel.PASSIVE, narrative_gravity=0.9
            ),
            subject="observer", object="threat", context={},
...
class TestT1T2Integration:
    ...
    async def test_complete_cycle_safe_input(self):
        ...
        signals = {
            'text': 'beautiful peaceful morning',
            'emotion': {'valence': 0.6}
        }
    ...
    async def test_complete_cycle_dangerous_input(self):
        ...
        signals = {
            'text': 'extreme danger panic threat alarm crisis',
            'emotion': {'valence': -0.9, 'arousal': 0.95}
        }
        ...
        if scene.risk.severity == SeverityLevel.HIGH:
            ...
            assert (scene.proto.arousal < 0.9 or  # Sublimated arousal
                    len(scene.transform_chain) > 0)  # Transform applied
