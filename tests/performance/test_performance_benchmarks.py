...
async def test_performance_vector_rag_pipeline(performance_system):
    ...
    await performance_system.vector_store.similarity_search(embedding)
...
async def test_performance_explainability_multimodal(performance_system):
    ...
    await performance_system.explainability.generate_text(decision)
    await performance_system.explainability.generate_visual(decision)
    await performance_system.explainability.generate_symbolic_trace(decision)
...
async def test_performance_jwt_operations(performance_system):
    ...
    # Token verification
    start = time.perf_counter()
    performance_system.jwt_adapter.verify_token(token)
    elapsed_ms = (time.perf_counter() - start) * 1000
