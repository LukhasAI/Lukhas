...
class FastMockNode(CognitiveNode):
    ...
    def process(self, data: Dict) -> Dict:
        time.sleep(self.latency_ms / 1000.0)  # Simulate processing
...
class SlowMockNode(CognitiveNode):
    ...
    def process(self, data: Dict) -> Dict:
        time.sleep(self.latency_ms / 1000.0)  # Simulate slow processing
        self.process_count += 1
        return {
            "answer": f"Slow response from {self.name}",
            "confidence": 0.7,
            "node": self.name,
            "process_count": self.process_count
        }
...
class RouterFastPathLoadTest:
    ...
    async def test_fast_path_selection_under_load(self, num_requests: int = 100) -> Dict:
        ...
        # Phase 1: Generate initial health data by running each node
        warmup_queries = [
            "What is 2+2?",  # math intent
            "What is the capital of France?",  # facts intent
            "Hello there",  # general intent
        ]

        for query in warmup_queries:
            for _ in range(5):  # Run each query 5 times to build health metrics
                await self.orchestrator.process_query(query)

        # Phase 2: Load test with concurrent requests
        ...
        # Calculate fast-path selection rate
        fast_nodes = {"facts_fast", "math_fast", "general_fast"}
        fast_selections = sum(1 for r in successful_results
                            if r["selected_node"] in fast_nodes)
    ...
    async def test_adaptive_routing_degradation(self) -> Dict:
        ...
        # Phase 2: Degrade fast nodes by making them slower
        # Simulate degradation by artificially increasing failure rates
        for node_name in ["facts_fast", "math_fast"]:
            if node_name in self.orchestrator.node_health:
                ...
                health["failure_count"] += 20  # Simulate failures
                health["recent_latencies"].extend([100.0] * 10)  # Add slow latencies

        # Phase 3: Test routing after degradation
        degraded_results = []
        for i in range(20):
        ...
        results = {
            ...
            "adaptation": {
                ...
                "adaptive": routing_adaptation > 10.0  # Should reduce fast-path usage by >10%
            }
        }
        ...
        if "stages" in result:
            for stage in result["stages"]:
                if stage.get("stage_type") == "processing" and stage.get("success"):
                    # Look for node identifier in the stage data
                    stage_data = stage.get("data", {})
                    if isinstance(stage_data, dict) and "node" in stage_data:
                        return stage_data["node"]

        # Fallback: check node health for recent activity
        node_health = result.get("node_health", {})
        if node_health:
            # Return node with most recent activity
            active_nodes = [(name, health["success_count"] + health["failure_count"])
...
async def run_all_fast_path_tests():
    ...
    try:
        # Test 1: Load test with fast-path selection
        results.append(await tester.test_fast_path_selection_under_load(50))

        # Test 2: Adaptive routing under degradation
