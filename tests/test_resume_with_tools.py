...
def test_resume_with_tools_allows_and_blocks():
    ...
    sys_msgs = [
        m for m in out if m["role"] == "system" and "blocked" in m["content"].lower()
    ]
    ...
    assert (
        "knowledge" in tool_msgs[0]["content"].lower()
        or "context" in tool_msgs[0]["content"].lower()
    )

    # Should have one system message about blocked browser
    assert (
        len(sys_msgs) == 1
    ), f"Expected 1 system message about blocking, got {len(sys_msgs)}"
    assert (
        "open_url" in sys_msgs[0]["content"]
        or "blocked" in sys_msgs[0]["content"].lower()
    )
...
def test_resume_with_scheduler():
    ...
    tool_calls = [
        {
            ...
            "function": {
                ...
                "arguments": json.dumps(
                    {"when": "tomorrow 2pm", "note": "Team meeting"}
                ),
            },
        }
    ]
    ...
    assert (
        "scheduled" in tool_msgs[0]["content"].lower()
        or "task" in tool_msgs[0]["content"].lower()
    )
...
def test_all_tools_blocked():
    ...
    tool_calls = [
        ...
        {
            "id": "tc3",
            "function": {"name": "schedule_task", "arguments": "{}"},
        },
    ]
