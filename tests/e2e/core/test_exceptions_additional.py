    ...
    def test_memory_drift_basic(self):
        ...
        with pytest.raises(MemoryDriftError):
            raise MemoryDriftError(
                "Memory drift threshold exceeded", memory_id="test_memory", drift_level=0.8, threshold=0.5
            )
    ...
    def test_circuit_breaker_error(self):
        ...
        with pytest.raises(CircuitBreakerError):
            raise CircuitBreakerError(
                "Circuit breaker is open", service_name="test_service", failure_count=5, threshold=3, reset_timeout=30.0
            )
