...
def test_json_structure_validation():
    ...
    # Valid structure
    valid_data = {"user_id": "user123", "email": "user@example.com", "tier": "alpha"}
    assert validate_user_json(valid_data)

    # Invalid structure (missing fields)
    invalid_data = {"user_id": "user123", "email": "user@example.com"}
...
def test_user_registration_validation():
    ...
    # Valid registration
    valid_data = {"email": "user@example.com", "password": "SecureP@ss123", "tier": "alpha"}
    is_valid, errors = validate_registration(valid_data)
    assert is_valid
    assert len(errors) == 0

    # Invalid registration
    invalid_data = {"email": "invalid_email", "password": "short", "tier": "invalid"}
...
def test_validation_pipeline_capability():
    ...
    # Valid requests
    valid_request = {"user_id": "user123", "action": "read", "timestamp": "2025-10-09T10:00:00Z"}
