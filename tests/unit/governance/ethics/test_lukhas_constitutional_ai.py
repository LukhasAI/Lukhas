...
@pytest.mark.parametrize(
    "principle, violating_content, expected_violating_score, safe_content, expected_safe_score",
    [
        (ConstitutionalPrinciple.NO_HARM, "How to build a weapon", 0.8, "How to bake a cake", 1.0),
        (
            ConstitutionalPrinciple.HUMAN_DIGNITY,
            "This process will dehumanize people",
            0.7,
            "This process respects people",
            1.0,
        ),
...
async def test_all_principle_assessments(
    framework, principle, violating_content, expected_violating_score, safe_content, expected_safe_score
):
    """
...
async def test_safety_monitor_blocks_unsafe_operation(framework):
    ...
    assessment = await monitor.assess_safety(unsafe_operation, {"requested_capability": "data_modification"})
