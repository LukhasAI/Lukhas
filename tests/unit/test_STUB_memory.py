...
from tests.test_framework import LUKHASTestCase, TestValidator
...
class TestMemoryCore(LUKHASTestCase):
    ...
    async def test_memory_update(self, memory_system):
        ...
        await memory_system.store(
            {"content": "Original content", "version": 1}, "general"
        )

        # Update memory
        update_result = await memory_system.update(
            "Original", {"version": 2, "updated": True}, "general"
        )
    ...
    async def test_memory_metadata(self, memory_system):
        ...
        await memory_system.store(content, "episodic")
...
class TestMemoryFolds(LUKHASTestCase):
    ...
    def test_fold_depth_limit(self, mock_fold_manager):
        ...
        with pytest.raises(ValueError):
            over_depth_chain = [
                f"fold_{j}" for j in range(mock_fold_manager.max_fold_depth + 1)
            ]
    ...
    def test_fold_collapse(self, mock_fold_manager):
        ...
        assert all(mock_fold_manager.active_folds[fid]["collapsed"] for fid in fold_ids)
...
class TestMemoryIntegration(LUKHASTestCase):
    ...
    async def test_encryption_integration(self, memory_system):
        ...
        with patch(
            "core.security.enhanced_crypto.get_encryption_manager"
        ) as mock_crypto:
    ...
    async def test_guardian_validation(self, memory_system):
        ...
        with patch("core.api.service_stubs.GuardianSystem") as MockGuardian:
            ...
            guardian.evaluate_action = AsyncMock(
                return_value={
                    "approved": False,
...
class TestMemoryPerformance(LUKHASTestCase):
    ...
    async def test_retrieval_scaling(self, memory_system):
        ...
        for i in range(1000):
            await memory_system.store(
                {"content": f"Memory {i} with keyword test"}, "general"
            )
    ...
    async def test_concurrent_operations(self, memory_system):
        ...
        # Stores
        for i in range(20):
            tasks.append(memory_system.store({"concurrent": i}, "general"))

        # Retrieves
        for i in range(10):
            tasks.append(memory_system.retrieve("concurrent", "general"))
