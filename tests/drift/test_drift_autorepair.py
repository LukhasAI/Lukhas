...
class TestAutonomousDriftCorrection:
    ...
    def test_threshold_exceedance_triggers_repair(self):
        ...
        high_drift_ctx = {"top_symbols": ["ethical.compliance", "ethical.drift_score"], "probe": "test"}
        ...
        repair_events = [entry for entry in manager.drift_ledger if entry.get("event", "").startswith("auto_repair")]
        assert len(repair_events) > 0

        # Verify policy ledger line was emitted
        policy_events = [entry for entry in manager.drift_ledger if "rationale" in entry]
    ...
    def test_repair_method_selection(self):
        ...
        memory_result = repair_engine.reconsolidate("memory", 0.20, {}, ["memory.fold_stability", "memory.fold_count"])
        assert memory_result.method == RepairMethod.RECONSOLIDATE

        # Ethical drift should prefer realignment
        ethical_result = repair_engine.reconsolidate(
            "ethical", 0.20, {}, ["ethical.compliance", "ethical.constitutional"]
        )
        ...
        identity_result = repair_engine.reconsolidate(
            "identity", 0.20, {}, ["identity.coherence", "identity.namespace_integrity"]
        )
    ...
    def test_repair_effectiveness_by_severity(self):
        ...
        minor_result = repair_engine.reconsolidate(
            "ethical", 0.18, {}, ["ethical.compliance"]  # Just above critical (0.15)
        )

        # Severe drift should have lower repair effectiveness
        severe_result = repair_engine.reconsolidate(
            "ethical", 0.35, {}, ["ethical.compliance"]  # Well above severe (0.25)
        )
    ...
    def test_repair_rate_limiting(self):
        ...
        repair_attempts = [
            entry
            for entry in manager.drift_ledger
    ...
    def test_drift_reduction_cycle(self):
        ...
        manager.on_exceed("ethical", initial_score, {"top_symbols": drift_result["top_symbols"], "state": curr_state})

        # 5. Verify repair was attempted and succeeded
        successful_repairs = [entry for entry in manager.drift_ledger if entry.get("event") == "auto_repair_success"]
    ...
    def test_hybrid_repair_for_unified_drift(self):
        ...
        unified_result = repair_engine.reconsolidate(
            "unified", 0.30, {}, ["ethical.compliance", "memory.fold_stability", "identity.coherence"]
        )
    ...
    def test_repair_history_tracking(self):
        ...
        for i in range(3):
            repair_engine.reconsolidate("ethical", 0.20 + i * 0.05, {}, [f"ethical.test_symbol_{i}"])
    ...
    def test_emergency_rollback_for_severe_drift(self):
        ...
        severe_result = repair_engine.reconsolidate(
            "memory", 0.45, {}, ["memory.fold_collapse", "memory.critical_failure"]  # Very high drift
        )
    ...
    def test_namespace_change_repair_effectiveness(self):
        ...
        result = repair_engine.reconsolidate("identity", 0.20, {}, ["identity.namespace_change", "identity.coherence"])
    ...
    def test_repair_ledger_rationale(self):
        ...
        # Trigger repair
        manager.on_exceed("ethical", 0.25, {"top_symbols": ["ethical.compliance"], "test": "rationale"})

        # Check that ledger entries contain rationale
        repair_events = [entry for entry in manager.drift_ledger if entry.get("event") == "auto_repair_success"]
    ...
    def test_repair_without_engine_available(self):
        ...
        exceedance_events = [entry for entry in manager.drift_ledger if entry.get("event") == "threshold_exceeded"]
    ...
    def test_symbol_extraction_from_context(self):
        ...
        symbols = manager._extract_symbols_from_context(
            "ethical", {"state": {"compliance": 0.8, "constitutional": 0.7}}
        )
...
class TestAutorepairAcceptanceGates:
    ...
    def test_drift_reduction_percentage(self):
        ...
        for kind, score, symbols in test_cases:
            ...
            assert result.improvement_pct >= 20.0, f"Improvement {result.improvement_pct:.1f}% < 20% for {kind}"
    ...
    def test_repair_within_cycle_limit(self):
        ...
        while current_score > manager.critical_threshold and cycles < max_cycles:
            ...
            # Trigger repair
            manager.on_exceed("ethical", current_score, {"top_symbols": ["ethical.compliance"], "cycle": cycles})
        ...
        assert (
            current_score <= manager.critical_threshold
    ...
    def test_ledger_includes_repair_rationale(self):
        ...
        # Trigger repair
        manager.on_exceed("memory", 0.22, {"top_symbols": ["memory.fold_instability"], "test": "rationale_check"})
