...
from governance.identity.gateway.stargate_gateway import StargateGateway, GlyphPayload
...
async def test_hybrid_session_keys():
    ...
    for test_case in test_cases:
        ...
        payload = GlyphPayload(
            ...
            prompt_payload={
                ...
                "consent_types": ["biometric_processing", "consciousness_analysis", "symbolic_transmission"],
        ...
        if success:
            ...
            if session_data:
                ...
                print(f"   Internal Key (first 16 chars): {session_data['internal_session_key'][:16]}...")
                print(f"   Algorithm: {session_data['algorithm']}")
                print(f"   Key Length: {len(session_data['internal_session_key'])} chars")
                
                print(f"\n🛡️  Public Verification Hash:")
                print(f"   Hash: {session_data['public_verification_hash'][:32]}...")
                print(f"   Algorithm: SHAKE256")
                print(f"   Length: {len(session_data['public_verification_hash'])} chars")
                ...
                print(f"❌ Failed to get session audit data")
        else:
    ...
    if audit_path.exists():
        with open(audit_path, 'r') as f:
            lines = f.readlines()
            recent_lines = [line for line in lines[-20:] if "session_key" in line or "verification_hash" in line]
