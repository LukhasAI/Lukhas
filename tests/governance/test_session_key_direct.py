...
def test_session_key_generation():
    ...
    if payload.user_id in gateway.session_keys:
        ...
        if 'public_verification_hash' in session_data:
            ...
            print(f"   Hash (first 32 chars): {session_data['public_verification_hash'][:32]}...")
            print(f"   Full Length: {len(session_data['public_verification_hash'])} characters")
        ...
        print(f"\n✓ Retrieval Methods:")
        print(f"   get_public_verification_hash: {'Working' if public_hash else 'Failed'}")
