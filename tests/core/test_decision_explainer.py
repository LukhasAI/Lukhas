...
class TestDecisionExplainer:
    ...
    async def test_causal_explanation(self):
        ...
        ethical_factors = [
            f for f in explanation.factors if "ethical" in f.name.lower()
        ]
    ...
    async def test_comparative_explanation(self):
        ...
        outcome = {"selected_alternative": "a", "score": 0.8}
    ...
    def test_decision_report_generation(self):
        ...
        for i in range(5):
            factors = [
                DecisionFactor(
                    "Safety", "value", 0.7, "negative", "explanation", set()
                ),
                DecisionFactor(
                    "Efficiency",
                    "value",
    ...
    async def test_convenience_functions(self):
        ...
        context = {"decision_id": "test-007", "decision_type": "operational"}

        outcome = {"selected_alternative": "option_x", "confidence": 0.75}
    ...
    async def test_dmb_style_explanation(self):
        ...
        explainer = get_decision_explainer()
        explanation = await explainer.explain_decision(
            dmb_style_context, dmb_style_outcome
        )
        ...
        assert len(explanation.factors) > 0
        assert (
            "strategic" in str(explanation.relevant_tags).lower()
            or len(explanation.factors) > 0
        )
