    ...
    for module_info in pkgutil.iter_modules([str(adapter_path)]):
        if module_info.name.endswith('_adapter'):
            ...
            for attr_name in dir(module):
                ...
                if isinstance(attr, type) and issubclass(attr, MatrizNode) and (attr != MatrizNode):
...
def test_all_adapters_discoverable():
    ...
    expected_adapters = ['BioAdapter', 'MemoryAdapter', 'ConsciousnessAdapter', 'BridgeAdapter', 'GovernanceAdapter', 'EmotionAdapter', 'OrchestrationAdapter', 'ComplianceAdapter', 'IdentityAdapter', 'ContradictionAdapter', 'CreativeAdapter']
