...
def mock_services():
    ...
    services["consciousness_service"].make_decision = AsyncMock(
        return_value={
    ...
    services["memory_service"].search = AsyncMock(
        return_value=[
            {
                "id": "mem1",
    ...
    services["emotion_service"].get_current_state = AsyncMock(
        return_value={
    ...
    services["dream_engine"].generate_dream_sequence = AsyncMock(
        return_value={
            "dream_sequence": {
    ...
    services["parallel_reality_simulator"].create_simulation = AsyncMock(
        return_value=Mock(
            branches=[
                Mock(
                    probability=0.8,
                    divergence_point={"summary": "optimistic path"},
                ),
                Mock(
                    probability=0.6,
                    divergence_point={"summary": "neutral path"},
                ),
                Mock(
                    probability=0.4,
                    divergence_point={"summary": "challenging path"},
                ),
            ]
        )
    )
...
async def nl_interface(mock_services):
    ...
    with patch(
        "consciousness.interfaces.natural_language_interface.get_service"
    ) as mock_get:
...
class TestNaturalLanguageInterface:
    ...
    async def test_entity_extraction(self, nl_interface):
        ...
        # Dream topic
        entities = nl_interface._extract_entities(
            "Dream about space exploration", ConversationIntent.DREAM_REQUEST
        )
    ...
    async def test_awareness_query_processing(self, nl_interface):
        ...
        assert any(
            word in response.lower() for word in ["aware", "consciousness", "attention"]
        )
    ...
    async def test_decision_processing(self, nl_interface):
        ...
        assert (
            ...
            or "decision" in response.lower()
            or "option" in response.lower()
        )
        assert any(
    ...
    async def test_conversation_context(self, nl_interface):
        ...
        # First turn
        await nl_interface.process_input("How aware are you?", session_id=session_id)
        ...
        # Second turn
        await nl_interface.process_input(
            "And how do you feel about that?", session_id=session_id
        )
    ...
    async def test_tone_application(self, nl_interface):
        ...
        response = nl_interface._apply_tone(
            "Your request has been processed.", EmotionalTone.EMPATHETIC
        )
        assert response.startswith("I understand")

        # Test analytical tone
        response = nl_interface._apply_tone(
            "I think this is the best option.", EmotionalTone.ANALYTICAL
        )
        assert "analyze" in response

        # Test supportive tone
        response = nl_interface._apply_tone(
            "That's an interesting question.", EmotionalTone.SUPPORTIVE
        )
    ...
    async def test_conversation_manager(self, nl_interface):
        ...
        response = await manager.continue_conversation(
            session_id, "Tell me about your awareness"
        )
    ...
    async def test_session_cleanup(self, nl_interface):
        ...
        nl_interface.active_sessions[old_session] = ConversationContext(
            session_id=old_session, user_id="test"
        )
    ...
    async def test_confidence_calculation(self, nl_interface):
        ...
        confidence = nl_interface._calculate_confidence(
            "xyz", ConversationIntent.UNKNOWN, context
        )
    ...
    async def test_nlu_pipeline(self, nl_interface):
        ...
        nlu_result = await nl_interface._understand_input(
            "I'm feeling sad, can you help me decide what to do?", context
        )
