...
def mock_services():
    ...
    services["guardian_service"].validate_action = AsyncMock(return_value={
        "approved": True,
        "confidence": 0.85,
...
async def simulator(mock_services):
    ...
    with patch('consciousness.dream.parallel_reality_simulator.get_service') as mock_get:
        ...
        sim = ParallelRealitySimulator(config={
            "max_branches": 5,
            "max_depth": 3,
...
class TestParallelRealitySimulator:
    ...
    async def test_create_simulation(self, simulator):
        ...
        for branch in simulation.branches:
            ...
            assert branch.reality_type in [RealityType.QUANTUM, RealityType.CAUSAL]
    ...
    async def test_explore_branch(self, simulator):
        ...
        simulation = await simulator.create_simulation(
            origin_scenario={"test": "scenario"},
            branch_count=2
        )
        
        initial_branch_count = len(simulation.branches)
    ...
    async def test_merge_realities(self, simulator):
        ...
        # Merge branches
        merged = await simulator.merge_realities(
            simulation.simulation_id,
            branch_ids
        )
    ...
    async def test_ethical_validation(self, simulator, mock_services):
        ...
        mock_services["guardian_service"].validate_action = AsyncMock(return_value={
            "approved": False,
            "confidence": 0.2,
    ...
    async def test_performance_metrics(self, simulator):
        ...
        for _ in range(3):
            await simulator.create_simulation(
                origin_scenario={"metrics": "test"},
                branch_count=4
            )
        
        # Check metric updates
        assert simulator.metrics["simulations_created"] == initial_metrics["simulations_created"] + 3
        assert simulator.metrics["branches_explored"] > initial_metrics["branches_explored"]
