...
def _adapters_sandbox(monkeypatch):
    ...
    _stub(
        "googleapiclient.discovery", build=lambda *a, **k: types.SimpleNamespace(users=types.SimpleNamespace().__dict__)
    )
    ...
    _stub(
        ...
        Dropbox=lambda *a, **k: types.SimpleNamespace(
            files=types.SimpleNamespace(list_folder=lambda *a, **k: {"entries": []})
        ),
    )
    # HTTP layer guard
