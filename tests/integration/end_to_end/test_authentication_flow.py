...
def test_full_authentication_flow():
    ...
    try:
        ...
        ), f"Expected status code 200, but got {protected_response.status_code}. Response: {protected_response.text}"

        protected_response_json = protected_response.json()
        assert (
            protected_response_json["status"] == "success"
        ), "The status of the protected resource response was not 'success'"
