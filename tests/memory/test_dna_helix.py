...
from memory.dna_helix import (
    SymbolicStrand, DNAHealixCore, SymbolicRepairLoop,
    MemoryHelix, RepairMethod, RepairMetadata
)
...
class TestSymbolicStrand:
    ...
    def test_distance_calculation(self):
        ...
        # Cosine distance
        assert abs(strand1.distance_to(strand1, 'cosine')) < 1e-10  # Should be exactly 0
        assert strand1.distance_to(strand2, 'cosine') < strand1.distance_to(strand3, 'cosine')
...
class TestMemoryHelix:
    ...
    def test_memory_export_import(self):
        ...
        assert restored.helix_core.current.sequence == memory.helix_core.current.sequence
...
class TestHelixVault:
    ...
    async def test_drift_oracle(self, temp_vault_path):
        ...
        mem1 = vault.create_memory("low_drift", ["A", "B", "C"])
