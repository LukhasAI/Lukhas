...
class TestMemoryNode:
    ...
    def test_node_importance(self):
        ...
        # Low importance node
        low_node = MemoryNode(
            state=CognitiveState(salience=0.1, confidence=0.1)
        )
        
        # High importance node with connections
        high_node = MemoryNode(
            state=CognitiveState(salience=0.9, confidence=0.9)
        )
        high_node.links = [
...
class TestDNAHelixMemory:
    ...
    def test_trace_causal_chain(self):
        ...
        node2 = node1.evolve(
            {"version": 2},
            CognitiveState(confidence=0.6)
        )
        id2 = memory.add_node(node2)
        
        node3 = node2.evolve(
            {"version": 3},
            CognitiveState(confidence=0.8)
        )
        id3 = memory.add_node(node3)
