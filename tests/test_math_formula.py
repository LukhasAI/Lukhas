...
def test_mathematical_properties():
    ...
    print(f"Formula: z(t) = A * [e^(iθ) + e^(i(π-θ))] * W")
    print(f"Using: A=1.0, W=1.0\n")
    ...
    for theta, description in test_cases:
        ...
        identity_check = abs(z_result.real - manual_calc) < 1e-10 and abs(z_result.imag) < 1e-10
        print(f"  Identity check: {'✅' if identity_check else '❌'} (2*cos(θ-π/2) = {manual_calc:.6f})")
        print()
...
def find_good_baseline_cases():
    ...
    for i in range(13):  # Test θ from 0 to π in 12 steps
        ...
        print(f"θ = {theta:.4f} ({theta/math.pi:.2f}π): {z_result:.6f} ({'Simple' if is_simple else 'Complex'})")
