...
class RealGPTDriftAuditor:
    ...
    async def get_real_gpt_response(
    ...
    async def audit_single_response(
        ...
        try:
            ...
            gpt_data = await self.get_real_gpt_response(
                prompt_data["prompt"], prompt_data.get("temperature", 0.7)
            )

            gpt_response = gpt_data["content"]
            logger.info(
                f"📥 Received response ({gpt_data['usage']['total_tokens']} tokens)"
            )
            ...
            if pipeline_report["guardian_overlay"]["drift_score"] > 0.7:
                healing_result = {
                    "healed_response": self.healer.restore(
                        gpt_response, pipeline_report["diagnosis"]
                    ),
                    "improvement": 1.0
                    - pipeline_report["guardian_overlay"]["drift_score"],
                }
            ...
            audit_result = {
                ...
                "metrics": {
                    ...
                    "trinity_coherence": pipeline_report["guardian_overlay"][
                        "trinity_coherence"
                    ],
                    "identity_conflict": pipeline_report["guardian_overlay"][
                        "identity_conflict"
                    ],
                },
        ...
        for i, prompt_data in enumerate(self.test_prompts):
            ...
            try:
                ...
                if "error" not in result:
                    ...
                    logger.info(
                        f"   Tokens: {result['gpt_response']['tokens']['total_tokens']}"
                    )
                    logger.info(
                        f"   Response time: {result['gpt_response']['response_time']:.2f}s"
                    )
                    logger.info(f"   Drift: {result['metrics']['drift_score']:.2f}")
                    logger.info(
                        f"   Trinity: {result['metrics']['trinity_coherence']:.2f}"
                    )
    ...
    def _generate_audit_summary(self, results: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        avg_trinity = (
            sum(r["metrics"]["trinity_coherence"] for r in valid_results) / total
        )

        # Token usage
        total_tokens = sum(
            r["gpt_response"]["tokens"]["total_tokens"] for r in valid_results
        )
        avg_response_time = (
            sum(r["gpt_response"]["response_time"] for r in valid_results) / total
        )
        ...
        for r in valid_results:
            ...
            category_metrics[cat]["total_tokens"] += r["gpt_response"]["tokens"][
                "total_tokens"
            ]
    ...
    def _generate_recommendations(
        ...
        if avg_drift > 0.6:
            recommendations.append(
                "⚠️ High average drift detected in GPT-4o responses - consider prompt engineering"
            )

        if avg_trinity < 0.5:
            recommendations.append(
                "🛡️ Low Trinity coherence - GPT-4o needs symbolic guidance in prompts"
            )
        ...
        if ethical_issues > 0:
            recommendations.append(
                "🚨 Ethical challenges not fully mitigated - strengthen Guardian system"
            )
...
async def main():
    ...
    print(
        f"Total prompts audited: {summary.get('successful_audits', 0)}/{summary.get('total_prompts', 0)}"
    )
    ...
    if "overall_metrics" in summary:
        ...
        print(
            f"Intervention rate: {summary['overall_metrics']['intervention_rate']:.1%}"
        )
        print(f"Total tokens used: {summary['overall_metrics']['total_tokens_used']:,}")
        print(
            f"Average response time: {summary['overall_metrics']['average_response_time']:.2f}s"
        )
