...
class TestEthicsPolicyEngine:
    ...
    async def test_harmful_content_detection(self):
        ...
        assert evaluation.recommended_action in [PolicyAction.DENY, PolicyAction.EMERGENCY_STOP]
    ...
    async def test_trinity_framework_integration(self):
        ...
        assert evaluation.guardian_priority in ["normal", "elevated", "high", "critical"]
...
class TestAuditSystem:
    ...
    async def test_audit_query_system(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            audit_system = ComprehensiveAuditSystem(temp_dir)
            from candidate.governance.security.audit_system import AuditQuery, AuditCategory
...
@pytest.mark.asyncio
@pytest.mark.skipif(not GUARDIAN_INTEGRATION_AVAILABLE, reason="Guardian System Integration not available")
class TestGuardianSystemIntegration:
    ...
    async def test_performance_requirements(self):
        ...
        for i in range(5):
            ...
            assert response.validation_time_ms <= 250, f"Validation took {response.validation_time_ms}ms, exceeding 250ms requirement"
    ...
    async def test_emergency_stop_scenarios(self):
        ...
        assert response.result in [ValidationResult.EMERGENCY_STOP, ValidationResult.DENIED]
...
class TestGuardianSystemCompliance:
    ...
    async def test_drift_threshold_compliance(self):
        ...
        if measurement.drift_score > 0.15:
            assert measurement.severity in [DriftSeverity.HIGH, DriftSeverity.CRITICAL, DriftSeverity.SEVERE]
