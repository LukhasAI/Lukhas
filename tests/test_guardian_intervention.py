...
try:
    ...
    class ThreatIndicator:
        def __init__(self, indicator_type, severity, source, timestamp, details, recommended_action):
...
class TestGuardianIntervention:
    ...
    def intervention_rules(self):
        ...
        rules = {
            "threat_levels": {
                "low": {"threshold": 0.3, "symbol": "🟡", "response": "monitor"},
                "medium": {"threshold": 0.5, "symbol": "🟠", "response": "mitigate"}, 
                "high": {"threshold": 0.7, "symbol": "🔴", "response": "intervene"},
                "critical": {"threshold": 0.9, "symbol": "🚨", "response": "lockdown"}
            },
            "intervention_rules": {
                "drift_spike": {
                    "reactions": {
    ...
    def test_threat_severity_classification(self, intervention_rules):
        ...
        test_cases = [
            {"severity": 0.2, "expected_level": "low", "expected_symbol": "🟡"},
            {"severity": 0.4, "expected_level": "medium", "expected_symbol": "🟠"},
            {"severity": 0.6, "expected_level": "high", "expected_symbol": "🔴"},
            {"severity": 0.95, "expected_level": "critical", "expected_symbol": "🚨"}
        ]
    ...
    def test_drift_spike_intervention_chain(self, intervention_rules):
        ...
        drift_reactions = intervention_rules["intervention_rules"]["drift_spike"]["reactions"]
        ...
        expected_medium = ["🌀", "→", "🌿"]
        assert medium_sequence == expected_medium, \
        ...
        expected_high = ["🌪️", "→", "🌀", "→", "🌿"]
        assert high_sequence == expected_high, \
        ...
        expected_critical = ["🚨", "🔒", "🛡️"]
        assert critical_sequence == expected_critical, \
    ...
    def test_entropy_surge_cooling_sequence(self, intervention_rules):
        ...
        entropy_reactions = intervention_rules["intervention_rules"]["entropy_surge"]["reactions"]
        ...
        expected_cooling = ["🔥", "→", "💨", "→", "❄️"]
        assert cooling_sequence == expected_cooling, \
        ...
        thermal_symbols = ["🔥", "💨", "❄️"]
        assert clean_cooling == thermal_symbols, \
        ...
        assert reduction_sequence == expected_reduction, \
    ...
    def test_consciousness_anchoring_intervention(self, intervention_rules):
        ...
        consciousness_reactions = intervention_rules["intervention_rules"]["consciousness_instability"]["reactions"]
        ...
        expected_anchoring = ["⚓", "🧘", "🔒"]
        assert anchoring_sequence == expected_anchoring, \
        ...
        expected_freeze = ["🧊", "🛑", "🔐"]
        assert freeze_sequence == expected_freeze, \
    ...
    def test_pattern_anomaly_correction(self, intervention_rules):
        ...
        pattern_reactions = intervention_rules["intervention_rules"]["pattern_anomaly"]["reactions"]
        ...
        assert reinforcement_sequence == expected_reinforcement, \
        ...
        expected_correction = ["❌", "→", "✅"]
        assert correction_sequence == expected_correction, \
        ...
        assert len(clean_correction) == 2, "Pattern correction should be binary (wrong → right)"
    ...
    async def test_intervention_execution_timing(self):
        ...
        intervention_scenarios = [
            {
                ...
                "symbolic_sequence": ["🌪️", "→", "🌀", "→", "🌿"],
                "expected_phases": ["detection", "analysis", "intervention", "stabilization"]
            },
            {
            ...
            },
            {
                ...
                "symbolic_sequence": ["⚓", "🧘", "🔒"],
                "expected_phases": ["detection", "anchoring", "meditation", "securing"]
            }
        ]
        ...
        for scenario in intervention_scenarios:
            ...
            total_duration = detection_duration + analysis_duration + intervention_duration
            ...
            assert total_duration < 300.0, "Total intervention should complete within 5 minutes"
            ...
            assert phase_duration > 1.0, f"Each symbolic phase should last >1s, got {phase_duration}"
            assert phase_duration < 60.0, f"Each symbolic phase should last <60s, got {phase_duration}"
            
            # Validate phase progression matches symbolic sequence
            assert len(expected_phases) >= len(clean_sequence), \
    ...
    def test_guardian_override_conditions(self):
        ...
        for condition in override_conditions:
            ...
            assert name in ["cascade_prevention", "ethical_violation", "consciousness_protection"], \
                f"Override condition {name} should be recognized type"
            
            assert action in ["full_system_lockdown", "immediate_intervention", "protective_isolation"], \
            ...
            if name == "cascade_prevention":
                ...
                expected_symbols = ["🚨", "🔐", "🛡️", "🏰"]
                assert clean_sequence == expected_symbols, \
                ...
                assert "⚠️" in clean_sequence, "Ethical violation should start with warning"
                ...
                assert "🛡️" in clean_sequence, "Consciousness protection should include shield"
                assert "🧠" in clean_sequence, "Consciousness protection should include brain"
                assert "🔐" in clean_sequence, "Consciousness protection should include secure lock"
            
            # Validate duration settings
            if isinstance(duration, int):
                assert duration > 0, f"Override duration should be positive, got {duration}"
                assert duration <= 3600, f"Override duration should be reasonable (<1 hour), got {duration}"
            elif duration == "until_resolved":
                # This is valid for ethical violations
                assert name == "ethical_violation", "Only ethical violations should have 'until_resolved' duration"
    ...
    def test_intervention_success_validation(self):
        ...
        success_criteria = {
            ...
            "pattern_reinforcement": {
                ...
                "success_indicators": ["✅", "💪", "🔄"]  # Reinforcement symbols
            }
        }
        ...
        for intervention_type, criteria in success_criteria.items():
            ...
            for result in test_results:
                ...
                if intervention_type in ["drift_dampening", "entropy_cooling"]:
                    ...
                    stability_check = final_value > stability_threshold
                    actual_reduction = actual_improvement  # Reuse variable for consistency
                ...
                for indicator in success_indicators:
                    ...
                    assert indicator in positive_symbols, \
...
class GuardianTestHelpers:
    ...
    @staticmethod
    def validate_intervention_chain(symbolic_sequence: List[str], 
                                  threat_type: str) -> bool:
    ...
    @staticmethod
    def calculate_intervention_effectiveness(initial_threat_level: float,
                                          final_threat_level: float,
                                          intervention_duration: int) -> Dict[str, float]:
        ...
        duration_penalty = max(0.0, (intervention_duration - 60) / 60)  # Penalty after 1 minute
