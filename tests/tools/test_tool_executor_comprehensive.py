...
from candidate.tools.tool_orchestrator import ToolOrchestrator, MultiAIConsensus
...
class TestToolExecutor:
    ...
    async def test_sandboxed_execution_success(self, mock_docker_client):
        ...
        with patch.dict(os.environ, {"LUKHAS_ENABLE_CODE_EXEC": "true"}):
            executor = ToolExecutor()
            result = await executor.execute("exec_code", '{"language": "python", "source": "print(\\"Hello World\\")"}')
