...
pytestmark = pytest.mark.skipif(
    not MEMORY_AVAILABLE, reason="Memory systems (EmbeddingIndex, IndexManager) not available"
)
...
def test_memory_embedding_index_stores_vectors(client, auth_headers):
    ...
    response = client.post(
        "/v1/embeddings",
        json={"input": "Knowledge to remember: LUKHAS uses MATRIZ", "store": True},  # Request storage in index
...
def test_memory_embedding_index_retrieval(client, auth_headers):
    ...
    r1 = client.post(
        "/v1/embeddings",
        json={"input": "Paris is the capital of France", "store": True, "metadata": {"topic": "geography"}},
        headers=auth_headers,
    )
    ...
    r2 = client.post(
        "/v1/embeddings",
        json={"input": "What is the capital of France?", "retrieve_similar": True, "top_k": 3},
        headers=auth_headers,
    )
...
def test_memory_vector_similarity_search(client, auth_headers):
    ...
    for topic in topics:
        response = client.post("/v1/embeddings", json={"input": topic, "store": True}, headers=auth_headers)
        assert response.status_code == 200

    # Search for similar content
    response = client.post(
        "/v1/embeddings",
        json={"input": "What is deep learning?", "retrieve_similar": True, "top_k": 2},
        headers=auth_headers,
    )
...
def test_memory_index_manager_tenant_isolation(client, auth_headers):
    ...
    r1 = client.post("/v1/embeddings", json={"input": "Tenant A secret data", "store": True}, headers=tenant_a_headers)
    assert r1.status_code == 200
    ...
    r2 = client.post(
        "/v1/embeddings",
        json={"input": "Tenant A secret data", "retrieve_similar": True},  # Same query
...
def test_memory_context_preservation_across_requests(client, auth_headers):
    ...
    r1 = client.post(
        "/v1/responses",
        json={"input": "My project is called Phoenix", "context": {"session_id": session_id, "save_context": True}},
        headers=auth_headers,
    )
    assert r1.status_code == 200
    ...
    r2 = client.post(
        "/v1/responses",
        json={"input": "What is my project called?", "context": {"session_id": session_id, "load_context": True}},
        headers=auth_headers,
    )
    assert r2.status_code == 200
...
def test_memory_context_handoff_performance(client, auth_headers):
    ...
    r2 = client.post(
        "/v1/responses",
        json={"input": "Retrieve my preferences", "context": {"session_id": session_id, "load_context": True}},
        headers=auth_headers,
    )
...
def test_memory_long_term_storage(client, auth_headers):
    ...
    r2 = client.post(
        "/v1/embeddings", json={"input": "When was LUKHAS founded?", "retrieve_similar": True}, headers=auth_headers
    )
    assert r2.status_code == 200
...
def test_memory_working_memory_capacity(client, auth_headers):
    ...
    response = client.post(
        "/v1/responses",
        json={"input": "What items do you remember?", "context": {"session_id": session_id, "working_memory": True}},
        headers=auth_headers,
    )
    assert response.status_code == 200
...
def test_memory_working_memory_eviction(client, auth_headers):
    ...
    for i in range(15):
        response = client.post(
            "/v1/responses",
            json={"input": f"Item {i}", "context": {"session_id": session_id, "working_memory": True}},
            headers=auth_headers,
        )
        assert response.status_code == 200
...
def test_memory_episodic_memory_timeline(client, auth_headers):
    ...
    events = ["First, I woke up", "Then, I had breakfast", "After that, I went to work", "Finally, I came home"]

    # Store episodic sequence
    for event in events:
        response = client.post(
            "/v1/responses",
            json={"input": event, "context": {"session_id": session_id, "memory_type": "episodic"}},
            headers=auth_headers,
        )
        assert response.status_code == 200
    ...
    response = client.post(
        ...
        json={
            ...
            "context": {"session_id": session_id, "memory_type": "episodic"},
        },
        headers=auth_headers,
    )
    assert response.status_code == 200
...
def test_memory_episodic_memory_recall(client, auth_headers):
    ...
    r1 = client.post(
        ...
        json={
            ...
            "context": {"session_id": session_id, "memory_type": "episodic", "timestamp": "2024-01-15T10:30:00Z"},
        },
        headers=auth_headers,
    )
    assert r1.status_code == 200
    ...
    r2 = client.post(
        "/v1/responses",
        json={"input": "Who did I meet yesterday?", "context": {"session_id": session_id, "memory_type": "episodic"}},
        headers=auth_headers,
    )
    assert r2.status_code == 200
...
def test_memory_semantic_memory_facts(client, auth_headers):
    ...
    facts = ["The Earth orbits the Sun", "Water boils at 100°C", "Python is a programming language"]

    for fact in facts:
        response = client.post(
            "/v1/embeddings",
            json={"input": fact, "store": True, "metadata": {"type": "semantic_fact"}},
            headers=auth_headers,
        )
        assert response.status_code == 200
    ...
    response = client.post(
        "/v1/embeddings",
        json={"input": "What is Python?", "retrieve_similar": True, "metadata_filter": {"type": "semantic_fact"}},
        headers=auth_headers,
    )
    assert response.status_code == 200
...
def test_memory_semantic_memory_concepts(client, auth_headers):
    ...
    response = client.post(
        "/v1/embeddings",
        json={"input": "Consciousness is the state of awareness", "store": True, "metadata": {"type": "concept"}},
        headers=auth_headers,
    )
...
def test_memory_consolidation_short_to_long_term(client, auth_headers):
    ...
    r1 = client.post(
        ...
        json={
            ...
            "context": {"session_id": session_id, "working_memory": True, "consolidate": True},
        },
        headers=auth_headers,
    )
    assert r1.status_code == 200
    ...
    r2 = client.post(
        "/v1/responses",
        json={"input": "What is MATRIZ?", "context": {"session_id": session_id, "use_long_term": True}},
        headers=auth_headers,
    )
    assert r2.status_code == 200
...
def test_memory_consolidation_importance_weighting(client, auth_headers):
    ...
    r1 = client.post(
        ...
        json={
            ...
            "context": {"session_id": session_id, "importance": 10, "consolidate": True},
        },
        headers=auth_headers,
    )
    assert r1.status_code == 200
    ...
    r2 = client.post(
        "/v1/responses",
        json={"input": "Random thought: clouds are fluffy", "context": {"session_id": session_id, "importance": 1}},
        headers=auth_headers,
    )
    assert r2.status_code == 200
...
def test_memory_retrieval_by_metadata(client, auth_headers):
    ...
    r1 = client.post(
        ...
        json={
            ...
            "store": True,
            "metadata": {"project": "alpha", "status": "completed", "date": "2024-01-01"},
        },
        headers=auth_headers,
    )
    assert r1.status_code == 200
    ...
    r2 = client.post(
        "/v1/embeddings",
        json={"input": "project status", "retrieve_similar": True, "metadata_filter": {"project": "alpha"}},
        headers=auth_headers,
    )
    assert r2.status_code == 200
...
def test_memory_retrieval_by_timestamp(client, auth_headers):
    ...
    response = client.post(
        "/v1/embeddings",
        json={"input": "Event from last week", "store": True, "metadata": {"timestamp": "2024-01-08T12:00:00Z"}},
        headers=auth_headers,
    )
    assert response.status_code == 200
    ...
    response = client.post(
        "/v1/embeddings",
        json={"input": "recent events", "retrieve_similar": True, "time_range": {"after": "2024-01-01"}},
        headers=auth_headers,
    )
    assert response.status_code == 200
...
def test_memory_retrieval_relevance_scoring(client, auth_headers):
    ...
    items = ["Dogs are mammals", "Cats are mammals", "Birds are not mammals"]

    for item in items:
        response = client.post("/v1/embeddings", json={"input": item, "store": True}, headers=auth_headers)
        assert response.status_code == 200

    # Retrieve most relevant
    response = client.post(
        "/v1/embeddings",
        json={"input": "Are cats mammals?", "retrieve_similar": True, "top_k": 3, "return_scores": True},
        headers=auth_headers,
    )
    assert response.status_code == 200
...
def test_memory_index_lookup_performance(client, auth_headers):
    ...
    # Store item
    r1 = client.post("/v1/embeddings", json={"input": "Performance test item", "store": True}, headers=auth_headers)
    assert r1.status_code == 200

    # Measure lookup time
    start = time.time()
    r2 = client.post(
        "/v1/embeddings", json={"input": "Performance test item", "retrieve_similar": True}, headers=auth_headers
    )
