...
from orchestration.signals import (
    Signal, SignalType, SignalPattern, SignalBus, get_signal_bus,
    SystemEvent, ModulationParams, HomeostasisController,
    PromptModulator, AdaptiveModulator, PromptStyle
)
from feedback import (
    FeedbackRating, FeedbackCard, PatternSet, 
    PolicyUpdate, FeedbackCardSystem
)
...
def test_env_validator():
    ...
    try:
        ...
        assert result == "valid_test_value", f"Expected 'valid_test_value', got {result}"
...
async def test_homeostasis_comprehensive():
    ...
    try:
        ...
        for sig in regulated:
            assert 0.0 <= sig.level <= 1.0, f"Signal {sig.name} not bounded: {sig.level}"
        ...
        assert modulation.reasoning_effort > 0.5, "Reasoning not increased for ambiguity"
        print_success(f"Modulation computation: temp={modulation.temperature:.2f}, reasoning={modulation.reasoning_effort:.2f}")
        
        # Test 6: Audit trail
        audit_id = controller.audit_trails[-1].audit_id if controller.audit_trails else None
        if audit_id:
...
def test_modulator_comprehensive():
    ...
    try:
        ...
        assert "clarification" in modulation.modulated_prompt.lower(), "Clarification not added"
        assert modulation.style == PromptStyle.BALANCED or modulation.style == PromptStyle.STRICT
        ...
        assert risk_modulation.style == PromptStyle.STRICT, f"Expected STRICT, got {risk_modulation.style}"
        assert "safety" in risk_modulation.system_preamble.lower(), "Safety context not added"
        print_success("High risk modulation working")
        
        # Test 3: Creative modulation
        creative_signals = [
            Signal(name=SignalType.NOVELTY, level=0.8, source="test")
        ]
        
        creative_modulation = modulator.modulate("Generate ideas", creative_signals)
        assert creative_modulation.style == PromptStyle.CREATIVE, f"Expected CREATIVE, got {creative_modulation.style}"
        assert "creative" in creative_modulation.modulated_prompt.lower(), "Creative encouragement not added"
        print_success("Creative modulation working")
        
        # Test 4: Urgency modulation
        urgent_signals = [
            Signal(name=SignalType.URGENCY, level=0.9, source="test")
        ]
        
        urgent_modulation = modulator.modulate("Help me", urgent_signals)
        assert "[Time-sensitive]" in urgent_modulation.modulated_prompt, "Urgency prefix not added"
        assert urgent_modulation.api_params.max_output_tokens < 1024, "Output not reduced for urgency"
...
def test_adaptive_modulator():
    ...
    try:
        ...
        assert all(w == 1.0 for w in initial_weights.values()), "Initial weights not 1.0"
        ...
        strict_signals = [Signal(name=SignalType.ALIGNMENT_RISK, level=0.5, source="test")]
        ...
        assert conservative_weight < 1.0, f"Conservative weight not decreased: {conservative_weight}"
        print_success(f"Weight reduction working: conservative={conservative_weight:.2f}")
        ...
        assert final_weight > new_weight, "Weight not increasing with multiple successes"
...
def test_feedback_system_comprehensive():
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            print_success(f"Multiple feedback captured: {len(feedback_system.feedback_cards)} cards")
            ...
            for adjustment in policy_update.parameter_adjustments.values():
                assert abs(adjustment) <= 0.2, f"Adjustment exceeds bounds: {adjustment}"
            ...
            print_success(f"Learning report: satisfaction={report.overall_satisfaction:.2f}")
            
            # Test 8: Symbol tracking
            card_with_symbol = feedback_system.capture_feedback(
...
async def test_full_integration():
    ...
    try:
        ...
        event_context = {"text": "This is urgent! Help me understand this complex topic."}
        ...
        assert prompt_modulation.style in [PromptStyle.STRICT, PromptStyle.BALANCED]
        # Reasoning effort should be adjusted (not necessarily > 0.5 due to urgency)
        assert prompt_modulation.api_params.reasoning_effort >= 0.2  # More flexible check
        ...
        print(f"  Events processed: {final_metrics['controller_metrics']['events_processed']}")
        print(f"  Feedback captured: {final_metrics['feedback_metrics']['cards_captured']}")
...
async def main():
    ...
    for name, result in results:
        status = f"{TestColors.GREEN}✅ PASSED{TestColors.RESET}" if result else f"{TestColors.RED}❌ FAILED{TestColors.RESET}"
    ...
    if failed == 0:
        ...
        print(f"{TestColors.RED}{TestColors.BOLD}⚠️  Some tests failed{TestColors.RESET}")
        return 1
