...
def maybe_statically_parse_project_metadata(
    ...
    if "project" not in pyproject_contents or "name" not in pyproject_contents["project"]:
        return None
    ...
    install_requirements = [
        InstallRequirement(Requirement(req), comes_from) for req in project_table.get("dependencies", [])
    ]
    for extra, reqs in pyproject_contents.get("project", {}).get("optional-dependencies", {}).items():
        for req in reqs:
            ...
            install_requirements.append(InstallRequirement(requirement, comes_from, markers=marker))
