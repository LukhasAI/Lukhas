...
def _msvc14_find_vcvarsall(plat_spec):
    ...
    if best_dir:
        vcredist = join(
            best_dir, "..", "..", "redist", "MSVC", "**", vcruntime_plat, "Microsoft.VC14*.CRT", "vcruntime140.dll"
        )
        try:
            import glob
    ...
    def return_env(self, exists=True):
        ...
        env = dict(
            include=self._build_paths(
                "include", [self.VCIncludes, self.OSIncludes, self.UCRTIncludes, self.NetFxSDKIncludes], exists
            ),
            lib=self._build_paths(
                "lib",
                [self.VCLibraries, self.OSLibraries, self.FxTools, self.UCRTLibraries, self.NetFxSDKLibraries],
                exists,
            ),
            libpath=self._build_paths(
                "libpath", [self.VCLibraries, self.FxTools, self.VCStoreRefs, self.OSLibpath], exists
            ),
