...
class ConfigHandler:
        ...
        for value in values:
            ...
            if any(char in value for char in glob_characters):
                ...
                expanded_values.extend(
                    sorted(os.path.relpath(path, os.getcwd()) for path in iglob(os.path.abspath(value)))
                )
    ...
    def _parse_file(cls, value):
        ...
        return "\n".join(
            cls._read_file(path) for path in filepaths if (cls._assert_local(path) or True) and os.path.isfile(path)
        )
        ...
        for section_name, section_options in self.sections.items():
            ...
            if section_parser_method is None:
                raise DistutilsOptionError(
                    "Unsupported distribution option section: [%s.%s]" % (self.section_prefix, section_name)
                )
