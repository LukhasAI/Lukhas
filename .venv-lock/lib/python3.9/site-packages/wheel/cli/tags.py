...
def tags(
    ...
    if platforms != set(original_plat_tags):
        msg = f"Wheel internal tags {platforms!r} != filename tags {original_plat_tags!r}"
        raise AssertionError(msg)

    if existing_build_tag != build:
        msg = f"Incorrect filename '{build}' " f"& *.dist-info/WHEEL '{existing_build_tag}' build numbers"
        raise AssertionError(msg)
    ...
    if original_wheel_name != final_wheel_name:
        ...
        for a, b, c in itertools.product(final_python_tags, final_abi_tags, final_plat_tags):
            info["Tag"] = f"{a}-{b}-{c}"
        if build:
            info["Build"] = build

        original_wheel_path = os.path.join(os.path.dirname(f.filename), original_wheel_name)
        final_wheel_path = os.path.join(os.path.dirname(f.filename), final_wheel_name)

        with WheelFile(original_wheel_path, "r") as fin, WheelFile(final_wheel_path, "w") as fout:
