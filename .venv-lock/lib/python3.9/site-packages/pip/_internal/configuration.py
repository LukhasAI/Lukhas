...
def _disassemble_key(name: str) -> list[str]:
    if "." not in name:
        error_message = (
            "Key does not contain dot separated section and key. " f"Perhaps you wanted to use 'global.{name}' instead?"
        )
...
class Configuration:
    ...
    def __init__(self, isolated: bool, load_only: Kind | None = None) -> None:
        ...
        self._config: dict[Kind, dict[str, dict[str, Any]]] = {variant: {} for variant in OVERRIDE_ORDER}
    ...
    def unset_value(self, key: str) -> None:
        ...
        if key not in self._config[self.load_only][fname] and key not in self._config[self.load_only]:
            raise ConfigurationError(f"No such key - {orig_key}")
    ...
    def save(self) -> None:
        ...
        for fname, parser in self._modified_parsers:
            ...
            try:
                ...
                raise ConfigurationError(
                    f"An error occurred while writing to the configuration file " f"{fname}: {error}"
                )
    ...
    def _load_environment_vars(self) -> None:
        ...
        self._config[kinds.ENV_VAR][":env:"].update(self._normalized_keys(":env:", self.get_environ_vars()))
