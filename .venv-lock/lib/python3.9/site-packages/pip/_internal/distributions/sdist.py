...
class SourceDistribution(AbstractDistribution):
    ...
    def _prepare_build_backend(self, build_env_installer: BuildEnvironmentInstaller) -> None:
    ...
    def _install_build_reqs(self, build_env_installer: BuildEnvironmentInstaller) -> None:
        ...
        if conflicting:
            self._raise_conflicts("the backend dependencies", conflicting)
        self.req.build_env.install_requirements(missing, "normal", kind="backend dependencies", for_req=self.req)
    ...
    def _raise_conflicts(self, conflicting_with: str, conflicting_reqs: set[tuple[str, str]]) -> None:
        ...
        error_message = format_string.format(
            ...
            description=", ".join(
                f"{installed} is incompatible with {wanted}" for installed, wanted in sorted(conflicting_reqs)
            ),
        )
