...
2. Validate configuration: `python -c "import memory; print('OK')"`
3. Review logs in `logs/memory.log`
...
```bash
# Check module health
python -m memory --health-check

# Validate configuration
python -m memory --validate-config

# Test connections
python -m memory --test-connections

# Generate diagnostic report
python -m memory --diagnostic-report
```
