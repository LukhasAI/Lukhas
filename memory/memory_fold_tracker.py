...
class MemoryFoldTracker:
    ...
    def _detect_collapses(self, sessions: List[Dict]) -> List[Dict]:
        ...
        for i in range(1, len(sessions)):
            ...
            entropy_spike = (
                curr.get('entropy', 0) > self.collapse_indicators['entropy_spike'] and
                curr.get('entropy', 0) > prev.get('entropy', 0) + 0.3
            )
            ...
            if entropy_spike or trinity_collapse or drift_critical:
                collapses.append({
                    ...
                    "type": "entropy_spike" if entropy_spike else 
                           "trinity_collapse" if trinity_collapse else "drift_critical",
                    "metrics": {
    ...
    def _assess_recursion_risk(self, glyph_rec: List, persona_rec: List,
                              collapses: List, voids: List) -> Dict[str, Any]:
