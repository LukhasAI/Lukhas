...
class IntentRouter:
    ...
    def _initialize_patterns(self) -> Dict[IntentType, List[Tuple[str, float]]]:
        ...
        return {
            IntentType.QUERY_MEMORY: [
                (r"(show|find|search|get|retrieve)\s+(memory|memories|fold|folds)", 0.9),
                (r"what do you remember about", 0.8),
                (r"recall.*about", 0.7),
                (r"memory.*(?:of|about|regarding)", 0.8)
            ],
            IntentType.EXPLORE_CONSCIOUSNESS: [
                (r"(explore|show|display)\s+(consciousness|awareness|state)", 0.9),
            ...
            IntentType.ANALYZE_TRINITY: [
                ...
                (r"(?:identity|consciousness|guardian)\s+(?:status|health)", 0.7)
            ],
            IntentType.REQUEST_INTERVENTION: [
                (r"(?:need|require|request)\s+(?:help|intervention|guardian)", 0.9),
    ...
    def _extract_parameters(self, text: str, intent_type: IntentType, match: re.Match) -> Dict[str, Any]:
        ...
        modules = ["consciousness", "memory", "guardian", "identity", "quantum", "emotion"]
