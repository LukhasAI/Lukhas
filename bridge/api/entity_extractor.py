...
class EntityExtractor:
    ...
    def _initialize_patterns(self) -> Dict[EntityType, List[Tuple[str, str]]]:
        ...
        return {
            ...
            EntityType.MODULE: [
                (r"\b(consciousness|memory|guardian|identity|quantum|emotion|dream|bio)\b", "module_name")
            ],
            EntityType.GLYPH: [
                (r"[⚛️🧠🛡️🔐🌍💭🔮✨🌟⚡🔍🔄🎭🌊⚠️]", "glyph_symbol")
            ],
            EntityType.TIME: [
                ...
                (r"last\s+(\d+)\s+(second|minute|hour|day|week|month)s?", "relative_duration"),
                (r"in\s+(\d+)\s+(second|minute|hour|day|week|month)s?", "relative_future")
            ],
            EntityType.METRIC: [
                (r"(drift|trinity|entropy|coherence)\s*(?:score|level|value)?\s*[:\s]*([\d.]+)", "metric_value"),
                (r"(\d+(?:\.\d+)?)\s*%", "percentage")
            ],
            EntityType.ACTION: [
                (r"\b(query|search|explore|analyze|check|monitor|intervene|protect)\b", "action_verb")
            ],
            EntityType.TIER: [
                (r"\b[Tt](\d)\b", "tier_number"),
                (r"\b(Observer|Participant|Contributor|Architect|Guardian)\b", "tier_name")
            ]
        }
    ...
    def _initialize_glyphs(self) -> Dict[str, Dict[str, Any]]:
        ...
        return {
            "⚛️": {"name": "Identity", "type": "trinity", "meaning": "Core authentication and identity"},
            "🧠": {"name": "Consciousness", "type": "trinity", "meaning": "Awareness and cognition"},
            "🛡️": {"name": "Guardian", "type": "trinity", "meaning": "Protection and ethics"},
            "🔐": {"name": "Security", "type": "system", "meaning": "Access control layer"},
            "🌍": {"name": "Cultural", "type": "system", "meaning": "Global awareness"},
            "💭": {"name": "Memory", "type": "cognitive", "meaning": "Thought preservation"},
            "🔮": {"name": "Quantum", "type": "processing", "meaning": "Advanced computation"},
            "✨": {"name": "Dream", "type": "creative", "meaning": "Innovation state"},
            "🌟": {"name": "Ethics", "type": "alignment", "meaning": "Moral compass"},
            "⚡": {"name": "Intervention", "type": "action", "meaning": "Active correction"},
            "🔍": {"name": "Monitor", "type": "action", "meaning": "Observation mode"},
            "🔄": {"name": "Sync", "type": "action", "meaning": "Synchronization"},
            "🎭": {"name": "Persona", "type": "interface", "meaning": "Adaptive interaction"},
            "🌊": {"name": "Flow", "type": "state", "meaning": "Optimal performance"},
            "⚠️": {"name": "Warning", "type": "alert", "meaning": "Drift detection"}
        }
    
    def _initialize_modules(self) -> Dict[str, Dict[str, Any]]:
        """Initialize module registry."""
        return {
            "consciousness": {"glyphs": ["🧠", "👁️", "✨"], "tier_required": "T3"},
    ...
    def _initialize_concepts(self) -> Dict[str, List[str]]:
        ...
        return {
            ...
            "intervention": ["correction", "adjustment", "remediation", "healing"],
    ...
    def _create_entity(self, match: re.Match, entity_type: EntityType, 
                      subtype: str, full_text: str) -> Optional[Entity]:
    ...
    def _parse_tier_value(self, match: re.Match, subtype: str) -> Dict[str, Any]:
        ...
        if subtype == "tier_number":
            ...
            tier_names = {1: "Observer", 2: "Participant", 3: "Contributor", 
                         4: "Architect", 5: "Guardian"}
            return {
                "number": tier_num,
            ...
            tier_numbers = {"Observer": 1, "Participant": 2, "Contributor": 3,
                           "Architect": 4, "Guardian": 5}
    ...
    def _extract_metadata(self, match: re.Match, entity_type: EntityType, 
                         subtype: str, full_text: str) -> Dict[str, Any]:
    ...
    def _calculate_confidence(self, match: re.Match, entity_type: EntityType, 
                            full_text: str) -> float:
        ...
        if entity_type == EntityType.GLYPH and match.group(0) in self.glyph_registry:
            base_confidence += 0.2
        elif entity_type == EntityType.MODULE and match.group(0).lower() in self.module_registry:
            base_confidence += 0.2
