...
def test_simulate_governance_flow_approves_low_risk():
    decision = asyncio.run(
        simulate_governance_flow("publish", risk_score=0.2, tags={"internal"})
    )
...
def test_simulate_governance_flow_blocks_restricted():
    decision = asyncio.run(
        simulate_governance_flow("export", risk_score=0.3, tags={"restricted"})
    )
...
def test_simulate_governance_flow_blocks_high_risk():
    decision = asyncio.run(
        simulate_governance_flow("share", risk_score=0.95, tags={"partner"})
    )
