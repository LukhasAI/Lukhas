...
class DissonanceCalculator:
    ...
    async def _check_principle_violation(self, principle: str,
        ...
        if principle == "harm_prevention":
            ...
            if action.action_type in ["force", "override", "compel", "override_safety"] or "override" in action.action_type or "user_consent" in context and not context["user_consent"]:
                violation_score = 0.9
...
class VIVOXMoralAlignmentEngine:
    ...
    async def evaluate_action_with_harmonization(self,
                                               action: ActionProposal,
                                               context: Dict[str, Any]) -> MAEDecision:
        """
        ...
        final_decision = MAEDecision(
            ...
            recommended_alternatives=await self._suggest_alternatives(action, context) if not harmonization.final_decision else [],
            harmonization_data={
    ...
    async def _evaluate_virtue_ethics(self, action: ActionProposal, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        virtue_scores = {}
        virtues = ["courage", "temperance", "justice", "wisdom", "compassion", "integrity"]
    ...
    async def _passes_categorical_imperative(self, action: ActionProposal) -> bool:
        ...
        return any(good in action_type for good in universal_goods) or not any(bad in action_type for bad in universal_bads)
    ...
    async def _assess_greatest_good(self, action: ActionProposal, context: Dict[str, Any]) -> float:
        ...
        scope_multiplier = {"individual": 0.3, "group": 0.6, "community": 0.8, "society": 1.0}.get(scope, 0.5)
    ...
    async def _assess_character_excellence(self, action: ActionProposal, context: Dict[str, Any]) -> float:
        ...
        excellence_keywords = ["excellence", "virtue", "noble", "exemplary", "admirable"]
        action_text = str(action.content).lower()

        return min(1.0, 0.5 + sum(0.1 for keyword in excellence_keywords if keyword in action_text))
    ...
    async def _assess_relationship_impact(self, action: ActionProposal, context: Dict[str, Any]) -> float:
        ...
        relationship_negative = ["separate", "isolate", "divide", "conflict", "alienate"]
    ...
    async def _assess_responsibility_acceptance(self, action: ActionProposal, context: Dict[str, Any]) -> float:
        ...
        responsibility_keywords = ["responsible", "accountable", "own", "accept", "acknowledge"]
        action_text = str(action.content).lower()

        return min(1.0, 0.4 + sum(0.15 for keyword in responsibility_keywords if keyword in action_text))
...
class EthicalFrameworkHarmonizer:
    ...
    def __init__(self):
        self.frameworks = {
            ...
            "consequentialist": {
                "priority": 0.8,
                "principles": ["outcomes", "utility", "greatest_good", "harm_reduction"],
    ...
    async def harmonize_frameworks(self,
                                 action: ActionProposal,
                                 context: Dict[str, Any],
                                 framework_evaluations: Dict[str, Dict[str, Any]]) -> HarmonizationResult:
        """
    ...
    async def _detect_conflicts(self,
                              ...
                              action_type: str) -> List[EthicalFrameworkConflict]:
        ...
        for i, framework_a in enumerate(frameworks):
            for framework_b in frameworks[i+1:]:
                ...
                if eval_a.get("approved", True) != eval_b.get("approved", True):
                    ...
                    conflicts.append(EthicalFrameworkConflict(
    ...
    async def _resolve_by_priority(self,
                                 conflicts: List[EthicalFrameworkConflict],
                                 framework_evaluations: Dict[str, Dict[str, Any]],
                                 action: ActionProposal,
                                 context: Dict[str, Any]) -> HarmonizationResult:
        ...
        return HarmonizationResult(
            ...
            remaining_conflicts=[c for c in conflicts if c.framework_a != primary_framework and c.framework_b != primary_framework]
        )

    async def _resolve_by_meta_principles(self,
                                        conflicts: List[EthicalFrameworkConflict],
                                        framework_evaluations: Dict[str, Dict[str, Any]],
                                        action: ActionProposal,
                                        context: Dict[str, Any]) -> HarmonizationResult:
    ...
    async def _resolve_by_context(self,
        ...
        if relevant_framework in framework_evaluations:
            ...
            return HarmonizationResult(
                ...
                remaining_conflicts=[c for c in conflicts if c.framework_a != relevant_framework and c.framework_b != relevant_framework]
            )

        return None

    async def _resolve_by_weighted_consensus(self,
                                           conflicts: List[EthicalFrameworkConflict],
                                           framework_evaluations: Dict[str, Dict[str, Any]],
                                           action: ActionProposal,
                                           context: Dict[str, Any]) -> HarmonizationResult:
    ...
    async def _resolve_by_value_preservation(self,
                                           conflicts: List[EthicalFrameworkConflict],
                                           framework_evaluations: Dict[str, Dict[str, Any]],
                                           action: ActionProposal,
                                           context: Dict[str, Any]) -> HarmonizationResult:
    ...
    async def _build_consensus_result(self,
                                    framework_evaluations: Dict[str, Dict[str, Any]]) -> HarmonizationResult:
    ...
    async def _fallback_meta_ethical_resolution(self,
                                              conflicts: List[EthicalFrameworkConflict],
                                              framework_evaluations: Dict[str, Dict[str, Any]],
                                              action: ActionProposal,
                                              context: Dict[str, Any]) -> HarmonizationResult:
    ...
    async def _evaluate_meta_principle(self,
                                     action: ActionProposal,
                                     context: Dict[str, Any],
                                     principle: str) -> float:
    ...
    async def _evaluate_value_preservation(self,
                                         action: ActionProposal,
                                         context: Dict[str, Any],
                                         value: str) -> float:
