...
class OrchestrationRouter:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.bypass_verification_for_safe_actions = self.config.get("bypass_verification_for_safe_actions", True)

        # Safe actions that can bypass verification (for performance)
        self.safe_actions = set(
            self.config.get("safe_actions", ["status_check", "health_check", "log_event", "metrics_update"])
        )

        logger.info(f"OrchestrationRouter initialized: verification={self.enable_verification}")

    async def execute_plan(self, plan: Dict[str, Any], context: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        if should_verify:
            ...
            if not verification_outcome.allow:
                ...
                logger.warning(
                    f"Plan verification DENIED: action={plan.get('action')}, " f"reasons={verification_outcome.reasons}"
                )
                ...
                "verification": (
                    {
                        ...
                        "verification_time_ms": (
                            verification_outcome.verification_time_ms if verification_outcome else 0
                        ),
                    }
                    if verification_outcome or should_verify
                    else None
                ),
            }
    ...
    def _should_verify_plan(self, plan: Dict[str, Any]) -> bool:
        ...
        if self.bypass_verification_for_safe_actions and isinstance(action, str) and action in self.safe_actions:
            return False

        return True

    async def _execute_action(self, plan: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    async def _execute_safe_action(self, action: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"type": "safe_action", "action": action, "params": params, "verified": False, "simulated": True}
    ...
    async def _execute_generic_action(self, action: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"type": "generic_action", "action": action, "params": params, "simulated": True}
    ...
    def get_verifier_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_10_verifications": (
                self.plan_verifier.verification_ledger[-10:] if self.plan_verifier.verification_ledger else []
            ),
            "config": {
