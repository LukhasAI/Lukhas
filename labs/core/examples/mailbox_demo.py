...
async def demonstrate_priority_processing():
    ...
    for i in range(5):
        tasks.append(
            priority_actor.tell(
                "process_task",
                {"task_name": f"background-{i}", "priority": "LOW"},
            )
        )
    ...
    for i in range(3):
        tasks.append(
            priority_actor.tell(
                "process_task",
                {"task_name": f"normal-{i}", "priority": "NORMAL"},
            )
        )

    # High priority urgent task
    tasks.append(priority_actor.tell("process_task", {"task_name": "urgent-request", "priority": "HIGH"}))

    # System critical task
    tasks.append(
        priority_actor.tell(
            "process_task",
            {"task_name": "system-health-check", "priority": "SYSTEM"},
        )
    )
