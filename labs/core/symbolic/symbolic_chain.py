...
class SymbolicDiff:
    ...
    drift_before: float
    drift_after: float
    constellation_before: float
    constellation_after: float
...
class SymbolicChain:
    ...
    def _gentle_heal(self, response: str, diagnosis: dict) -> str:
        ...
        # Only add Constellation if completely missing
        if not any(g in response for g in self.healer.constellation_core):
            healed = f"{' '.join(self.healer.constellation_core)} {healed}"
    ...
    def _generate_diff(
        ...
        elif diagnosis["primary_issue"] == "constellation_violation":
            intervention_type = "constellation_alignment"
        else:
        ...
        return SymbolicDiff(
            ...
            constellation_before=orig_assessment["constellation_coherence"],
            constellation_after=healed_assessment["constellation_coherence"],
    ...
    def generate_healing_report(self, result: ChainResult) -> str:
        ...
        report_lines = [
            ...
            f"  Constellation Coherence: {diff.constellation_before:.2f} → {diff.constellation_after:.2f}",
            "",
            "🔄 TRANSFORMATIONS:",
        ]
