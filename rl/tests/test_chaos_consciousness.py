...
async def test_memory_fold_failure_resilience():
    ...
    async with chaos_injector.async_inject_failure(ChaosFailureType.MEMORY_FOLD_FAILURE, 0.1):
        for i in range(iterations):
            ...
            memory = await system.buffer.store_experience(context, decision, reward, next_context)
        ...
        for i in range(total_operations):
            try:
                ...
                with chaos_injector.inject_failure(ChaosFailureType.MODULE_DISCONNECTION, 0.2) as ctx:
                    module_result = ctx.simulate_module_disconnection(f"consciousness_module_{i % 5}")
...
async def test_performance_under_chaos():
    ...
    failure_types = [
        (ChaosFailureType.SLOW_RESPONSE, 0.1),
        (ChaosFailureType.DATA_CORRUPTION, 0.05),
        (ChaosFailureType.GUARDIAN_SYSTEM_DELAY, 0.08)
    ]
