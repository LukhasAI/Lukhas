...
class SemanticAnalyzer(
        ...
        if (
            ...
            for arg in unbound.args:
                ...
                if tvar:
                    ...
                    self.fail(f"Free type variable expected in {sym.node.name}[...]", base)
    ...
    def analyze_namedtuple_assign(self, s: AssignmentStmt) -> bool:
        ...
        with self.tvar_scope_frame(self.tvar_scope.class_frame(namespace)):
            ...
            if internal_name != name:
                self.fail(
                    f'First argument to namedtuple() should be "{name}", not "{internal_name}"',
                    s.rvalue,
    ...
    def check_and_set_up_type_alias(self, s: AssignmentStmt) -> bool:
        ...
        if existing and (
            ...
            if isinstance(existing.node, TypeAlias) and not s.is_alias_def:
                self.fail(
                    f'Cannot assign multiple types to name "{lvalue.name}"'
    ...
    def analyze_name_lvalue(
        ...
        if (
            ...
            if existing is not None and not isinstance(existing.node, PlaceholderNode):
                self.fail(
                    f'Attempted to reuse member name "{name}" in Enum definition "{self.type.name}"',
                    lvalue,
                )
                ...
                for lval in lvals:
                    ...
                    if lnode:
                        if isinstance(lnode.node, MypyFile) and lnode.node is not rnode.node:
                            ...
                            self.fail(
                                f'Cannot assign multiple modules to name "{lval.name}" '
                                'without explicit "types.ModuleType" annotation',
                                ctx,
                            )
        ...
        if self.is_module_scope():
            ...
            for name in d.names:
                ...
                if self.locals[-1] is not None and name in self.locals[-1]:
                    self.fail(
                        f'Name "{name}" is already defined in local '
                        "scope before nonlocal declaration",
                        d,
                    )
