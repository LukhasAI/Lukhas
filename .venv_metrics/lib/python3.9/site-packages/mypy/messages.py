...
class MessageBuilder:
        ...
        if isinstance(original_type, Instance) and original_type.type.has_readable_member(member):
            ...
            if not self.are_type_names_disabled():
                ...
                if isinstance(bound, UnionType):
                    ...
                    self.fail(
                        f"Item {typ_fmt} of the upper bound {bound_fmt} of type variable {original_type_fmt} has no "
                        f'attribute "{member}"{extra}',
                        context,
                        code=codes.UNION_ATTR,
                    )
                    return codes.UNION_ATTR
    ...
    def incompatible_argument(
        ...
        if callee_name == "<list>":
            ...
            msg = f"{name.title()} item {n} has incompatible type {actual_type_str}; expected {expected_type_str}"
            ...
            msg = f"{name.title()} entry {n} has incompatible type {key_type_str}: {value_type_str}; expected {expected_key_type_str}: {expected_value_type_str}"
            code = codes.DICT_ITEM
        elif callee_name == "<dict>":
            value_type_str, expected_value_type_str = format_type_distinctly(
                arg_type, callee.arg_types[n - 1], options=self.options
            )
            msg = f"Unpacked dict entry {n - 1} has incompatible type {value_type_str}; expected {expected_value_type_str}"
            code = codes.DICT_ITEM
            ...
            msg = f"List comprehension has incompatible type List[{actual_type_str}]; expected List[{expected_type_str}]"
        elif callee_name == "<set-comprehension>":
            actual_type_str, expected_type_str = map(
                strip_quotes,
                format_type_distinctly(arg_type, callee.arg_types[0], options=self.options),
            )
            msg = f"Set comprehension has incompatible type Set[{actual_type_str}]; expected Set[{expected_type_str}]"
            ...
            msg = f"Generator has incompatible item type {actual_type_str}; expected {expected_type_str}"
        else:
            if self.prefer_simple_messages():
                ...
                if isinstance(outer_context, IndexExpr) and isinstance(
                    ...
                    msg = f'Value of "{outer_context.index.value}" has incompatible type {quote_type_string(arg_type_str)}; expected {quote_type_string(expected_type_str)}'
                else:
                    msg = f"Argument {arg_label} {target}has incompatible type {quote_type_string(arg_type_str)}; expected {quote_type_string(expected_type_str)}"
    ...
    def invalid_index_type(
        ...
        *,
        ...
        return self.fail(
            f"Invalid index type {index_str} for {base_str}; expected type {expected_str}",
            context,
            code=code,
        )
    ...
    def argument_incompatible_with_supertype(
        ...
        self.fail(
            f'Argument {arg_num} of "{name}" is incompatible with {target}; '
            f'supertype defines the argument type as "{arg_type_in_supertype_f}"',
            context,
            code=codes.OVERRIDE,
            secondary_context=secondary_context,
        )
    ...
    def return_type_incompatible_with_supertype(
        ...
        self.fail(
            f'Return type {override_str} of "{name}" incompatible with return type {original_str} in {target}',
            context,
            code=codes.OVERRIDE,
        )
    ...
    def base_class_definitions_incompatible(
        ...
        self.fail(
            f'Definition of "{name}" in base class "{base1.name}" is incompatible '
            f'with definition in base class "{base2.name}"',
            context,
        )
    ...
    def overloaded_signatures_overlap(
        ...
        self.fail(
            f"Overloaded function signatures {index1} and {index2} overlap with "
    ...
    def operator_method_signatures_overlap(
        ...
        self.fail(
            f'Signatures of "{reverse_method}" of "{reverse_class.name}" and "{forward_method}" of {format_type(forward_class, self.options)} are unsafely overlapping',
            context,
        )
    ...
    def unsupported_target_for_star_typeddict(self, typ: Type, ctx: Context) -> None:
        self.fail(
            f"Unsupported type {format_type(typ, self.options)} for ** expansion in TypedDict",
            ctx,
            code=codes.TYPEDDICT_ITEM,
        )
    ...
    def non_required_keys_absent_with_star(self, keys: list[str], ctx: Context) -> None:
        self.fail(
            f"Non-required {format_key_list(keys, short=True)} not explicitly found in any ** item",
            ctx,
            code=codes.TYPEDDICT_ITEM,
        )
    ...
    def unexpected_typeddict_keys(
        ...
        if not typ.is_anonymous():
            ...
            if missing:
                self.fail(
                    f"Missing {format_key_list(missing, short=True)} for TypedDict {format_type(typ, self.options)}",
                    context,
                    code=codes.TYPEDDICT_ITEM,
                )
            ...
            if extra:
                self.fail(
                    f"Extra {format_key_list(extra, short=True)} for TypedDict {format_type(typ, self.options)}",
                    context,
    ...
    def typeddict_key_must_be_string_literal(self, typ: TypedDictType, context: Context) -> None:
        self.fail(
            f"TypedDict key must be a string literal; expected one of {format_item_name_list(typ.items.keys())}",
            context,
    ...
    def typeddict_key_not_found(
        ...
        if typ.is_anonymous():
            self.fail(
                f'"{item_name}" is not a valid TypedDict key; expected one of {format_item_name_list(typ.items.keys())}',
                context,
            )
        else:
    ...
    def bad_proto_variance(
        ...
        msg = capitalize(
            f'{variance_string(actual)} type variable "{tvar_name}" used in protocol where {variance_string(expected)} one is expected'
        )
        self.fail(msg, context)
    ...
    def note_call(
        ...
        self.note(
            f'"{format_type_bare(subtype, self.options)}.__call__" has type {format_type(call, self.options, verbosity=1)}',
            context,
            code=code,
        )
        ...
        *,
        ...
        if (
            ...
            if missing == ["__call__"] and class_obj:
                self.note(
                    f'"{subtype.type.name}" has constructor incompatible with "__call__" of "{supertype.type.name}"',
                    context,
                    parent_error=parent_error,
                )
            else:
                self.note(
                    f'"{subtype.type.name}" is missing following "{supertype.type.name}" protocol member{plural_s(missing)}:',
                    context,
                    parent_error=parent_error,
                )
        ...
        for name, subflags, superflags in conflict_flags[:MAX_ITEMS]:
            if not class_obj and IS_CLASSVAR in subflags and IS_CLASSVAR not in superflags:
                self.note(
                    f"Protocol member {supertype.type.name}.{name} expected instance variable, got class variable",
                    context,
                    parent_error=parent_error,
                )
            if not class_obj and IS_CLASSVAR in superflags and IS_CLASSVAR not in subflags:
                self.note(
                    f"Protocol member {supertype.type.name}.{name} expected class variable, got instance variable",
                    context,
                    parent_error=parent_error,
                )
            if IS_SETTABLE in superflags and IS_SETTABLE not in subflags:
                self.note(
                    f"Protocol member {supertype.type.name}.{name} expected settable variable,"
            ...
            if IS_CLASS_OR_STATIC in superflags and IS_CLASS_OR_STATIC not in subflags:
                self.note(
                    f"Protocol member {supertype.type.name}.{name} expected class or static method",
                    context,
                    parent_error=parent_error,
                )
            if (
                ...
                self.note(
                    ...
                    f' {name} is an instance variable of "{subtype.type.name}"',
                    context,
                    parent_error=parent_error,
                )
            if class_obj and IS_CLASSVAR in superflags:
                self.note(
                    f"ClassVar protocol member {supertype.type.name}.{name} can never be matched by a class object",
                    context,
                    parent_error=parent_error,
                )
                ...
                if len(subtype.items) != len(supertype.items):
                    if supertype_label is not None and subtype_label is not None:
                        msg = msg.with_additional_msg(
                            f" ({subtype_label} {self.format_long_tuple_type(subtype)}, {supertype_label} {self.format_long_tuple_type(supertype)})"
                        )
    ...
    def format_long_tuple_type(self, typ: TupleType) -> str:
        ...
        if item_cnt > MAX_TUPLE_ITEMS:
            return f"tuple[{format_type_bare(typ.items[0], self.options)}, {format_type_bare(typ.items[1], self.options)}, ... <{str(item_cnt - 2)} more items>]"
        else:
    ...
    def generate_incompatible_tuple_error(
        ...
        for i, (lhs_t, rhs_t) in enumerate(zip(lhs_types, rhs_types)):
            if not is_subtype(rhs_t, lhs_t):
                if error_cnt < 3:
                    notes.append(
                        f"Expression tuple item {str(i)} has type {format_type(rhs_t, self.options)}; {format_type(lhs_t, self.options)} expected; "
                    )
                error_cnt += 1
    ...
    def add_fixture_note(self, fullname: str, ctx: Context) -> None:
        ...
        if fullname in SUGGESTED_TEST_FIXTURES:
            self.note(
                f"Consider adding [builtins fixtures/{SUGGESTED_TEST_FIXTURES[fullname]}] to your test description",
                ctx,
            )
...
def make_inferred_type_note(
    ...
    if (
        ...
        return f'Perhaps you need a type annotation for "{var_name}"? Suggestion: {supertype_str}'
    return ""
