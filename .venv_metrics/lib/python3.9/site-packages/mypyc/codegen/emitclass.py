...
def generate_class(cl: ClassIR, module: str, emitter: Emitter) -> None:
    ...
    if cl.has_method("__call__"):
        fields["tp_vectorcall_offset"] = f"offsetof({cl.struct_name(emitter.names)}, vectorcall)"
...
def generate_vtables(
    ...
    def trait_offset_table_name(trait: ClassIR) -> str:
        return f"{base.name_prefix(emitter.names)}_{trait.name_prefix(emitter.names)}_offset_table"

    # Emit array definitions with enough space for all the entries
    emitter.emit_line(
        f"static CPyVTableItem {vtable_name}[{max(1, len(base.vtable_entries) + 3 * len(base.trait_vtables))}];"
    )
    ...
    for trait, vtable in base.trait_vtables.items():
        ...
        emitter.emit_line(
            f"static size_t {trait_offset_table_name(trait)}[{max(1, len(trait.attributes))}];"
        )
...
def generate_offset_table(
    ...
    emitter.emit_line("};")
    emitter.emit_line(
        f"memcpy({trait_offset_table_name}, {trait_offset_table_name}_scratch, sizeof({trait_offset_table_name}));"
    )
...
def generate_vtable(
    ...
    if subtables:
        ...
        for trait, table, offset_table in subtables:
            emitter.emit_line(
                f"(CPyVTableItem){emitter.type_struct_name(trait)}, (CPyVTableItem){table}, (CPyVTableItem){offset_table},"
            )
    ...
    for entry in entries:
        ...
        emitter.emit_line(
            f"(CPyVTableItem){emitter.get_group_prefix(entry.method.decl)}{NATIVE_PREFIX}{method.cname(emitter.names)},"
        )
    ...
    emitter.emit_line("};")
    emitter.emit_line(f"memcpy({vtable_name}, {vtable_name}_scratch, sizeof({vtable_name}));")
...
def generate_setup_for_class(
    ...
    if defaults_fn is not None:
        emitter.emit_lines(
            f"if ({NATIVE_PREFIX}{defaults_fn.cname(emitter.names)}((PyObject *)self) == 0) {{",
            "Py_DECREF(self);",
            "return NULL;",
            "}",
        )
...
def generate_constructor_for_class(
    ...
    if init_fn is not None:
        emitter.emit_line(
            f"char res = {emitter.get_group_prefix(init_fn.decl)}{NATIVE_PREFIX}{init_fn.cname(emitter.names)}({args});"
        )
...
def generate_init_for_class(cl: ClassIR, init_fn: FuncIR, emitter: Emitter) -> str:
    ...
    if cl.allow_interpreted_subclasses or cl.builtin_base:
        emitter.emit_line(
            f"return {PREFIX}{init_fn.cname(emitter.names)}(self, args, kwds) != NULL ? 0 : -1;"
        )
    else:
...
def generate_finalize_for_class(
    ...
    emitter.emit_line(
        f"{emitter.get_group_prefix(del_method.decl)}{NATIVE_PREFIX}{del_method.cname(emitter.names)}(self);"
    )
...
def generate_getseter_declarations(cl: ClassIR, emitter: Emitter) -> None:
    if not cl.is_trait:
        for attr in cl.attributes:
            emitter.emit_line("static PyObject *")
            emitter.emit_line(
                f"{getter_name(cl, attr, emitter.names)}({cl.struct_name(emitter.names)} *self, void *closure);"
            )
            emitter.emit_line("static int")
            emitter.emit_line(
                f"{setter_name(cl, attr, emitter.names)}({cl.struct_name(emitter.names)} *self, PyObject *value, void *closure);"
            )
    ...
    for prop, (getter, setter) in cl.properties.items():
        ...
        emitter.emit_line("static PyObject *")
        emitter.emit_line(
            f"{getter_name(cl, prop, emitter.names)}({cl.struct_name(emitter.names)} *self, void *closure);"
        )
        ...
        if setter:
            emitter.emit_line("static int")
            emitter.emit_line(
                f"{setter_name(cl, prop, emitter.names)}({cl.struct_name(emitter.names)} *self, PyObject *value, void *closure);"
            )
...
def generate_getseters_table(cl: ClassIR, name: str, emitter: Emitter) -> None:
    ...
    if not cl.is_trait:
        for attr in cl.attributes:
            ...
            emitter.emit_line(
                f" (getter){getter_name(cl, attr, emitter.names)}, (setter){setter_name(cl, attr, emitter.names)},"
            )
...
def generate_readonly_getter(
    ...
    if rtype.is_unboxed:
        emitter.emit_line(
            f"{emitter.ctype_spaced(rtype)}retval = {NATIVE_PREFIX}{func_ir.cname(emitter.names)}((PyObject *) self);"
        )
