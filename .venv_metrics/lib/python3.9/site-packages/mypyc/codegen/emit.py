...
class Emitter:
    ...
    def emit_printf(self, fmt: str, *args: str) -> None:
        fmt = fmt.replace("\n", "\\n")
        self.emit_line("printf({});".format(", ".join([f'"{fmt}"'] + list(args))))
    ...
    def tuple_undefined_check_cond(
        ...
        *,
        ...
        if len(rtuple.types) == 0:
            # empty tuple
            return f"{tuple_expr_in_c}.empty_struct_error_flag {compare} {c_type_compare_val(int_rprimitive)}"
    ...
    def emit_inc_ref(self, dest: str, rtype: RType, *, rare: bool = False) -> None:
        ...
        if is_int_rprimitive(rtype):
            if rare:
                self.emit_line(f"CPyTagged_IncRef({dest});")
            else:
                self.emit_line(f"CPyTagged_INCREF({dest});")
        elif isinstance(rtype, RTuple):
            ...
            if rtype.may_be_immortal or not HAVE_IMMORTAL:
                self.emit_line(f"CPy_INCREF({dest});")
            else:
    ...
    def pretty_name(self, typ: RType) -> str:
        ...
        if value_type is not None:
            return f"{self.pretty_name(value_type)} or None"
        return str(typ)
        ...
        *,
        ...
        if (
            ...
            if not concrete or len(concrete) > FAST_ISINSTANCE_MAX_SUBCLASSES + 1:
                check = f"(PyObject_TypeCheck({src}, {self.type_struct_name(typ.class_ir)}))"
            else:
            ...
            assert False, f"Cast not implemented: {typ}"
    ...
    def emit_cast_error_handler(
        ...
        if isinstance(error, AssignHandler):
            ...
            self.emit_line(f"return {error.value};")
    ...
    def emit_tuple_cast(
        ...
        self.emit_lines(
            f"if (unlikely(!(PyTuple_Check({src}) && PyTuple_GET_SIZE({src}) == {len(typ.types)}))) {{",
            f"{dest} = NULL;",
            f"goto {out_label};",
            "}",
        )
    ...
    def emit_unbox(
        ...
        *,
        ...
        if isinstance(error, AssignHandler):
            ...
            failure = f"goto {error.label};"
        else:
            assert isinstance(error, ReturnHandler)
            failure = f"return {error.value};"
        if raise_exception:
        ...
        if is_int_rprimitive(typ) or is_short_int_rprimitive(typ):
            ...
            assert False, f"Unboxing not implemented: {typ}"
    ...
    def emit_gc_visit(self, target: str, rtype: RType) -> None:
        ...
        if not rtype.is_refcounted:
            ...
            assert False, f"emit_gc_visit() not implemented for {repr(rtype)}"
    ...
    def emit_gc_clear(self, target: str, rtype: RType) -> None:
        ...
        if not rtype.is_refcounted:
            ...
            assert False, f"emit_gc_clear() not implemented for {repr(rtype)}"
