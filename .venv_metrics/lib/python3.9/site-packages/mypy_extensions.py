...
for _int_type in i64, i32, i16, u8:
    _int_type.__doc__ = \
        f"""A native fixed-width integer type when used with mypyc.

        In code not compiled with mypyc, behaves like the 'int' type in these
        runtime contexts:

        * {_int_type.__name__}(x[, base=n]) converts a number or string to 'int'
        * isinstance(x, {_int_type.__name__}) is the same as isinstance(x, int)
        """
del _int_type
