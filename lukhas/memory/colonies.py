...
class MemoryColony:
    ...
    def _init_default_nodes(self):
        ...
        # Create cache node
        self.add_node(
            ColonyNode(id="cache-1", role=ColonyRole.CACHE, capacity=100, memories={})
        )
    ...
    def store_memory(self, memory_id: str, content: Any) -> List[str]:
        ...
        storage_nodes = [
            n for n in self.nodes.values() if n.role == ColonyRole.STORAGE and n.active
        ]
    ...
    def retrieve_memory(self, memory_id: str) -> Optional[Any]:
        ...
        # Check cache first
        cache_nodes = [
            n for n in self.nodes.values() if n.role == ColonyRole.CACHE and n.active
        ]
    ...
    def _cache_memory(self, memory_id: str, content: Any):
        """Add memory to cache"""
        cache_nodes = [n for n in self.nodes.values() if n.role == ColonyRole.CACHE]
