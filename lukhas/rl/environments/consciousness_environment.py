...
class ConsciousnessEnvironment(gym.Env):
    ...
    async def _execute_consciousness_action(
        self, 
        ...
        if self.guardian_system:
            ethical_check = await self.guardian_system.assess_consciousness_action(
                self.consciousness_state, action
            )
        ...
        for i, (module_name, module) in enumerate(list(self.consciousness_modules.items())[:modules_engaged]):
            if hasattr(module, 'engage_reflection'):
                reflection_result = await module.engage_reflection(
                    depth=reflection_increase,
                    focus=action.parameters.get("focus_strength", 0.5)
                )
    ...
    def close(self):
        ...
        if hasattr(self, 'episode_rewards') and self.episode_rewards:
            ...
            avg_reward = np.mean(self.episode_rewards)
            logger.info(f"📊 Episode Summary: {len(self.episode_rewards)} steps, total reward: {total_reward:.2f}, avg: {avg_reward:.3f}")
