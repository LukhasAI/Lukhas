...
ConsciousnessExperience = namedtuple('ConsciousnessExperience', [
...
class ConsciousnessReplayBuffer:
    ...
    def __init__(
        ...
        logger.info("🧠 ConsciousnessReplayBuffer initialized: capacity=%d, alpha=%.2f, beta=%.2f", 
                   capacity, alpha, beta)
    ...
    def sample(
        self, 
        batch_size: int,
        consciousness_focus: Optional[str] = None,
        temporal_coherence_threshold: float = 0.0
    ...
    def _sample_prioritized(
        ...
        sampled_indices = np.random.choice(
            ...
            size=min(batch_size, len(valid_experiences)),
            p=probabilities,
            replace=False
        )
    ...
    def get_consciousness_statistics(self) -> Dict[str, Any]:
        ...
        if total_experiences > 0:
            stats['consciousness_ratios'] = {
                key: count / total_experiences 
                for key, count in self.consciousness_stats.items()
            }
        else:
...
class EpisodicConsciousnessBuffer:
    ...
    def finish_episode(self) -> None:
        ...
        if self.current_episode:
            ...
            logger.debug("📝 Finished consciousness episode: length=%d, reward=%.2f", 
                        episode_data['episode_length'], episode_data['total_reward'])
    ...
    def _calculate_episode_consciousness_metrics(self) -> Dict[str, float]:
        ...
        metrics = {
            'avg_temporal_coherence': np.mean([
                info.get('temporal_coherence', 0.9) for info in consciousness_infos
            ]),
            'reflection_actions': sum([
                1 for info in consciousness_infos if info.get('is_reflection', False)
            ]),
            'ethical_significance': np.mean([
                info.get('ethical_significance', 0.5) for info in consciousness_infos
            ]),
            'consciousness_growth': sum([
                info.get('consciousness_growth', 0.0) for info in consciousness_infos
            ])
        }
