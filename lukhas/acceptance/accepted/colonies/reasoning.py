...
from .base import BaseColony, ColonyTask, ColonyAgent, ColonyRole
...
class ReasoningColony(BaseColony):
    ...
    def _add_reasoning_specialists(self):
        ...
        causal_agent = ColonyAgent(
            ...
            capabilities=["causal_inference", "pattern_recognition", "temporal_reasoning"],
        ...
        logic_agent = ColonyAgent(
            role=ColonyRole.SPECIALIST,
            capabilities=["deductive_reasoning", "inductive_reasoning", "logical_validation"],
                    ...
                    for fact in derived_facts:
                        for implication in derived_facts:
                            if self._matches_modus_ponens(fact, implication):
                                ...
                                if new_fact and new_fact not in derived_facts:
                                    new_facts.add(new_fact)
                                    reasoning_steps.append(f"Applied modus ponens: {fact} + {implication} -> {new_fact}")
