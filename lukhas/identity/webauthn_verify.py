...
def _parse_authenticator_data(auth_data: bytes) -> Dict[str, Any]:
    ...
    """
    if len(auth_data) < 37:
        raise InvalidAssertionError(f"Authenticator data too short: {len(auth_data)} bytes (minimum 37)")
...
def verify_assertion(
    ...
    try:
        ...
        if client_data["type"] != "webauthn.get":
            raise InvalidAssertionError(f"Invalid type: {client_data['type']} (expected 'webauthn.get')")

        # Verify origin
        if client_data["origin"] != expected_origin:
            raise InvalidAssertionError(f"Origin mismatch: {client_data['origin']} != {expected_origin}")

        # Step 3: Verify challenge (constant-time comparison)
        received_challenge = client_data["challenge"]
        if not _constant_time_compare(received_challenge.encode("utf-8"), expected_challenge.encode("utf-8")):
        ...
        if not signature_verified:
            raise InvalidSignatureError(f"Signature verification failed for both ES256 and RS256: {last_error}")
    ...
    except (InvalidAssertionError, InvalidChallengeError, InvalidSignatureError, ReplayAttackError) as e:
