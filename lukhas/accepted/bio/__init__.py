...
if _engine_mod is not None and hasattr(_engine_mod, 'BioEngine'):
    BioEngine = getattr(_engine_mod, 'BioEngine')
else:
    class BioEngine:
        def __init__(self):
...
if _symbolic_mod is not None and hasattr(_symbolic_mod, 'get_symbolic_processor'):
    get_symbolic_processor = getattr(_symbolic_mod, 'get_symbolic_processor')
else:
    def get_symbolic_processor(*args, **kwargs):
        return None

if _awareness_mod is not None and hasattr(_awareness_mod, 'BioAwareness'):
    BioAwareness = getattr(_awareness_mod, 'BioAwareness')
else:
    class BioAwareness:
        def __init__(self):
            self.level = 0.5
