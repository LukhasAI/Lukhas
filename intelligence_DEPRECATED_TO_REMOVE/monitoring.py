...
logger = logging.getLogger("LUKHAS.Intelligence.Monitoring")
...
class LukhasIntelligenceMonitor:
    ...
    def record_trinity_compliance(
        ...
        # Record as metrics
        self.record_metric("trinity_identity", identity_score, "score", MetricType.TRINITY_COMPLIANCE, agent_id)
        self.record_metric("trinity_consciousness", consciousness_score, "score", MetricType.TRINITY_COMPLIANCE, agent_id)
        self.record_metric("trinity_guardian", guardian_score, "score", MetricType.TRINITY_COMPLIANCE, agent_id)
        self.record_metric("trinity_overall", trinity_data["overall"], "score", MetricType.TRINITY_COMPLIANCE, agent_id)
    ...
    async def _monitor_system_health(self):
        ...
        while self._monitoring_active:
            try:
                ...
                self.record_metric("cpu_usage", cpu_percent / 100, "percentage", MetricType.SYSTEM_HEALTH)
                
                # Memory usage
                memory = psutil.virtual_memory()
                memory_percent = memory.percent
                self.record_metric("memory_usage", memory_percent / 100, "percentage", MetricType.SYSTEM_HEALTH)
                
                # Disk usage
                disk = psutil.disk_usage('/')
                disk_percent = disk.percent
                self.record_metric("disk_usage", disk_percent / 100, "percentage", MetricType.SYSTEM_HEALTH)
    ...
    async def _analyze_agent_performance_trends(self):
        ...
        for agent_id, stats in self.agent_coordination_stats.items():
            ...
            avg_confidence = sum(s["confidence"] for s in recent_stats if s["confidence"] > 0) / max(1, len([s for s in recent_stats if s["confidence"] > 0]))
            
            # Record agent-specific metrics
            self.record_metric(f"agent_avg_processing_time", avg_processing_time, "seconds", MetricType.AGENT_COORDINATION, agent_id)
            self.record_metric(f"agent_success_rate", success_rate, "percentage", MetricType.AGENT_COORDINATION, agent_id)
            self.record_metric(f"agent_avg_confidence", avg_confidence, "score", MetricType.AGENT_COORDINATION, agent_id)
        ...
        while self._monitoring_active:
            try:
                ...
                if trinity_scores:
                    ...
                    self.record_metric("trinity_overall_compliance", overall_compliance, "score", MetricType.TRINITY_COMPLIANCE)
    ...
    def get_real_time_metrics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                name: metric.to_dict() for name, metric in self.real_time_metrics.items()
            }
    ...
    def get_agent_metrics(self, agent_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        with self._lock:
            if agent_id:
                return {
                    name: metric.to_dict() for name, metric in self.agent_metrics.get(agent_id, {}).items()
                }
            return {
    ...
    def get_engine_metrics(self, engine: Optional[str] = None) -> Dict[str, Any]:
        ...
        with self._lock:
            if engine:
                return {
                    name: metric.to_dict() for name, metric in self.engine_metrics.get(engine, {}).items()
                }
            return {
    ...
    def get_system_health_summary(self) -> Dict[str, Any]:
        ...
        health_metrics = ["cpu_usage", "memory_usage", "disk_usage", "active_operations"]
...
def record_operation_metric(
    ...
    monitor = get_monitor()
    monitor.record_metric(metric_name, value, unit, MetricType.PERFORMANCE, agent_id, intelligence_engine, metadata)
