...
class ColonySignalIntegration:
    ...
    async def emit_hormone(self, signal_type: SignalType, level: float, metadata: Optional[Dict] = None):
    ...
    async def _handle_emergency_mode(self, state: Dict[str, Any]):
        ...
        # Emit stress signal
        await self.emit_hormone(SignalType.STRESS, 0.9, {
...
class EnhancedReasoningColony(ColonySignalIntegration):
    ...
    async def process_query(self, query: str, context: Dict[str, Any] = None) -> ConsensusResult:
        ...
        await self.emit_hormone(SignalType.NOVELTY, 0.5, {
    ...
    def _calculate_consensus(self, votes: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            'dissent_reasons': [
                f"Agent voted for {d}" 
                for d in decisions if d != majority_decision[0]
            ]
        }
    ...
    async def reorganize_for_emergency(self):
        ...
        fast_agents = {
            aid: agent for aid, agent in self.reasoning_agents.items()
        ...
        await self.emit_hormone(SignalType.STRESS, 0.7, {
