...
class DASTEngine:
    ...
    def __init__(self) -> None:
        ...
        self.gesture_interpreter = GestureInterpretationSystem(
            self,
            scorer=self.gesture_scorer,
            repository=self.gesture_repository,
        )
...
class SymbolicActivityTracker:
    ...
    async def track_activity(self, activity: Any, user_id: str) -> Symbol:
        activity_symbol = self.engine.symbolic.create_symbol(
            "activity",
            {
                "type": getattr(activity, "type", str(activity)),
...
class GestureScoringSystem:
    ...
    def score_gestures(
        self, gestures: Iterable[dict[str, Any]], context: dict[str, Any]
    ) -> dict[str, float]:
        ...
        timestamps = [
            _to_datetime(g.get("timestamp")) for g in gestures_list if g.get("timestamp") is not None
        ]
        ...
        weights = context.get(
            "gesture_weights",
            {"frequency": 0.25, "context": 0.25, "emotion": 0.25, "temporal": 0.25},
        )
        ...
        for index, gesture in enumerate(gestures_list):
            gesture_id = str(
                gesture.get("id")
                or gesture.get("gesture_id")
                or f"gesture_{index}"
            )
            gesture_type = gesture.get("type", gesture_id)

            frequency_score = _clamp(
                frequency_counter.get(gesture_type, 1) / max_frequency
                if max_frequency
                else 0.0
            )
            ...
            weighted_sum = (
                frequency_score * weights.get("frequency", 0.25)
                + context_score * weights.get("context", 0.25)
                + emotion_score * weights.get("emotion", 0.25)
                + temporal_score * weights.get("temporal", 0.25)
            )
            score = _clamp(weighted_sum / total_weight)

            breakdown[gesture_id] = GestureScoreBreakdown(
                score=score,
            ...
            # 📊 Logging to support affect tracing
            logger.debug(
                "# ΛTAG: gesture_scoring computed score",
                extra={
...
class GestureCorpusRepository:
    ...
    def __init__(self, corpus_path: Optional[Path | str] = None) -> None:
        ...
        self._corpus_path = (
            ...
            else Path(__file__).with_name("gesture_corpus.json")
        )

    async def fetch_gesture_data(
        self,
        ...
        *,
        ...
        async with self._lock:
            ...
            logger.debug(
                "# ΛTAG: gesture_repository cache_update",
                extra={"user_id": user_id, "entries": len(data)},
            )
            return data

    async def _fetch_from_matriz_memory(
        self, user_context: dict[str, Any]
    ) -> Optional[list[dict[str, Any]]]:
        """Attempt to retrieve gesture data from MATRIZ memory systems."""

        try:
            from matriz.core.memory_system import get_memory_system  # type: ignore
    ...
    async def _fetch_from_file(self) -> list[dict[str, Any]]:
        ...
        if not self._corpus_path.exists():
            logger.debug(
                "Gesture corpus fallback file missing", extra={"path": str(self._corpus_path)}
            )
            return []
        ...
        try:
            ...
            if isinstance(data, list):
                return data
        except Exception as exc:  # pragma: no cover - corrupted fallback guard
...
class GestureInterpretationSystem:
    ...
    def __init__(
        self,
        engine: Optional[DASTEngine],
        *,
        scorer: Optional[GestureScoringSystem] = None,
        repository: Optional[GestureCorpusRepository] = None,
        ethics: Optional[Any] = None,
        symbolic: Optional[SymbolicVocabulary] = None,
    ) -> None:
        self.engine = engine
    ...
    async def interpret(
        ...
        if self._ethics is not None:
            decision = await self._ethics.evaluate_action(
                {"type": "interpret_gestures", "data_type": "biometric"},
                {"gestures": gestures, **user_context},
                "DAST",
            )
        ...
        interpretation = {
            ...
            "anomalies": anomalies,
            "metrics": metrics,
            "overall_confidence": metrics.get("confidence", 0.0),
        }

        logger.debug(
            "# ΛTAG: gesture_interpretation summary",
            extra={
    ...
    async def interpret_gesture(
        ...
        if not interpretation.get("states") or self._symbolic is None:
            return None
        symbol = self._symbolic.create_symbol(
            "gesture",
            {
                "interpretation": interpretation["states"],
                "confidence": interpretation.get("overall_confidence", 0.0),
            },
        )
        return symbol

    def _compute_metrics(
        self, breakdown: dict[str, GestureScoreBreakdown]
    ) -> dict[str, float]:
        if not breakdown:
            return {
                "average_score": 0.0,
        ...
        return {
            ...
            "average_temporal": average_temporal,
    ...
    def _map_states(
        ...
        if average_score >= 0.7 and drift_indicator <= 0.3:
            states.append(
                {
                    "state": "focused",
                    "confidence": _clamp(average_score - (drift_indicator / 2.0)),
                    "supporting_gestures": self._top_gestures(breakdown, limit=3),
                    "signals": {"affect_delta": affect_delta},
                }
            )
        if affect_delta <= -0.2:
            states.append(
                {
                    "state": "stressed",
                    "confidence": _clamp(abs(affect_delta)),
                    "supporting_gestures": self._gestures_with_low_emotion(breakdown),
                    "signals": {"affect_delta": affect_delta},
                }
            )
        if drift_indicator >= 0.6:
            states.append(
                {
                    "state": "drifting",
                    "confidence": _clamp(drift_indicator),
                    "supporting_gestures": self._gestures_with_low_context(breakdown),
                    "signals": {"drift_indicator": drift_indicator},
                }
            )
        if not states:
            states.append(
                {
                    "state": "calm",
                    "confidence": _clamp(metrics.get("average_emotion", 0.5)),
                    "supporting_gestures": self._top_gestures(breakdown, limit=2),
                    "signals": {"affect_delta": affect_delta},
                }
            )
        ...
        if tag_counter.get("analysis", 0) >= 2:
            states.append(
                {
                    "state": "cognitive_analysis",
                    "confidence": _clamp(average_score),
                    "supporting_gestures": self._top_gestures(breakdown, limit=2),
                    "signals": {"tag_frequency": tag_counter.get("analysis", 0)},
                }
            )
    ...
    def _detect_anomalies(
        self, breakdown: dict[str, GestureScoreBreakdown], metrics: dict[str, float]
    ) -> list[dict[str, Any]]:
                ...
                anomalies.append(
                    {
                        ...
                        "gesture_id": gesture_id,
                        "confidence": _clamp(1.0 - detail.context_relevance),
                    }
                )
                ...
                anomalies.append(
                    {
                        "type": "score_outlier",
                        "gesture_id": gesture_id,
                        "confidence": _clamp(abs(detail.score - average_score)),
                    }
                )
        return anomalies

    def _top_gestures(
        self, breakdown: dict[str, GestureScoreBreakdown], *, limit: int
    ) -> list[str]:
        return [
            gesture_id
            for gesture_id, _ in sorted(
                breakdown.items(), key=lambda item: item[1].score, reverse=True
            )[:limit]
        ]

    def _gestures_with_low_emotion(
        self, breakdown: dict[str, GestureScoreBreakdown]
    ) -> list[str]:
        return [
            gesture_id
            for gesture_id, detail in breakdown.items()
            if detail.emotional_valence <= 0.4
        ]

    def _gestures_with_low_context(
        self, breakdown: dict[str, GestureScoreBreakdown]
    ) -> list[str]:
        return [
            gesture_id
            for gesture_id, detail in breakdown.items()
            if detail.context_relevance <= 0.4
        ]
