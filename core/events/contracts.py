...
@dataclass
class QuantumStateCreated(DomainEvent):
...
@dataclass  
class QuantumSuperpositionCreated(DomainEvent):
...
EVENT_REGISTRY = {
    ...
    # Quantum events
    "quantum.state.created": QuantumStateCreated,
    "quantum.state.collapsed": QuantumStateCollapsed,
...
def serialize_event(event: DomainEvent) -> Dict[str, Any]:
    ...
    return {
        ...
        "data": {k: v for k, v in event.__dict__.items() 
                if k not in ["event_id", "timestamp", "source_module", "correlation_id", "causation_id"]}
    }
