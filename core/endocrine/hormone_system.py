...
from core.tags import get_tag_registry, TagCategory
from core.events.contracts import SystemStressLevelChanged, DomainEvent
...
class EndocrineSystem:
    ...
    def trigger_rest_cycle(self, intensity: float = 0.5):
        ...
        self.effect_history.append({
            ...
            "intensity": intensity,
            "timestamp": datetime.now(),
            "hormones_affected": ["melatonin", "gaba", "adrenaline", "cortisol"]
        })
    ...
    async def _emit_stress_event(self, stress_level: float):
        ...
        event = SystemStressLevelChanged(
            ...
            hormone_levels={
                hormone.value: level.level 
                for hormone, level in self.hormones.items()
            }
        )
