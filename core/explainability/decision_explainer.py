...
from core.tags import get_tag_registry, explain_tag
    ...
    def to_human_readable(self, level: ExplanationLevel = ExplanationLevel.STANDARD) -> str:
        ...
        if level == ExplanationLevel.SUMMARY:
            ...
            for factor in top_factors:
                influence_symbol = "↑" if factor.influence == "positive" else "↓" if factor.influence == "negative" else "→"
            ...
            parts = [
                f"=== Detailed Decision Explanation ===",
...
class DecisionExplainer:
    ...
    async def explain_decision(self,
                              ...
                              level: ExplanationLevel = ExplanationLevel.STANDARD) -> DecisionExplanation:
        """
        ...
        try:
            ...
            explanation = DecisionExplanation(
                ...
                process_steps=process_steps if 'process_steps' in locals() else self._trace_decision_process(decision_context, decision_outcome),
    ...
    async def _analyze_decision_factors(self,
                                      context: Dict[str, Any],
                                      outcome: Dict[str, Any]) -> List[DecisionFactor]:
    ...
    def _generate_factor_explanation(self, factor_name: str, value: Any, influence: str, factor_info: Dict) -> str:
        ...
        if factor_name == "safety":
            return template.replace("{influence}", "strongly influenced" if influence == "negative" else "supported")
        elif factor_name == "efficiency":
    ...
    def _explain_confidence(self, confidence: float, factors: List[DecisionFactor], hormonal_state: Dict[str, float]) -> str:
        ...
        return template.replace("{reasons}", reason).replace("{factors}", reason).replace("{confidence}", f"{confidence:.0%}")
...
async def explain_decision(decision_context: Dict[str, Any],
                         decision_outcome: Dict[str, Any],
                         level: ExplanationLevel = ExplanationLevel.STANDARD) -> str:
    """
...
async def get_decision_comparison(decision_context: Dict[str, Any],
                                decision_outcome: Dict[str, Any]) -> str:
...
async def get_decision_counterfactuals(decision_context: Dict[str, Any],
                                     decision_outcome: Dict[str, Any]) -> List[str]:
