...
from .exceptions import ModuleTimeoutError, LukhasError
...
def with_timeout(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        if not asyncio.iscoroutinefunction(func):
            raise TypeError(f"@with_timeout can only be used with async functions, "
...
def lukhas_tier_required(
    ...
    if isinstance(tier, str):
        required_tier = tier_map.get(tier.lower(), 5)
    else:
        required_tier = tier
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            if current_tier >= required_tier:
                ...
                logger.warning(
                    ...
                    f"requires tier {required_tier}, current tier {current_tier}"
                )
                if fallback:
...
def cached(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs):
            ...
            # Check cache
            if key in cache:
                if ttl is None or (datetime.now() - cache_times[key]).total_seconds() < ttl:
                    return cache[key]
