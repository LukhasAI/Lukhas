...
class RegistrationRequest(BaseModel):
    ...
    tier: Optional[str] = Field(
        "T1", pattern="^T[1-5]$", description="Requested tier (T1-T5)"
    )
    cultural_profile: Optional[str] = Field(
        "universal", description="Cultural background"
    )
    personality_type: Optional[str] = Field(
        "balanced", description="Personality profile"
    )
...
async def register_user(request: RegistrationRequest):
    ...
    try:
        ...
        if not request.consent:
            raise HTTPException(
                status_code=400, detail="GDPR consent is required for registration"
            )
