...
def parse_requirement(req):
    ...
    m = IDENTIFIER.match(remaining)
    if not m:
        raise SyntaxError(f"name expected: {remaining}")
    distname = m.groups()[0]
    remaining = remaining[m.end():]
    ...
    if remaining and remaining[0] == "[":
        i = remaining.find("]", 1)
        if i < 0:
            raise SyntaxError(f"unterminated extra: {remaining}")
        s = remaining[1:i]
        ...
        while s:
            ...
            if not m:
                raise SyntaxError(f"malformed extra: {s}")
            extras.append(m.groups()[0])
            s = s[m.end():]
            if not s:
                break
            if s[0] != ",":
                raise SyntaxError(f"comma expected in extras: {s}")
            s = s[1:].lstrip()
        if not extras:
            extras = None
    if remaining:
        if remaining[0] == "@":
            ...
            if not m:
                raise SyntaxError(f"invalid URI: {remaining}")
            uri = m.groups()[0]
            t = urlparse(uri)
            ...
            if not (t.scheme and t.netloc):
                raise SyntaxError(f"Invalid URL: {uri}")
            ...
            if remaining[0] != "(":
                ...
                if COMPARE_OP.match(s):
                    ...
                    if not m:
                        raise SyntaxError(f"invalid constraint: {s}")
                    v = m.groups()[0]
                    s = s[m.end():].lstrip()
                    if s:
                        raise SyntaxError(f"invalid constraint: {s}")
    ...
    if remaining:
        if remaining[0] != ";":
            raise SyntaxError(f"invalid requirement: {remaining}")
        remaining = remaining[1:].lstrip()

        mark_expr, remaining = parse_marker(remaining)

    if remaining and remaining[0] != "#":
        raise SyntaxError(f"unexpected trailing data: {remaining}")

    if not versions:
        rs = distname
    else:
        rs = "{} {}".format(distname, ", ".join(["{} {}".format(*con) for con in versions]))
...
class FileOperator:
    ...
    def newer(self, source, target):
        ...
        if not os.path.exists(source):
            raise DistlibException(f"file '{os.path.abspath(source)!r}' does not exist")
    ...
    def set_executable_mode(s, f):
        return s.set_mode(0o555, 0o7777, f)
...
def parse_name_and_version(p):
    ...
    if not m:
        raise DistlibException(f"Ill-formed name/version string: '{p}'")
    d = m.groupdict()
...
class EventMixin:
    ...
    def remove(self, event, subscriber):
        ...
        subs = self._subscribers
        if event not in subs:
            raise ValueError(f"No subscribers: {event!r}")
...
class Sequencer:
    ...
    def remove(self, pred, succ):
        ...
        try:
            ...
            raise ValueError(f"{succ!r} not a successor of anything")
        try:
            preds.remove(pred)
            succs.remove(succ)
        except KeyError:  # pragma: no cover
    ...
    def get_steps(self, final):
        if not self.is_step(final):
            raise ValueError(f"Unknown: {final!r}")
        result = []
        todo = []
        seen = set()
        todo.append(final)
...
def unarchive(archive_filename, dest_dir, format=None, check=True):
    ...
    def check_path(path):
        ...
        if not p.startswith(dest_dir) or p[plen] != os.sep:
            raise ValueError(f"path outside destination: {p!r}")
    ...
    if format is None:
        if archive_filename.endswith((".zip", ".whl")):
            ...
            raise ValueError(f"Unknown format for {archive_filename!r}")
    try:
        if format == "zip":
