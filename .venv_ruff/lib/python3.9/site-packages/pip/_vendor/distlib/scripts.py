...
class ScriptMaker:
    ...
    def _get_shebang(self, encoding, post_interp=b"", options=None):
        ...
        if self.executable:
            ...
            executable = os.path.join(sysconfig.get_path("scripts"), "python{}".format(sysconfig.get_config_var("EXE")))
        else:  # pragma: no cover
            if os.name == "nt":
                # for Python builds from source on Windows, no Python executables with
                # a version suffix are created, so we use python.exe
                executable = os.path.join(sysconfig.get_config_var("BINDIR"),
                                          "python{}".format(sysconfig.get_config_var("EXE")))
            else:
                executable = os.path.join(
                    sysconfig.get_config_var("BINDIR"),
                    "python{}{}".format(sysconfig.get_config_var("VERSION"), sysconfig.get_config_var("EXE")))
        if options:
        ...
        try:
            ...
            raise ValueError(f"The shebang ({shebang!r}) is not decodable from utf-8")
        ...
        if encoding != "utf-8":
            try:
                ...
                raise ValueError(f"The shebang ({shebang!r}) is not decodable "
                                 f"from the script encoding ({encoding!r})")
        return shebang
    ...
    def get_script_filenames(self, name):
        ...
        if "X.Y" in self.variants:
            result.add(f"{name}{self.variant_separator}{self.version_info[0]}.{self.version_info[1]}")
        return result
