...
from re import Match as MatchHint  # Replaced by `re.Match` in 3.9.
from re import Pattern as PatternHint  # Replaced by `re.Pattern` in 3.9.
from typing import (
	Any,
	AnyStr,  # Replaced by `collections.abc.Iterator` in 3.9.
...
class Pattern:
	...
	def match(self, files: Iterable[str]) -> Iterator[str]:
		...
		*files* (:class:`~collections.abc.Iterable` of :class:`str`) contains each
		...
		"""
		warnings.warn((
			f"{self.__class__.__module__}.{self.__class__.__qualname__}.match() is deprecated. Use "
			f"{self.__class__.__module__}.{self.__class__.__qualname__}.match_file() with a loop for "
			"similar results."
	...
	def match_file(self, file: str) -> Optional[Any]:
		...
		*file* (:class:`str`) is the normalized file path to match against.
		...
		raise NotImplementedError(
			f"{self.__class__.__module__}.{self.__class__.__qualname__} must override match_file()."
		)
