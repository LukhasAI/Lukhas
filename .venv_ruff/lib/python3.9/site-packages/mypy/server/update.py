...
class FineGrainedBuildManager:
    ...
    def update(
        ...
        self.manager.log_fine_grained(
            "==== update {} ====".format(", ".join(repr(id) for id, _ in changed_modules))
        )
        if self.previous_targets_with_errors and is_verbose(self.manager):
            self.manager.log_fine_grained(
                f"previous targets with errors: {sorted(self.previous_targets_with_errors)}"
            )
    ...
    if to_process:
        if is_verbose(manager):
            manager.log_fine_grained(
                f"Calling process_fresh_modules on set of size {len(to_process)} ({sorted(to_process)})"
            )
...
def reprocess_nodes(
    ...
    if module_id not in graph:
        manager.log_fine_grained(f"{module_id} not in graph (blocking errors or deleted?)")
        return set()
