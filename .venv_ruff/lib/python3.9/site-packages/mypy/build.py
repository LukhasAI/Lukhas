...
def load_plugins_from_config(
    ...
    for plugin_path in options.plugins:
        ...
        if not hasattr(module, func_name):
            plugin_error(
                f'Plugin "{plugin_path}" does not define entry point function "{func_name}"'
            )
        ...
        if not isinstance(plugin_type, type):
            plugin_error(
                f'Type object expected as the return value of "plugin"; got {plugin_type!r} (in {plugin_path})'
            )
...
def _load_json_file(
    ...
    try:
        ...
        manager.errors.report(
            ...
            f"Try removing the {manager.options.cache_dir} directory"
            " and run mypy again.",
            blocker=True,
        )
        return None
...
class NodeInfo:
    ...
    def dumps(self) -> str:
        ...
        return f"[{json.dumps(self.node_id)}, {json.dumps(total_size)}, {json.dumps(self.scc)},\n     {json.dumps(self.sizes)},\n     {json.dumps(self.deps)}]"
...
def process_graph(graph: Graph, manager: BuildManager) -> None:
    ...
    if sccs_left:
        manager.log(
            f"{sccs_left} fresh SCCs ({nodes_left} nodes) left in queue (and will remain unprocessed)"
        )
