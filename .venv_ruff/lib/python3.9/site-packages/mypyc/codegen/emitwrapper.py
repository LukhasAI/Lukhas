...
def wrapper_function_header(fn: FuncIR, names: NameGenerator) -> str:
    ...
    return (
        f"PyObject *{PREFIX}{fn.cname(names)}("
...
def legacy_wrapper_function_header(fn: FuncIR, names: NameGenerator) -> str:
    return f"PyObject *{PREFIX}{fn.cname(names)}(PyObject *self, PyObject *args, PyObject *kw)"
...
def generate_bin_op_both_wrappers(
    ...
    emitter.emit_line(
        f"if (PyObject_IsInstance(obj_left, (PyObject *){emitter.type_struct_name(cl)})) {{"
    )
    gen.emit_arg_processing(error=GotoHandler("typefail"), raise_exception=False)
    ...
    emitter.emit_label("typefail")
    emitter.emit_line(
        f"if (PyObject_IsInstance(obj_right, (PyObject *){emitter.type_struct_name(cl)})) {{"
    )
...
def generate_bin_op_reverse_dunder_call(fn: FuncIR, emitter: Emitter, rmethod: str) -> None:
    ...
    emitter.emit_line(
        f'return CPy_CallReverseOpMethod(obj_left, obj_right, "{op_methods_to_symbols[fn.name]}", &PyId_{rmethod});'
    )
    if fn.name in ("__pow__", "__rpow__"):
        emitter.emit_line("} else {")
        emitter.emit_line("Py_INCREF(Py_NotImplemented);")
...
def generate_richcompare_wrapper(cl: ClassIR, emitter: Emitter) -> str | None:
    ...
    emitter.emit_line(
        f"static PyObject *{name}(PyObject *obj_lhs, PyObject *obj_rhs, int op) {{"
    )
...
def generate_get_wrapper(cl: ClassIR, fn: FuncIR, emitter: Emitter) -> str:
    ...
    emitter.emit_line(
        f"static PyObject *{name}(PyObject *self, PyObject *instance, PyObject *owner) {{"
    )
...
def generate_hash_wrapper(cl: ClassIR, fn: FuncIR, emitter: Emitter) -> str:
    ...
    emitter.emit_line(
        f"{emitter.ctype_spaced(fn.ret_type)}retval = {emitter.get_group_prefix(fn.decl)}{NATIVE_PREFIX}{fn.cname(emitter.names)}(self);"
    )
...
def generate_bool_wrapper(cl: ClassIR, fn: FuncIR, emitter: Emitter) -> str:
    ...
    emitter.emit_line(
        f"{emitter.ctype_spaced(fn.ret_type)}val = {NATIVE_PREFIX}{fn.cname(emitter.names)}(self);"
    )
...
def generate_set_del_item_wrapper(cl: ClassIR, fn: FuncIR, emitter: Emitter) -> str:
    ...
    if del_name is not None:
        ...
        emitter.emit_line(
            f'PyObject *result = PyObject_CallMethod(super, "__delitem__", "O", obj_{args[1].name});'
        )
    ...
    if method_cls and method_cls[1] == cl:
        ...
        if method_cls is None and cl.builtin_base is None:
            ...
            emitter.emit_line(
                f'result = PyObject_CallMethod(super, "__setitem__", "OO", obj_{args[1].name}, obj_{args[2].name});'
            )
        emitter.emit_line("Py_DECREF(super);")
        emitter.emit_line("Py_XDECREF(result);")
        emitter.emit_line("return result == NULL ? -1 : 0;")
...
def generate_set_del_item_wrapper_inner(
    ...
    emitter.emit_line(
        f"{emitter.ctype_spaced(fn.ret_type)}val = {NATIVE_PREFIX}{fn.cname(emitter.names)}({native_args});"
    )
...
def generate_contains_wrapper(cl: ClassIR, fn: FuncIR, emitter: Emitter) -> str:
    ...
    emitter.emit_line(
        f"{emitter.ctype_spaced(fn.ret_type)}val = {NATIVE_PREFIX}{fn.cname(emitter.names)}(self, arg_item);"
    )
    emitter.emit_error_check("val", fn.ret_type, "return -1;")
...
class WrapperGenerator:
    ...
    def emit_header(self) -> None:
        ...
        self.emitter.emit_line(
            f"static PyObject *{self.wrapper_name()}({input_args}) {{"
        )
    ...
    def emit_call(self, not_implemented_handler: str = "") -> None:
        ...
        if ret_type.is_unboxed or self.use_goto():
            ...
            emitter.emit_line(
                f"{emitter.ctype_spaced(ret_type)}retval = {NATIVE_PREFIX}{self.target_cname}({native_args});"
            )
            ...
            if not_implemented_handler and not isinstance(ret_type, RInstance):
                ...
                emitter.emit_line(
                    f"PyObject *retbox = {NATIVE_PREFIX}{self.target_cname}({native_args});"
                )
                emitter.emit_lines(
