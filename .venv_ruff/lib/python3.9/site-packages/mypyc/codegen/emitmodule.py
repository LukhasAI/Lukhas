...
class GroupGenerator:
    ...
    def generate_shared_lib_init(self, emitter: Emitter) -> None:
        ...
        emitter.emit_lines(
            ...
            "{",
            (
                f'static PyModuleDef def = {{ PyModuleDef_HEAD_INIT, "{shared_lib_name(self.group_name)}", NULL, -1, NULL, NULL }};'
            ),
            "int res;",
            "PyObject *capsule;",
            "PyObject *tmp;",
        ...
        emitter.emit_lines(
            f'capsule = PyCapsule_New(&exports, "{shared_lib_name(self.group_name)}.exports", NULL);',
            "if (!capsule) {",
            "goto fail;",
            "}",
        ...
        for mod in self.modules:
            ...
            emitter.emit_lines(
                ...
                f'capsule = PyCapsule_New((void *)CPyInit_{name}, "{shared_lib_name(self.group_name)}.init_{name}", NULL);',
                "if (!capsule) {",
                "goto fail;",
                "}",
        ...
        for group in sorted(self.context.group_deps):
            ...
            emitter.emit_lines(
                f'tmp = PyImport_ImportModule("{shared_lib_name(group)}"); if (!tmp) goto fail; Py_DECREF(tmp);',
                f'struct export_table_{egroup} *pexports_{egroup} = PyCapsule_Import("{shared_lib_name(group)}.exports", 0);',
                f"if (!pexports_{egroup}) {{",
                "goto fail;",
                "}",
                f"memcpy(&exports_{egroup}, pexports_{egroup}, sizeof(exports_{egroup}));",
                "",
            )
    ...
    def emit_module_methods(
        ...
        for fn in module.functions:
            ...
            emitter.emit_line(
                    ...
                    f'{{"{name}", (PyCFunction){PREFIX}{fn.cname(emitter.names)}, {flag} | METH_KEYWORDS, '
                    "NULL /* docstring */},"
    ...
    def emit_module_exec_func(
        ...
        for cl in module.classes:
            ...
            if cl.is_generated:
                emitter.emit_lines(
                    f"{type_struct} = (PyTypeObject *)CPyType_FromTemplate("
                    f"(PyObject *){type_struct}_template, NULL, modname);"
                )
