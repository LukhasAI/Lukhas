    ...
    def _check_alias_dict(self, aliases, what):
        ...
        for (alias, opt) in aliases.items():
            if alias not in self.option_index:
                raise DistutilsGetoptError(f"invalid {what} '{alias}': "
                       f"option '{alias}' not defined")
            if opt not in self.option_index:
                raise DistutilsGetoptError(f"invalid {what} '{alias}': "
                       f"aliased option '{opt}' not defined")
    ...
    def _grok_option_table(self):
        ...
        for option in self.option_table:
            ...
            if not isinstance(long, str) or len(long) < 2:
                raise DistutilsGetoptError(f"invalid long option '{long}': "
                       "must be a string of length >= 2")

            if (not ((short is None) or
                     (isinstance(short, str) and len(short) == 1))):
                raise DistutilsGetoptError(f"invalid short option '{short}': "
