...
class DreamEngine:
    ...
    async def _generate_dialogue(
        ...
        return DreamElement(
            ...
            properties={
                ...
                "emotional_context": max(context.emotional_state.items(), key=lambda x: x[1])[0] if context.emotional_state else "neutral"
            },
    ...
    async def _generate_emotional_journey(
        self,
        elements: List[DreamElement],
        base_emotional_state: Dict[str, float],
        template: Dict[str, Any]
    ...
    async def _calculate_lucidity_level(
        ...
        if template.get("focus") == "conscious_control":
            base_lucidity += 0.3
        elif template.get("focus") == "self_discovery":
            base_lucidity += 0.2
