...
class OpenAIModulatedService:
    ...
    def get_modulation_stats(self) -> Dict[str, Any]:
        ...
        for signal_type in signal_types:
            values = [r['signal_context'].get(signal_type, 0) 
                     for r in self.modulation_history]
...
async def create_modulated_service(api_key: Optional[str] = None) -> OpenAIModulatedService:
    """Create and initialize a modulated OpenAI service"""
    service = OpenAIModulatedService(api_key)
    
    # Emit initial trust signal
    await service.emit_signal(Signal(
