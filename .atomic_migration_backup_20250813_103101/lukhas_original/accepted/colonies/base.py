...
class BaseColony(ABC):
    ...
    def submit_task(self, task_type: str, payload: Any, priority: int = 5, 
                   required_capabilities: List[str] = None) -> ColonyTask:
    ...
    def assign_task(self, task: ColonyTask) -> Optional[str]:
        ...
        for agent in self.agents.values():
            if (agent.status == ColonyStatus.ACTIVE and 
                agent.load < 0.8 and
    ...
    def process_queue(self) -> Dict[str, Any]:
        ...
        while self.task_queue and processed < 10:  # Limit per cycle
            ...
            try:
                ...
                if task.assigned_agent and task.assigned_agent in self.agents:
                    self.agents[task.assigned_agent].load = max(0, 
                        self.agents[task.assigned_agent].load - 0.2)
        ...
        return {
            ...
            "active_agents": len([a for a in self.agents.values() 
                                if a.status == ColonyStatus.ACTIVE])
        }
    ...
    def remove_agent(self, agent_id: str) -> bool:
        ...
        if agent_id in self.agents:
            ...
            if agent.role == ColonyRole.COORDINATOR:
                ...
                coordinators = [a for a in self.agents.values() 
                              if a.role == ColonyRole.COORDINATOR]
...
class ColonyRegistry:
    ...
    def unregister_colony(self, colony_name: str) -> bool:
        ...
        if colony_name in self.colonies:
            ...
            # Remove task routes
            self.task_routes = {k: v for k, v in self.task_routes.items() 
                              if v != colony_name}
            return True
        return False
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        active_colonies = len([c for c in self.colonies.values() 
                             if c.status == ColonyStatus.ACTIVE])
