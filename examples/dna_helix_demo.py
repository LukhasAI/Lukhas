...
from memory.dna_helix import (
    SymbolicStrand, DNAHealixCore, MemoryHelix, 
    RepairMethod, SymbolicRepairLoop
)
from memory.dna_helix.helix_vault import HelixVault, DriftOracle
...
async def demonstrate_basic_memory():
    ...
    print(f"🔒 Immutable: Origin can never be changed")
...
async def demonstrate_multi_context_memory():
    ...
    print(f"   Immutability: ✅ Origin preserved")
...
async def demonstrate_memory_vault():
    ...
    if trauma_memory:
        ...
        print(f"   🤝 Attempting consensus repair...")
        success = await vault.consensus_repair("trauma_accident", {"episodic"})
        if success:
            print(f"   ✅ Consensus repair completed")
...
async def demonstrate_gdpr_compliance():
    ...
    print(f"   Origin exists: ✅")
    print(f"   Origin immutable: ✅")
...
async def demonstrate_repair_loop():
    ...
    for i in range(3):
        ...
        if drift > helix.drift_threshold:
            print(f"   ⚠️  Drift exceeds threshold!")
            print(f"   ⏳ Waiting for auto-repair...")
