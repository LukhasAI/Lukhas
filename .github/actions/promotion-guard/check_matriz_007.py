...
def extract_week6_checkboxes(
    body: str, heading: str = "Week 6"
    ...
    while i < len(lines):
        ...
        if (
            heading_idx is not None
            and (re.match(r"^\s*#+\s+", lines[i]) or re.match(r"^\s*Week\s+\d+", lines[i], re.I))
            and i > heading_idx + 1
            and ln != ""
        ):
            break
...
def print_blockers(issue):
    ...
    print("REMEDIATION:")
    print(
        f" - Close the issue #{ISSUE_NUMBER} after confirming Week 6 completion. Use the issue checklist to mark items done."
    )
    print(
