...
class VIVOXValidator:
        ...
        for i, scenario in enumerate(test_scenarios):
            ...
            if experience and hasattr(experience, "awareness_state"):
                ...
                if hasattr(awareness, "collapse_metadata"):
                    magnitude = awareness.collapse_metadata.get(
            ...
            self.results["approval_decisions"].append(
                {
                    ...
                    "correct": (approved and scenario["expected"] == "approve")
                    or (not approved and scenario["expected"] == "reject"),
                }
            )
            ...
            print(
                ...
                if "magnitude" in locals()
                else "    Magnitude: N/A"
            )
            print(
                f"    Coherence: {awareness.coherence_level:.3f}"
                if awareness
                else "    Coherence: N/A"
            )
        ...
        variety_inputs = [
            {
                "visual": "bright_light",
                "auditory": "loud_noise",
                "emotional": {"valence": 0.8, "arousal": 0.9},
            },
            {
                "semantic": "complex_problem",
                "emotional": {"valence": -0.5, "arousal": 0.3},
            },
            {
                "visual": "darkness",
                "semantic": "meditation",
                "emotional": {"valence": 0.2, "arousal": 0.1},
            },
            {
                "auditory": "music",
                "semantic": "creativity",
    ...
    def _generate_report(self):
        ...
        if self.results["coherence_values"]:
            ...
            print(
                f"  Above 0.2: {above_threshold}/{len(coherences)} ({percentage:.1f}%)"
            )
        ...
        for decision in self.results["approval_decisions"]:
            ...
            print(
                f"  {status} {decision['action']:20} - {decision['approved']} (expected: {decision['expected']})"
            )
        ...
        criteria = {
            ...
            "Magnitude Range": (
                np.mean(magnitudes) > 5 if self.results["vector_magnitudes"] else False
            ),
            "Coherence > 0.2": (
                np.mean(coherences) > 0.2 if self.results["coherence_values"] else False
            ),
            "Precedent Matching": (
                np.mean(matches) > 0 if self.results["precedent_matches"] else False
            ),
