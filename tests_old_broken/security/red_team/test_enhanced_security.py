...
from core.security.enhanced_auth import EnhancedAuthenticationSystem
from core.security.enhanced_crypto import EnhancedEncryptionManager
...
class TestEnhancedCrypto:
    ...
    async def test_aes_encryption(self, crypto_manager):
        ...
        ciphertext, key_id = await crypto_manager.encrypt(
            plaintext, purpose="data", algorithm="AES-256-GCM"
        )
    ...
    async def test_chacha20_encryption(self, crypto_manager):
        ...
        # Encrypt
        ciphertext, key_id = await crypto_manager.encrypt(
            plaintext, purpose="session", algorithm="ChaCha20-Poly1305"
        )
    ...
    async def test_authenticated_encryption(self, crypto_manager):
        ...
        decrypted = await crypto_manager.decrypt(
            ciphertext, key_id, associated_data=associated_data
        )
        assert decrypted == plaintext
        ...
        with pytest.raises(Exception):
            await crypto_manager.decrypt(
                ciphertext, key_id, associated_data=b"wrong_metadata"
            )
...
class TestEnhancedAuth:
    ...
    def test_api_key_management(self, auth_system):
        ...
        # Verify valid key
        asyncio.run(
            self._verify_api_key(auth_system, key_id, key_secret, user_id, scopes)
        )
...
class TestSecurityIntegration:
    ...
    async def test_request_validation(self, security):
        ...
        # Create valid JWT
        jwt_token = security.auth.generate_jwt(
            "test_user", {"mfa_verified": True, "session_id": "test_session"}
        )
        ...
        jwt_no_mfa = security.auth.generate_jwt(
            "test_user", {"mfa_verified": False, "session_id": "test_session"}
        )
    ...
    async def test_api_key_validation(self, security):
        ...
        # Generate API key
        user_id = "api_user"
        key_id, key_secret = security.auth.generate_api_key(user_id, ["read", "write"])
