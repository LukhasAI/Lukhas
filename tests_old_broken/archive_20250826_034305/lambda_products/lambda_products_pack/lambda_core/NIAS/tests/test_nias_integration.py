...
async def test_nias_full_pipeline():
    ...
    test_user_context = {
        ...
        "preferences": {"theme": "eco_friendly", "accent_color": "#4caf50"},
        "recent_interactions": [],
    }
    ...
    try:
        from NIΛS.core.consent_filter import ConsentType, get_consent_filter
        ...
        grant_result = await consent_filter.grant_consent(
            test_user_id, consent_grants, consent_request["request_id"]
        )
    ...
    try:
        processing_result = await nias_hub.process_symbolic_message(
            test_message, test_user_context
        )
        ...
        if processing_result["status"] == "delivered":
            ...
            if processing_result.get("widget_config"):
                ...
                print(
                    f"   Widget generated: {widget['type']} (ID: {widget['widget_id']})"
                )
        elif processing_result["status"] == "deferred":
            print(
                f"   Deferred until: {processing_result.get('defer_until', 'unknown')}"
            )
        ...
        receptive_context["recent_interactions"] = (
            []
        )  # Less interactions = more receptive

        print("\n   Testing with more receptive state...")
        processing_result2 = await nias_hub.process_symbolic_message(
            test_message, receptive_context
        )
    ...
    try:
        if processing_result.get("widget_config"):
            ...
            interaction_result = await widget_engine.handle_interaction(
                widget_id, "tap", test_user_id, {"timestamp": "2024-01-01T12:00:00Z"}
            )

            print("✅ Widget interaction processed")
    ...
    try:
        ...
        print("✅ Tier analytics:")
        print(f"   Total users: {tier_analytics['total_users']}")
        print(f"   Tier distribution: {tier_analytics['tier_distribution']}")

        # Dream analytics
        dream_analytics = await dream_recorder.get_dream_analytics(
            user_id=test_user_id, days=1
        )
        print("✅ Dream analytics:")
    ...
    try:
        ...
        invalid_result = await nias_hub.process_symbolic_message(
            test_message, {"user_id": "invalid_user", "tier": "T3"}
        )
        ...
        withdraw_result = await consent_filter.withdraw_consent(
            test_user_id, ConsentType.BRAND_TARGETING
        )
