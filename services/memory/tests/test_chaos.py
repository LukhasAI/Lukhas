...
class ChaosVectorStore:
    ...
    async def search_vectors(self, query):
        ...
        results = [
            SearchResult(
                fold=MemoryFold(
                    fold_id=f"chaos_fold_{i}", content=f"Chaos test content {i}", metadata={"chaos": True}, embedding=[]
                ),
                score=0.8,
                rank=i + 1,
            )
        ...
        return SearchResponse(results=results, query_time_ms=10.0, total_results=len(results))
    ...
    async def upsert_documents(self, documents):
        ...
        return {"inserted": len(documents), "updated": 0, "failed": 0, "duration_ms": 15.0}
...
async def chaos_services():
    ...
    read_breaker = circuit_breakers.create_breaker("chaos_read", CircuitBreakerFactory.create_memory_service_config())
    write_breaker = circuit_breakers.create_breaker("chaos_write", CircuitBreakerFactory.create_memory_service_config())
...
class TestChaosEngineering:
    ...
    async def test_network_timeout_chaos(self, chaos_services):
        ...
        async def search_operation():
            query = SearchQuery(query_text="chaos network test", search_type=SearchType.SEMANTIC, top_k=5)
            await read_service.search(query)

        # Run chaos test
        result = await self.run_chaos_test(chaos_scenario, search_operation, test_duration=20.0, operation_interval=0.5)

        # Validate chaos behavior
        assert result.operations_failed > 0, "Should have some failures during network timeouts"
        assert (
            "TimeoutError" in result.error_types or "CircuitBreakerError" in result.error_types
        ), "Should see timeout or circuit breaker errors"
        assert result.system_recovered, "System should recover after network issues"

        print(
            f"Network Timeout Chaos: {result.operations_failed}/{result.operations_attempted} "
        ...
        async def mixed_operation():
            ...
            try:
                if random.random() < 0.7:
                    ...
                    # Write operation
                    await write_service.upsert_memory_fold(content="cascade test content")
    ...
    async def test_memory_pressure_resilience(self, chaos_services):
        ...
        result = await self.run_chaos_test(
            chaos_scenario, memory_intensive_operation, test_duration=15.0, operation_interval=0.2
        )
        ...
        print(f"Memory Pressure: recovered={result.system_recovered}, " f"memory_growth={memory_growth} objects")
    ...
    async def test_random_failure_resilience(self, chaos_services):
        ...
        async def random_operation():
            ...
            try:
                if random.random() < 0.6:
                    ...
                    # Write operation
                    await write_service.upsert_memory_fold(content=f"random content {time.time()}")
                success_count += 1
            except Exception:
                failure_count += 1
        ...
        print(
            f"Random Failure Resilience: {success_rate:.2f} success rate "
    ...
    async def test_slow_response_backpressure(self, chaos_services):
        ...
        backpressure_rejections = sum(
            1 for r in results if isinstance(r, Exception) and "backpressure" in str(r).lower()
        )
        ...
        print(
            f"Slow Response Backpressure: {rejection_rate:.2f} rejection rate, "
            f"{successes} successes, duration={duration:.1f}s"
        )

        # Should have some backpressure activation
        assert (
            rejection_rate > 0.1 or duration < 30.0
    ...
    async def test_recovery_time_validation(self, chaos_services):
        ...
        print(
            f"Recovery Time Validation: avg={avg_recovery_time:.1f}s, "
    ...
    async def test_graceful_degradation(self, chaos_services):
        ...
        async def test_degradation():
            ...
            try:
                await write_service.upsert_memory_fold(content=f"degradation content {time.time()}")
                write_successes += 1
            except Exception:
                pass
        ...
        print(
            f"Graceful Degradation: search_failures={search_failure_rate:.2f}, "
