...
Complete systematic migration from qi naming to qi naming conventions
...
class QIToQIMigrator:
            ...
            try:
                ...
                for match in class_matches:
                    ...
                    if class_name != 'Quantum':  # Preserve base 'Quantum' class if exists
                        qi_classes.append((py_file, class_name))
    ...
    def find_files_with_quantum_references(self) -> list[Path]:
        ...
        for py_file in self.root.glob('**/*.py'):
            try:
                content = py_file.read_text(encoding='utf-8')
                if re.search(r'\bquantum_\w+|\bQuantum[A-Z]\w*|from quantum|import qi|\bquantum\.', content):
    ...
    def generate_file_renames(self) -> None:
        ...
        qi_files = self.find_quantum_files()

        for old_path in qi_files:
    ...
    def generate_class_renames(self) -> None:
        ...
        qi_classes = self.find_quantum_classes()

        for filepath, old_class in qi_classes:
    ...
    def preview_migration(self) -> None:
        ...
        print("python qi_to_qi_migrator.py --execute")
        print()
