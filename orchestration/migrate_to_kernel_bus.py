...
class KernelBusMigration:
    ...
    def __init__(self):
        ...
        self.replacements = [
            ...
            (r'event_bus\.publish\((.*?)\)',
             r'kernel_bus.emit(\1)'),
            (r'self\.event_bus\.publish\((.*?)\)',
             r'kernel_bus.emit(\1)'),
            (r'await event_bus\.publish\((.*?)\)',
             r'kernel_bus.emit(\1)'),
            
            # Subscribe methods
            (r'event_bus\.subscribe\((.*?)\)',
             r'kernel_bus.subscribe(\1)'),
            ...
            try:
                ...
                if content != original:
                    with open(py_file, 'w', encoding='utf-8') as f:
                        f.write(content)
                    self.changes.append(("removed_experimental", str(py_file.relative_to(self.workspace))))
...
def main():
    """Run the migration"""
    import time  # Import for examples
