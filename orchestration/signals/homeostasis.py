...
from .signal_bus import Signal, SignalType, SignalBus, get_signal_bus
...
class OscillationDetector:
    ...
    def __init__(self, window_size: int = 20, threshold: float = 0.3):
        ...
        self.signal_history: Dict[SignalType, deque] = {
            sig_type: deque(maxlen=window_size)
...
class HomeostasisController:
    ...
    def compute_modulation(self, signals: List[Signal]) -> ModulationParams:
        ...
        if SignalType.ALIGNMENT_RISK in levels:
            ...
            if "alignment_risk" in maps:
                params.temperature = self._eval_expression(
                    maps["alignment_risk"].get("temperature", "0.7"),
                    {"x": risk}
                )
                params.top_p = self._eval_expression(
                    maps["alignment_risk"].get("top_p", "0.9"),
                    {"x": risk}
                )
                params.reasoning_effort = self._eval_expression(
                    maps["alignment_risk"].get("reasoning_effort", "0.5"),
                    {"x": risk}
                )
        ...
        if SignalType.STRESS in levels:
            ...
            if "stress" in maps:
                params.temperature = min(params.temperature, self._eval_expression(
                    maps["stress"].get("temperature", "0.7"),
                    {"x": stress}
                ))
                params.max_output_tokens = int(self._eval_expression(
                    maps["stress"].get("max_output_tokens", "1024"),
                    {"x": stress}
                ))
        ...
        if SignalType.AMBIGUITY in levels:
            ...
            if "ambiguity" in maps:
                params.reasoning_effort = max(params.reasoning_effort, self._eval_expression(
                    maps["ambiguity"].get("reasoning_effort", "0.5"),
                    {"x": ambiguity}
                ))
                params.retrieval_k = int(self._eval_expression(
                    maps["ambiguity"].get("retrieval_k", "5"),
                    {"x": ambiguity}
                ))
        ...
        if SignalType.URGENCY in levels:
            ...
            if "urgency" in maps:
                params.max_output_tokens = min(params.max_output_tokens, int(self._eval_expression(
                    maps["urgency"].get("max_output_tokens", "1024"),
                    {"x": urgency}
                )))
                params.reasoning_effort = min(params.reasoning_effort, self._eval_expression(
                    maps["urgency"].get("reasoning_effort", "0.5"),
                    {"x": urgency}
                ))
        ...
        if SignalType.NOVELTY in levels:
            ...
            if "novelty" in maps:
                params.temperature = max(params.temperature, self._eval_expression(
                    maps["novelty"].get("temperature", "0.7"),
                    {"x": novelty}
                ))
                params.memory_write_strength = self._eval_expression(
                    maps["novelty"].get("memory_write", "0.5"),
                    {"x": novelty}
                )
        ...
        if SignalType.TRUST in levels:
            ...
            if "trust" in maps:
                params.temperature = self._eval_expression(
                    maps["trust"].get("temperature", "0.7"),
                    {"x": trust}
                )
                params.memory_write_strength = max(params.memory_write_strength, self._eval_expression(
                    maps["trust"].get("memory_write", "0.5"),
                    {"x": trust}
                ))
    ...
    def _calculate_ambiguity(self, text: str) -> float:
        ...
        vague_words = ["maybe", "perhaps", "might", "could", "something", "stuff", "thing", "this", "what"]
