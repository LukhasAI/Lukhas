...
from core.colonies.enhanced_colony import EnhancedReasoningColony, ConsensusResult
from core.colonies.consensus_mechanisms import ColonyConsensus, ConsensusMethod, VoteType
from orchestration.signals.signal_bus import SignalBus, Signal, SignalType
from consciousness.reflection.openai_modulated_service import OpenAIModulatedService, OpenAICapability
...
class GPTColonyOrchestrator:
    ...
    async def _collaborative_orchestration(self, task: OrchestrationTask) -> OrchestrationResult:
        ...
        result = OrchestrationResult(
            task_id=task.task_id,
            mode_used=OrchestrationMode.COLLABORATIVE
        )
    ...
    async def _federated_orchestration(self, task: OrchestrationTask) -> OrchestrationResult:
        ...
        for colony_id, colony in self.colonies.items():
            colony_tasks.append(
                self._process_with_specific_colony(task, colony_id)
            )
    ...
    def _responses_agree(
        ...
        colony_response: ConsensusResult
    ) -> bool:
    ...
    def _update_metrics(self, result: OrchestrationResult):
        ...
        if result.gpt_response:
            self.performance_metrics['gpt_success_rate'] = (
