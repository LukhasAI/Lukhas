...
2. Validate configuration: `python -c "import mcp-server; print('OK')"`
3. Review logs in `logs/mcp-server.log`
...
```bash
# Check module health
python -m mcp-server --health-check

# Validate configuration
python -m mcp-server --validate-config

# Test connections
python -m mcp-server --test-connections

# Generate diagnostic report
python -m mcp-server --diagnostic-report
```
