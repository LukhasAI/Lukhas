...
2. Validate configuration: `python -c "import storage; print('OK')"`
3. Review logs in `logs/storage.log`
...
```bash
# Check module health
python -m storage --health-check

# Validate configuration
python -m storage --validate-config

# Test connections
python -m storage --test-connections

# Generate diagnostic report
python -m storage --diagnostic-report
```
