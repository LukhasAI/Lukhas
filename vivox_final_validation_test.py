...
async def validate_all_improvements():
    ...
    for i in range(100):
        ...
        internal_state = {
            ...
            "pending_decisions": [f"decision_{j}" for j in range(np.random.randint(0, 3))]
        }
        ...
        magnitude = experience.awareness_state.collapse_metadata.get("dimension_magnitude", 0)
    ...
    print(f"Most common state: {state_counts.most_common(1)[0][0]} ({state_counts.most_common(1)[0][1]}%)")
    ...
    for scenario in test_scenarios:
        precedent_analysis = await vivox["moral_alignment"].ethical_precedent_db.analyze_precedents(
    ...
    for scenario in ethical_scenarios:
        ...
        status = "✅" if (decision.approved == (scenario["expected"] == "approve")) else "❌"
    ...
    print(f"  Selected state: {collapsed.selected_state.state_id if collapsed.selected_state else 'None'}")
    print(f"  Collapse reason: {collapsed.collapse_reason}")
    
    if collapsed.selected_state:
        print(f"  Selected amplitude: {collapsed.selected_state.probability_amplitude:.3f}")
        print(f"  Selected emotional signature: {collapsed.selected_state.emotional_signature}")
    ...
    plt.axvline(coherence_array.mean(), color='red', linestyle='--', label=f'Mean: {coherence_array.mean():.3f}')
