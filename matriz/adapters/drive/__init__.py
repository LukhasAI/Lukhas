...
class DriveAdapter(ServiceAdapter):
    ...
    def _generate_mock_drive_files(
        self, limit: int, parent_id: Optional[str], resource_type: Optional[str]
    ) -> list[DriveFileMetadata]:
...
"""
    ...
    def _mock_upload_file(
        self, _parent_id: Optional[str], _name: str, content: bytes, _content_type: str
    ) -> str:
        """Mock file upload"""
    ...
    def _mock_move_file(
        self, file_id: str, new_parent_id: str, _new_name: Optional[str]
    ) -> OperationResult:
