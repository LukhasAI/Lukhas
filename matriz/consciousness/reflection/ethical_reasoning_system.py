...
class ValueAlignmentSystem:
    ...
    async def _update_alignment_metrics(self) -> None:
        ...
        avg_uncertainty = (
            np.mean(list(self.value_uncertainty.values())) if self.value_uncertainty else 0.0
        )
...
class EthicalReasoningSystem:
    ...
    def _extract_principle_weights(
        ...
        normalized_weights = (
            {p: w / total_weight for p, w in weights.items()} if total_weight > 0 else weights
        )
        self.logger.debug(
    ...
    async def get_ethical_system_report(self) -> dict[str, Any]:
        ...
        recent_judgments_24h = [
            ...
            if j.timestamp > datetime.now(timezone.utc) - timedelta(hours=24)
        ]
