...
class AttributionFallbackLadder:
    ...
    async def attribute_conversion(
        ...
        if affiliate_result and affiliate_result.confidence >= self.confidence_thresholds[AttributionMethod.AFFILIATE_LINK]:
        ...
        if receipt_result and receipt_result.confidence >= self.confidence_thresholds[AttributionMethod.RECEIPT_MATCHING]:
        ...
        if behavioral_result and behavioral_result.confidence >= self.confidence_thresholds[AttributionMethod.BEHAVIORAL_INFERENCE]:
        ...
        if last_touch_result and last_touch_result.confidence >= self.confidence_thresholds[AttributionMethod.LAST_TOUCH]:
    ...
    async def _try_affiliate_attribution(self, conversion_event: dict, user_context: dict) -> Optional[AttributionResult]:
    ...
    async def _try_behavioral_inference(self, conversion_event: dict, user_context: dict) -> Optional[AttributionResult]:
    ...
    async def _try_last_touch_attribution(self, conversion_event: dict, user_context: dict) -> Optional[AttributionResult]:
    ...
    def _get_match_factors(self, purchase_details: dict, opportunity: dict) -> dict:
        ...
        return {
            "product_match": opportunity.get("product", "").lower() in str(purchase_details.get("products", [])).lower(),
            "price_variance": abs(opportunity.get("price", 0) - purchase_details.get("amount", 0)) / max(opportunity.get("price", 1), 1),
            "time_gap_hours": abs((datetime.fromisoformat(purchase_details["timestamp"]) - datetime.fromisoformat(opportunity["timestamp"])).total_seconds()) / 3600,
...
async def main():
    ...
    user_context = {
        ...
        "session_history": [
            ...
            {"url": "https://store.example.com/product/headphones", "timestamp": "2024-01-01T10:05:00Z"},
