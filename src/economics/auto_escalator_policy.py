...
class AutoEscalatorPolicy:
    ...
    async def evaluate_tier_escalation(self, user_id: str) -> Dict[str, Any]:
        ...
        all_requirements_met = all(
            result['met'] for result in requirement_results.values()
        )
        ...
        return {
            ...
            'current_split': f"{current_config.user_bps/100:.0f}/{current_config.platform_bps/100:.0f}",
            'next_split': f"{next_config.user_bps/100:.0f}/{next_config.platform_bps/100:.0f}" if next_tier else None,
    ...
    async def apply_tier_escalation(self, user_id: str, force: bool = False) -> Dict[str, Any]:
        ...
        cooldown_expired = (
            not last_escalation or 
            (time.time() - last_escalation) > (self.escalation_cooldown * 24 * 3600)
        )
        ...
        report = {
            ...
            'value_breakdown': {
                'transaction_volume': {
                    ...
                    'contribution_to_score': self.value_weights['transaction_volume'] * min(metrics.monthly_transaction_volume / 1000, 1.0)
                },
                'engagement': {
...
async def main():
    ...
    config = {
        ...
        'bonus_thresholds': {
            ...
            'data_quality_bonus_threshold': 0.9
        }
    }
    ...
    transaction_split = await policy.calculate_transaction_split(
        ...
        opportunity_id="opp_headphones_123"
    )
