    ...
    for component, modules in hybrid_components.items():
        if len(modules) >= 2:
            ...
            hybrid_info = {
                ...
                'description': f"This component exists in multiple modules simultaneously"
            }
...
def consolidate_orphaned_files():
    ...
    orphan_sources = [
        'architectures/', 'systems/', 'modules/', 'components/',
        'tools/', 'utils/', 'helpers/', 'services/'
    ]
...
def main():
    ...
    total_candidates = sum(info['candidates_for_consolidation'] 
                          for info in plan['analysis'].values())
