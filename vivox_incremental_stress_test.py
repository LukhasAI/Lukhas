            ...
            try:
                ...
                for i in range(batch_size):
                    await me.record_decision_mutation(
                        decision={"action": f"mem_test_{i}", "batch": batch_size},
                ...
                print(f"   ✅ Retrieval time: {retrieval_time:.3f}s ({len(resonant)} matches)")
    ...
    async def test_ethical_limits(self):
        ...
        for batch_size in batch_sizes:
            ...
            try:
                ...
                collapsed = await mae.z_collapse_gating(states, {"timestamp": time.time()})
                ...
                result = {
                    ...
                    "suppression_rate": sum(1 for d in decisions if not d.approved) / len(decisions)
                }
        ...
        for batch_size in batch_sizes:
            ...
            try:
                ...
                for i in range(batch_size):
                    ...
                    max_drift = max(max_drift, experience.drift_measurement.drift_amount)
                ...
                print(f"   ✅ Vector collapse ({num_vectors} vectors): {collapse_time:.3f}s")
            ...
            try:
                ...
                for i in range(batch_size):
                    ...
                    if batch_size >= 10000 and i % 10000 == 0 and i > 0:
                        elapsed = time.time() - start_time
                        rate = i / elapsed
                        print(f"   Progress: {i:,}/{batch_size:,} ({rate:.0f} events/s)")
                ...
                report = await srm.structural_conscience_query("test")
                ...
                result = {
                    ...
                    "total_events": len(srm.collapse_archive.collapses) + len(srm.suppression_registry.suppressions)
                }
            ...
            async def single_operation(op_id: int):
                ...
                try:
                    ...
                    decision = await self.vivox["moral_alignment"].evaluate_action_proposal(
                        action, {"emotional_state": {"valence": 0.5, "arousal": 0.5}}
                    )
    ...
    def generate_limits_report(self):
        ...
        if self.performance_curves["memory_expansion"]:
            ...
            for result in self.performance_curves["memory_expansion"]:
                print(f"  {result['batch_size']:,} memories: "
        ...
        if self.performance_curves["moral_alignment"]:
            ...
            for result in self.performance_curves["moral_alignment"]:
                print(f"  {result['batch_size']:,} evaluations: "
        ...
        if self.performance_curves["consciousness"]:
            ...
            for result in self.performance_curves["consciousness"]:
                print(f"  {result['batch_size']:,} experiences: "
        ...
        if self.performance_curves["self_reflection"]:
            ...
            for result in self.performance_curves["self_reflection"]:
                print(f"  {result['batch_size']:,} events: "
        ...
        if "memory_creation" in self.breaking_points:
            ...
            print(f"  • Memory limit: Keep under {limit * 0.8:,.0f} memories for optimal performance")
        
        if "ethical_evaluation" in self.breaking_points:
            limit = self.breaking_points["ethical_evaluation"]
            print(f"  • Ethical evaluations: Batch in groups of {limit // 10:,} for best throughput")
        
        if "consciousness_simulation" in self.breaking_points:
            limit = self.breaking_points["consciousness_simulation"]
            print(f"  • Consciousness experiences: Process max {limit // 2:,} simultaneously")
