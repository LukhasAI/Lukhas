...
class LUKHASSystemDiagnostic:
    ...
    def check_python_environment(self):
        ...
        try:
            ...
            # Key dependencies
            dependencies = [
                "fastapi", "uvicorn", "openai", "anthropic", "pydantic", 
                "pytest", "numpy", "requests", "python-dotenv", "pathlib"
            ]
    ...
    def identify_issues(self):
        ...
        # Check core modules
        failed_modules = [name for name, data in self.results["core_modules"].items() 
        ...
        missing_apis = [name for name, data in self.results["api_credentials"].items()
        ...
        # Check test results
        failed_tests = [name for name, data in self.results["test_results"].items()
    ...
    def generate_summary(self):
        ...
        working_modules = len([m for m in self.results["core_modules"].values() 
                              if m["status"] == "working"])
        module_health = (working_modules / total_modules * 100) if total_modules > 0 else 0
        
        total_apis = len(self.results["api_credentials"])
        available_apis = len([a for a in self.results["api_credentials"].values()
                             if a["status"] == "✅ Available"])
        api_coverage = (available_apis / total_apis * 100) if total_apis > 0 else 0
        
        critical_issues = len([i for i in self.results["system_issues"] 
                              if i["category"] == "critical"])
        warning_issues = len([i for i in self.results["system_issues"] 
                             if i["category"] == "warning"])
    ...
    def print_summary(self):
        ...
        print(f"🧬 Module Health: {summary.get('module_health_percentage', 0)}% ({summary.get('working_modules', 0)}/{summary.get('total_modules_tested', 0)})")
        print(f"🔑 API Coverage: {summary.get('api_coverage_percentage', 0)}% ({summary.get('available_apis', 0)}/{summary.get('total_apis', 0)})")
