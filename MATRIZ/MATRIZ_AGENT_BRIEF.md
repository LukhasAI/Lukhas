...
---
{
  ...
  "title": "MATRIZ Node v1 (schema v1.1)",
  "type": "object",
...
# MATRIZ Agent Brief (v1.1)

> **Prime Directive:** “No node → no go.” All advisory artifacts MUST emit at least one valid **MATRIZ node**.

## Shared Contracts (all agents)

- **Schema:** `../matriz_node_v1.json` (schema v1.1 with `labels` and `provenance.colony`)
- **Validation:** Use `utils/matriz_validate.py::validate_nodes`
...
- **Outputs:** Events containing `matriz_node_ids`, `schema_ref`, `trace_id`.
...
- **Simulation → Writer:** `{ shards, scores, trace_id, matriz_nodes, schema_ref }`
- **Writer → Storage:** atomic JSON (shards + matriz_nodes)
