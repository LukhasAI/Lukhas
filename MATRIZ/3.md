...
collect() MUST validate each node against matriz_node_v1.json before returning. Deterministic seeds → stable id shape (e.g., LT-<trace>#N<index>) for golden tests. Canary tests fail on any schema drift.