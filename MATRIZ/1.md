...
**Contract:** Every Skill Capsule MUST emit a `node_out` conforming to `matriz_node_v1.json` (`$id: lukhas://schemas/matriz_node_v1.json`). The orchestrator treats nodes as first-class outputs and routes by declared `capabilities` + schema compatibility.
